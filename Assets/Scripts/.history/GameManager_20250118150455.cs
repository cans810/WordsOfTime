...
public class GameManager : MonoBehaviour
{
    ...
    public void StoreHintUsage(string word)
    {
        ...
        Debug.Log($"Stored hint for {word} in {CurrentEra}. Total hints used: {usedHints[key]}");
    }

    // Get hint level for a specific word
    ...
    public bool CanUseHint(string word)
    {
        string key = $"{CurrentEra}_{word}";
        int currentHints = usedHints.ContainsKey(key) ? usedHints[key] : 0;
        return currentHints < 2; // Maximum 2 hints per word
    }
    ...
    // Use hint and deduct points (this replaces the existing UseHint method)
    public void UseHint(int hintLevel)
    {
        int cost = hintLevel == 1 ? HINT_COST : SECOND_HINT_COST;
        if (CurrentPoints >= cost)
