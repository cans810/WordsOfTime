...
public class WordGameManager : MonoBehaviour
{
    ...
    public void StartNewGameInEra()
    {
        ...
        Debug.Log($"Got {currentEraWords.Count} words for era {GameManager.Instance.CurrentEra}");
        
        // Convert solved words to indices
        solvedWordsInCurrentEra = new HashSet<int>();
        var solvedWordsStrings = GameManager.Instance.GetSolvedWordsForEra(GameManager.Instance.CurrentEra);
        foreach (string word in solvedWordsStrings)
        {
    ...
    public void LoadWord(int index)
    {
        ...
        Debug.Log($"Loading word: {targetWord} with sentence: {sentence}");

        if (sentence == null)
        {
            Debug.LogError($"No sentence found for word {targetWord}");
            return;
        }
    ...
    private void AddPoints()
    {
        ...
        // Store the solved word
        GameManager.Instance.StoreSolvedWord(GameManager.Instance.CurrentEra, targetWord);
    ...
    {
        Debug.Log("Language changed, refreshing game state");
        StartNewGameInEra();
        UpdateSentenceDisplay();
        MarkSolvedWords();
    }

    private void MarkSolvedWords()
    {
        if (GameManager.Instance == null || GridManager.Instance == null) return;
        ...
        foreach (string word in solvedWordsStrings)
        {
            ...
            {
                var positions = GameManager.Instance.SolvedWordPositions[word];
                var tiles = positions.Select(pos => GridManager.Instance.GetTileAtPosition(pos)).ToList();
