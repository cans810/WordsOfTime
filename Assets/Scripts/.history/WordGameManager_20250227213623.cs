...
public class WordGameManager : MonoBehaviour
{
                    ...
                    if (wordIndex != -1)
                    {
                        ...
                        if (translatedWord.Equals(targetWord, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            else
                            {
                                ...
                                const int maxGridSize = 6; // Use a constant for grid size instead of accessing the private field
    ...
    private void ProcessSolvedWords()
    {
        try
        {
            Debug.Log($"[Android Debug] Processing solved words for era {GameManager.Instance.CurrentEra}");
            ...
            if (currentWordIndex < currentEraWords.Count)
            {
                ...
                foreach (string baseWord in solvedBaseWords)
                {
                    ...
                    {
                        Debug.LogWarning($"[Android Debug] Translated word {translatedWord} not found in current era words");
                    }
                }
            }
            else
