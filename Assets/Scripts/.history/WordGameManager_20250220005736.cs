...
public class WordGameManager : MonoBehaviour
{
    ...
    {
        Debug.Log($"Starting point animation from {GameManager.Instance.CurrentPoints} with change of {pointsToAdd}");
        ...
        if (pointText == null)
        {
            Debug.LogError("Point text is null!");
            yield break;
        }
        ...
        int startPoints = GameManager.Instance.CurrentPoints - pointsToAdd; // Start from before the change
        int targetPoints = GameManager.Instance.CurrentPoints; // Target is current points (after change)
        ...
        // Update hint button state after animation
        UpdateHintButton();
