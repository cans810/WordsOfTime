...
public class GridManager : MonoBehaviour
{
    ...
    [SerializeField] public int gridSize = 6;
    ...
    {
        Debug.Log($"Setting up new puzzle for word: {word}");
        ...
        {
            Debug.Log($"Word {baseWord} is already solved, highlighting in grid");
            
            // Try to get the stored positions for this word
            List<Vector2Int> positions = GameManager.Instance.GetSolvedWordPositions(word);
            ...
            {
                // If positions aren't stored, try to find the word in the grid
                WordGameManager.Instance.HighlightWordInGrid(word);
            }
        }
    ...
    {
        // Remove the condition that checks if the word matches the current word
        // This allows solved words to be displayed in all eras
        if (positions != null)
        {
            foreach (Vector2Int pos in positions)
            {
    ...
    public void UpdateSolvedWordsDisplay()
    {
        if (GameManager.Instance != null)
        {
            ...
            foreach (var baseWord in GameManager.Instance.GetSolvedBaseWordsForEra(GameManager.Instance.CurrentEra))
            {
                ...
                Debug.Log($"Updating solved word display for base word: {baseWord}, current language word: {currentLanguageWord}");
                ...
                if (GameManager.Instance.GetSolvedWordPositions(currentLanguageWord, out List<Vector2Int> positions))
                {
                    ...
                    Debug.Log($"Found stored positions for {currentLanguageWord}, highlighting in grid");
                }
                else
                {
                    Debug.LogWarning($"No stored positions found for {currentLanguageWord}");
                    
                    // If this is the current word, try to highlight it using WordGameManager
                    if (currentLanguageWord == currentWord && WordGameManager.Instance != null)
                    {
                        WordGameManager.Instance.HighlightWordInGrid(currentLanguageWord);
                    }
                }
            }
        }
    ...
    public List<Vector2Int> FindWordInGrid(string word, LetterTile[,] grid)
    {
        ...
        // Search the grid for the word
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                if (grid[x, y] != null && grid[x, y].GetLetter().ToString() == word[0].ToString())
                {
                    // Start DFS search from this position
                    bool[,] visited = new bool[gridSize, gridSize];
                    if (DFS(x, y, word, 0, visited, positions))
                    {
                        return positions;
                    }
                }
    ...
    private bool DFS(int x, int y, string word, int index, bool[,] visited, List<Vector2Int> positions)
    {
        ...
        if (x < 0 || x >= gridSize || y < 0 || y >= gridSize || 
            ...
            grid[x, y].GetLetter().ToString() != word[index].ToString())
        {
            return false;
        }
        ...
        for (int i = 0; i < 8; i++)
        {
            if (DFS(x + dx[i], y + dy[i], word, index + 1, visited, positions))
            {
                return true;
            }
        }
