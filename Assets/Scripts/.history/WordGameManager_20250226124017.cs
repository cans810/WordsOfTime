...
public class WordGameManager : MonoBehaviour
{
    ...
    public void StartNewGameInEra()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            // First, check if there are any unsolved words
            ...
            // If all words are solved, just load the first word
            if (allWordsSolved && currentEraWords.Count > 0)
            {
                Debug.Log("All words are solved, loading first word");
                currentWordIndex = 0;
            }
    ...
    public void LoadWord(int index)
    {
        ...
        if (isSolved)
        {
            ...
            // Make sure the hint button shows "Solved" or "Çözüldü" based on language
            UpdateHintButton();
        }
        else
        {
    ...
    public void HighlightWordInGrid(string word)
    {
        ...
        // Try to get the stored positions for the translated word
        List<Vector2Int> positions = GameManager.Instance.GetSolvedWordPositions(translatedWord);
        bool wordFound = false;
        
        if (positions != null && positions.Count > 0)
        {
            // Use the stored positions to highlight the word
            foreach (Vector2Int pos in positions)
            {
                ...
                {
                    GridManager.Instance.grid[pos.x, pos.y].SetSolvedColor();
                    GridManager.Instance.grid[pos.x, pos.y].isSolved = true;
                    GridManager.Instance.grid[pos.x, pos.y].GetComponent<Image>().raycastTarget = false;
                }
            }
            wordFound = true;
            Debug.Log($"Used stored positions to highlight word {translatedWord}");
        }
        else
        {
            // Fall back to searching for the word in the grid
    ...
    private void UpdateHintButton()
    {
        ...
        hintButton.interactable = !isSolved && CanUseHint();
        hintButtonText.text = isSolved ? 
            (GameManager.Instance.CurrentLanguage == "tr" ? "Çözüldü" : "Solved") : 
            GetHintButtonText();
    }

    private bool CanUseHint()
    {
        // Implement the logic to determine if the player can use a hint
        // This is a placeholder and should be replaced with the actual implementation
        return true; // Placeholder return, actual implementation needed
    }

    private string GetHintButtonText()
    {
        // Implement the logic to get the hint button text
        // This is a placeholder and should be replaced with the actual implementation
        return "Hint"; // Placeholder return, actual implementation needed
    }
