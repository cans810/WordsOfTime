...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private IEnumerator SpinWheel()
    {
        ...
        // Add pre-spin animation
        float preSpinDuration = 1.5f;
        float preSpinElapsed = 0f;
        float startRotation = wheel.eulerAngles.z;
        float preSpinTarget = startRotation - 120f; // Rotate back 120 degrees
        ...
        // Adjust target rotation to account for pre-spin and full rotations
        float targetRotation = targetAngle + (360f * 5) - (startRotation - preSpinTarget);
        ...
        Debug.Log($"Start Rotation: {startRotation}, Pre-spin Target: {preSpinTarget}, Final Target: {targetRotation}");
