    ...
    public void SelectEra(string era)
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            if (isUnlocked)
            {
                ...
                // If we're in the GameScene, reload the word game with the new era
                if (SceneManager.GetActiveScene().name == "GameScene" && WordGameManager.Instance != null)
                {
                    // Trigger a refresh of the game content with the new era
                    WordGameManager.Instance.StartNewGameInEra();
                    
                    // Force the canvas to deactivate - this will make the change feel more immediate
