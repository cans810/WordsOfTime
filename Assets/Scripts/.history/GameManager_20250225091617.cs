...
public class GameManager : MonoBehaviour
{
    ...
    private int wordsGuessedSinceLastAd = 0;
    ...
    public void AddGuessedWord(string word)
    {
        ...
        // Call OnWordGuessed to handle ad logic
        OnWordGuessed();
    }
    ...
    {
        if (noAdsBought) return; // Skip ad logic if user bought no ads

        wordsGuessedSinceLastAd++;
        Debug.Log($"Words guessed since last ad: {wordsGuessedSinceLastAd}/{WORDS_BEFORE_AD}");
        
        if (wordsGuessedSinceLastAd >= WORDS_BEFORE_AD)
        {
            Debug.Log("Playing ad after 3 words");
            if (AdManager.Instance != null)
            {
                AdManager.Instance.ShowInterstitialAd();
                wordsGuessedSinceLastAd = 0; // Reset counter after showing ad
            }
            else
            {
