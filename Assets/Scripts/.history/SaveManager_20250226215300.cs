    ...
    public void LoadGame()
    {
        ...
        try
        {
            if (File.Exists(SavePath))
            {
                ...
                // Load solved words if they exist
                if (Data.solvedWords != null && Data.solvedWords.Count > 0)
                {
                    ...
                    // Find WordGameManager instance and load the solved words
                    WordGameManager wordGameManager = FindObjectOfType<WordGameManager>();
                    if (wordGameManager != null)
                    {
                        wordGameManager.LoadSolvedWords(Data.solvedWords);
                    }
                    else
                    {
                        Debug.LogWarning("WordGameManager not found, solved words will be loaded when it initializes");
                    }
                }
                else
                {
