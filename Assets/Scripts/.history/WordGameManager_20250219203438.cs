...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator AnimatePointsIncrease(int pointsToAdd)
    {
        ...
        while (elapsedTime < POINT_ANIMATION_DURATION)
        {
            ...
            if (pointText != null)
            {
                pointText.text = currentPoints.ToString(); // Just the number
        ...
        if (pointText != null)
        {
            pointText.text = targetPoints.ToString(); // Just the number
            pointText.transform.localScale = originalScale;
            pointText.color = originalColor;
        }

        // Store hint usage and deduct points
        GameManager.Instance.AddPoints(pointsToAdd); // This should be the ONLY place points are added/removed
    }
