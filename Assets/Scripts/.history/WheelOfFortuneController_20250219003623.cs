...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin, System.Action onDestroy)
    {
        ...
        // Introduce a small random variation to the duration
        ...
        coin.transform.position = endPoint;
    ...
    {
        // Assuming you have a method to determine the number of coins
        int numberOfCoins = DetermineNumberOfCoins();

        for (int i = 0; i < numberOfCoins; i++)
        {
            // Instantiate the coin at the desired start position
            GameObject coin = Instantiate(coinForAnimationPrefab, coinSpawnPoint.transform.position, Quaternion.identity, safeArea.transform);

            // Start the coroutine to move the coin immediately
            StartCoroutine(MoveCoinToPanel(coin, () => {
                // Optional: Add any additional logic after the coin is destroyed
            }));
        }
    }
}
