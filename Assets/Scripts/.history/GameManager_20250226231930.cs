...
public class GameManager : MonoBehaviour
{
    ...
    private IEnumerator LoadWordsFromJsonAndroid()
    {
        ...
        try
        {
            ...
            Debug.Log($"[GameManager] Loading English words from: {enFilePath}");
            
            using (UnityWebRequest enRequest = UnityWebRequest.Get(enFilePath))
            {
                ...
                if (enRequest.result == UnityWebRequest.Result.Success)
                {
                    ...
                    Debug.Log($"[GameManager] Successfully loaded English words, JSON length: {enJsonContent.Length}");
                    LoadLanguageWords(enJsonContent, "en", false);
                }
                else
                {
                    Debug.LogError($"[GameManager] Failed to load English words: {enRequest.error}");
                }
            }
            
            // For Turkish words
            string trFilePath = Path.Combine(Application.streamingAssetsPath, "words_tr.json");
            Debug.Log($"[GameManager] Loading Turkish words from: {trFilePath}");
            
            using (UnityWebRequest trRequest = UnityWebRequest.Get(trFilePath))
            {
                ...
                if (trRequest.result == UnityWebRequest.Result.Success)
                {
                    ...
                    Debug.Log($"[GameManager] Successfully loaded Turkish words, JSON length: {trJsonContent.Length}");
                    LoadLanguageWords(trJsonContent, "tr", false);
                }
                else
                {
                    Debug.LogError($"[GameManager] Failed to load Turkish words: {trRequest.error}");
                }
            }
        ...
        {
            Debug.LogError($"[GameManager] Error loading words from JSON on Android: {e.Message}");
        }
    }
    #endif
