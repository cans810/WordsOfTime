...
public class WordGameManager : MonoBehaviour
{
    ...
    private void CheckWord(string word, List<LetterTile> selectedTiles)
    {
        ...
        if (word == targetWord)
        {
            ...
            // Store positions for the current word with the actual word as the key
            ...
            GameManager.Instance.AddPoints(pointsToAward);

            // Trigger animation for adding points
            numberAnimationCoroutine = StartCoroutine(AnimateNumberChange(currentPointsText, GameManager.Instance.CurrentPoints - pointsToAward, GameManager.Instance.CurrentPoints));

            // Show "Did You Know" panel for the word
            StartCoroutine(ShowDidYouKnowAfterDelay(1.5f));
            
            // Check if word is out of order and show appropriate sentence
            ...
            // Get a completion sentence in the current language
            string completionText = GameManager.Instance.CurrentLanguage == "tr" ? "KELİME BULUNDU!" : "WORD FOUND!";
            sentenceText.text = $"<color=#FFD700>{completionText}</color>";
            
            // Play the word guessed animation
            PlayWordGuessedAnimation();

            // Increment the counter for tracking when to show ads
            wordsGuessedCount++;
            if (wordsGuessedCount >= WORDS_BEFORE_AD)
            {
                ...
                if (AdManager.Instance != null && AdManager.Instance.IsInterstitialAdAvailable())
                {
                    Debug.Log("Showing interstitial ad after guessing 3 words");
    ...
    private void HandleLanguageChanged()
    {
                    ...
                    if (wordIndex != -1)
                    {
                        ...
                        if (translatedWord.Equals(targetWord, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (positions == null || positions.Count == 0)
                            {
                                ...
                                // If still no positions, check words in all languages
                                if (positions == null || positions.Count == 0)
                                {
                                    Debug.Log("[Android Debug] Still no positions found, checking all solved word positions");
                                    
                                    // Dump all positions we have stored to debug
                                    foreach (var entry in GameManager.Instance.SolvedWordPositions)
                                    {
                                        Debug.Log($"[Android Debug] Found positions for word: {entry.Key}, count: {entry.Value.Count}");
                                    }
                                    
                                    // Try English and Turkish translations again explicitly
                            ...
                            else
                            {
                                ...
                                // If we have the current grid positions, we could use them to mark the tiles
                                // This is a workaround for when we can't find the stored positions
                                if (GridManager.Instance.targetWord == targetWord)
                                {
                                    Debug.Log("[Android Debug] Using current grid to mark solved tiles as fallback");
                                    // Mark all tiles that match the letters of the target word
                                    // This is a rough estimation and may not work perfectly
                                    for (int x = 0; x < GridManager.Instance.gridSize; x++)
                                    {
                                        for (int y = 0; y < GridManager.Instance.gridSize; y++)
                                        {
                                            LetterTile tile = GridManager.Instance.GetTileAtPosition(new Vector2Int(x, y));
                                            if (tile != null && targetWord.Contains(tile.GetLetter()))
                                            {
                                                // Only mark tiles that match letters in the target word
                                                // This isn't perfect but helps with visualization
                                                tile.SetSolvedColor();
                                                tile.isSolved = true;
                                                tile.GetComponent<Image>().raycastTarget = false;
                                                Debug.Log($"[Android Debug] Marked matching tile at position {x},{y} with letter {tile.GetLetter()} as solved (fallback)");
                                            }
                                        }
                                    }
                                }
        ...
        Debug.Log("[Android Debug] Language change processing complete");
    }
