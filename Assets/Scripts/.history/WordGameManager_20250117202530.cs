...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator AnimatePointsChange(int startPoints, int endPoints)
    {
        ...
        while (elapsedTime < POINT_ANIMATION_DURATION)
        {
            ...
            if (currentPoints != previousPoints)
            {
                ...
                while (bumpTime < BUMP_DURATION)
                {
                    ...
                    // Change color based on increasing or decreasing
            ...
            if (gameSceneCanvasController != null)
            {
                gameSceneCanvasController.UpdateHintButtonText();
            }
        ...
        pointText.transform.localScale = originalScale;
        pointText.color = Color.white; // Reset color
    ...
    public void StartNewGameInEra()
    {
        ...
        // Get and shuffle words for the era
        currentEraWords = WordValidator.GetWordsForEra(GameManager.Instance.CurrentEra);
        ShuffleWords();
        Debug.Log($"Got and shuffled {currentEraWords.Count} words for era {GameManager.Instance.CurrentEra}"); // Debug log
    ...
    private void ShuffleWords()
    {
        ...
        while (n > 1)
        {
            n--;
            int k = rng.Next(n + 1);
            string temp = currentEraWords[k];
            currentEraWords[k] = currentEraWords[n];
            currentEraWords[n] = temp;
        }
        
        Debug.Log("Words shuffled: " + string.Join(", ", currentEraWords)); // Debug log to see the shuffled order
    }
    ...
    {
        int hintCost = gameSceneCanvasController.GetHintCost();
        if (GameManager.Instance.CanUseHint(gameSceneCanvasController.GetHintLevel()))
        {
            // Start points decrease animation before using hint
            int startPoints = GameManager.Instance.CurrentPoints;
            GameManager.Instance.UseHint(gameSceneCanvasController.GetHintLevel());
            StartCoroutine(AnimatePointsChange(startPoints, GameManager.Instance.CurrentPoints));

            // Give the actual hint
            if (gameSceneCanvasController.GetHintLevel() == 1)
            {
                GridManager.Instance.HighlightFirstLetter(targetWord[0]);
            }
            else
            {
                // Second level hint logic
            }
            gameSceneCanvasController.IncrementHintLevel();
        }
    }
