...
public class GameManager : MonoBehaviour
{
    ...
    public Dictionary<string, HashSet<int>> SolvedWordsPerEra => solvedWordsPerEra;
    ...
    public HashSet<string> GetSolvedWordsForEra(string era)
    {
        // With our new approach, we don't track solved words by era anymore
        // Just return all solved words
        return new HashSet<string>(solvedWords);
    }
    ...
    public void SaveGridData(out List<GridData> gridDataList)
    {
        ...
        foreach (var kvp in initialGrids)
        {
            ...
            // We no longer track word positions
            // Just initialize an empty list for backward compatibility
            gridData.correctWordPositions = new List<Vector2IntSerializable>();
    ...
    {
        // Return the list of solved words directly
        return new List<string>(solvedWords);
    }
    ...
    public void SetGuessedWords(List<string> words)
    {
        ...
        // Add all words to the solvedWords HashSet
        foreach (string word in words)
        {
            solvedWords.Add(word.ToUpper());
            Debug.Log($"Added word '{word}' to solved words");
        }
        
        // Initialize solvedWordsPerEra for backward compatibility
        solvedWordsPerEra.Clear();
        foreach (string era in eraList)
        {
        ...
        Debug.Log($"Set {solvedWords.Count} solved words: {string.Join(", ", solvedWords)}");
    }
    ...
    public bool IsWordGuessed(string word)
    {
        ...
        bool isSolved = solvedWords.Contains(baseWord.ToUpper());
    ...
    private void LoadSettings()
    {
        if (SaveManager.Instance != null && SaveManager.Instance.Data != null)
        {
            ...
            {
                solvedWords = new HashSet<string>(SaveManager.Instance.Data.guessedWords);
                Debug.Log($"Loaded {solvedWords.Count} guessed words");
            }
            else
            {
    ...
    {
        // Get the word from the current era words using the index
        if (eraWordsPerLanguage.ContainsKey(CurrentLanguage) && 
            eraWordsPerLanguage[CurrentLanguage].ContainsKey(era) &&
            wordIndex >= 0 && wordIndex < eraWordsPerLanguage[CurrentLanguage][era].Count)
        {
            string word = eraWordsPerLanguage[CurrentLanguage][era][wordIndex];
            
            // Add the word to the solvedWords HashSet
            solvedWords.Add(word.ToUpper());
            
            // Also add to the era-specific tracking for backward compatibility
            if (!solvedWordsPerEra.ContainsKey(era))
            {
                solvedWordsPerEra[era] = new HashSet<int>();
            }
            ...
            Debug.Log($"[GameManager] Stored solved word '{word}' (index {wordIndex}) for era '{era}'");
        }
        else
        {
            Debug.LogWarning($"[GameManager] Could not find word at index {wordIndex} for era '{era}'");
        }
    }
}