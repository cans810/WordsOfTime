...
public class GridManager : MonoBehaviour
{
    ...
    private IEnumerator GenerateGridCoroutine(List<char> letters)
    {
        ...
        // Use a longer delay to make the sequence very obvious
        ...
        // Now reveal each tile one by one with animation
        for (int y = 0; y < gridSize; y++)
        {
            for (int x = 0; x < gridSize; x++)
            {
                if (grid[x, y] != null)
                {
                    GameObject tileObj = grid[x, y].gameObject;
                    
                    // Make the tile visible but at zero scale
                    ...
                    AnimationCurve curve = new AnimationCurve(
                        new Keyframe(0f, 0f),
                        new Keyframe(0.7f, 1.2f), // Bigger overshoot for more visible effect
                        new Keyframe(1f, 1f)
                    );
                    
                    // Animate this single tile completely before moving to the next one
                    ...
                    // Wait before showing the next tile
