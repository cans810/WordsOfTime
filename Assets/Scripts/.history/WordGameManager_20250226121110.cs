...
public class WordGameManager : MonoBehaviour
{
    ...
    private void HighlightWordInGrid(string word)
    {
        ...
        // Check if the word is solved in any era, not just the current one
        bool isSolved = false;
        foreach (var era in GameManager.Instance.EraList)
        {
            if (GameManager.Instance.GetSolvedBaseWordsForEra(era).Contains(baseWord))
            {
                isSolved = true;
                break;
            }
        ...
        if (isSolved)
        {
            GridManager.Instance.HighlightWord(translatedWord);
        }
        else
        {
            Debug.Log($"Word {translatedWord} is not solved in any era");
        }
    }
    ...
    private void UpdateHintButton()
    {
        if (hintButton != null)
        {
            ...
            // Check if the word is solved in any era
            bool isSolved = false;
            foreach (var era in GameManager.Instance.EraList)
            {
                if (GameManager.Instance.GetSolvedBaseWordsForEra(era).Contains(baseWord))
    ...
    {
        Debug.Log("Language changed, updating grid and UI");
        
        // Get the base word for the current word
        string baseWord = GameManager.Instance.GetBaseWord(targetWord);
        
        // Get the translation for the new language
        string translatedWord = GameManager.Instance.GetTranslation(baseWord, GameManager.Instance.CurrentLanguage);
        
        // Update the grid with the translated word
        if (GameManager.Instance.IsWordSolved(baseWord))
        {
            Debug.Log($"Word {baseWord} is solved, highlighting {translatedWord} in grid");
            GridManager.Instance.HighlightWord(translatedWord);
        }
        
        // Update the sentence display
        UpdateSentenceDisplay(translatedWord);
