...
public static class WordValidator
{
    ...
    public static string GetFactForWord(string word, string era, string language = "en")
    {
        ...
        if (GameManager.Instance.CurrentLanguage == "tr")
        {
            ...
            // If not found, try to get the Turkish translation of the base word
            string baseWord = GameManager.Instance.GetBaseWord(word);
            string turkishWord = GameManager.Instance.GetTranslation(baseWord, "tr");
            if (wordSetsWithFacts[era].ContainsKey(turkishWord.ToUpper()))
            {
        ...
        string baseWordFallback = GameManager.Instance.GetBaseWord(word);
        
        // Fallback to English fact
        if (wordSetsWithFacts[era].ContainsKey(baseWordFallback.ToUpper()))
        {
            return wordSetsWithFacts[era][baseWordFallback.ToUpper()];
        }
