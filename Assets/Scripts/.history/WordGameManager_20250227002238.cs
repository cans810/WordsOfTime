...
public class WordGameManager : MonoBehaviour
{
    ...
    public void ShowDidYouKnow()
    {
        try
        {
            Debug.Log("[DidYouKnow Debug] ShowDidYouKnow called");
            
            // Check if the panel is already active
            if (didYouKnowPanel != null && didYouKnowPanel.activeSelf)
            {
                Debug.Log("[DidYouKnow Debug] Panel is already active, ignoring call");
                return;
            }
            ...
            if (string.IsNullOrEmpty(targetWord))
            {
                Debug.LogWarning("[DidYouKnow Debug] Current word is null or empty");
                return;
            }
            
            // Check if the word is solved
            bool isSolved = IsWordSolved(targetWord);
            Debug.Log($"[DidYouKnow Debug] Is word '{targetWord}' solved: {isSolved}");
            
            if (!isSolved)
            {
                Debug.Log("[DidYouKnow Debug] Word is not solved yet, not showing fact");
                return;
            }
            
            // Get the current era and language
            string era = currentEra;
            string language = GameManager.Instance.GetCurrentLanguage();
            
            Debug.Log($"[DidYouKnow Debug] Getting fact for word: {targetWord}, era: {era}, language: {language}");
            
            // Try to get the fact for the word
            string fact = WordValidator.GetFactForWord(targetWord, era, language);
            
            // If fact is empty, try with English language
            if (string.IsNullOrEmpty(fact) && language != "en")
            {
                Debug.Log("[DidYouKnow Debug] Fact is empty, trying with English language");
                fact = WordValidator.GetFactForWord(targetWord, era, "en");
            }
            
            // If still empty, try with the base word
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        // Check if didYouKnowText is null before proceeding
        if (didYouKnowText == null)
        {
        ...
        try
        {
            ...
            {
                Debug.LogError("[DidYouKnow Debug] didYouKnowPanel is null when trying to activate it!");
                EnableNavigationButtons();
            }
        }
        catch (System.Exception e)
