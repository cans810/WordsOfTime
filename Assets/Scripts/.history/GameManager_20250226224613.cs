...
public class GameManager : MonoBehaviour
{
    ...
    public bool NoAdsBought
    {
        get
        {
            // First check the local field for performance
            if (noAdsBought) return true;
            
            // Then check the saved data if available
            if (SaveManager.Instance != null && SaveManager.Instance.Data != null)
            {
                bool noAds = SaveManager.Instance.Data.noAdsBought;
                Debug.Log($"NoAdsBought checked from SaveManager: {noAds}");
                
                // Update the local field for future checks
                if (noAds) noAdsBought = true;
                
                return noAds;
            }
            
            Debug.LogWarning("SaveManager or Data is null when checking NoAdsBought");
            return noAdsBought; // Return the local value if SaveManager is not available
        }
    }
    ...
    {
        Debug.Log($"SetPoints: Setting points to {points}");
        if (points < 0)
        {
            Debug.LogWarning("Attempted to set negative points value. Using 0 instead.");
            points = 0;
        }
        ...
        try
        {
            ...
            if (pointsText != null)
            {
                ...
                Debug.Log($"Updated points UI to display {CurrentPoints}");
            }
            else
            {
                Debug.LogWarning("Points UI reference is null, cannot update display");
            }
            ...
            {
                OnPointsChanged.Invoke(CurrentPoints);
                Debug.Log("Notified subscribers of points change");
            }
            
            Debug.Log($"SetPoints complete: CurrentPoints = {CurrentPoints}");
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error in SetPoints: {ex.Message}");
        }
    }
    ...
    public void SetGuessedWords(List<string> words)
    {
        ...
        // If no words to load, just return after initialization
        if (words == null || words.Count == 0)
        {
            Debug.Log("No guessed words to set");
            return;
        }
        ...
        foreach (string word in words)
        {
            ...
            {
                Debug.LogWarning("Skipping empty word in guessed words list");
                continue;
            }
            ...
            try
            {
                ...
                {
                    int wordIndex = -1;
                    List<string> eraWords = eraWordsPerLanguage["en"][era];
                    
                    // Try case-insensitive match first
                    for (int i = 0; i < eraWords.Count; i++)
                    {
                        if (string.Equals(eraWords[i], word, StringComparison.OrdinalIgnoreCase))
                        {
                            wordIndex = i;
                            break;
                        }
            ...
            {
                Debug.LogError($"Error processing guessed word '{word}': {ex.Message}");
                failCount++;
            }
        }
        
        // Log the results for debugging
        Debug.Log($"SetGuessedWords complete: {successCount} words added successfully, {failCount} failed");
        foreach (var era in solvedBaseWordsPerEra.Keys)
        {
            Debug.Log($"Era '{era}' has {solvedBaseWordsPerEra[era].Count} solved words");
        }
    }
    ...
    public void SetNoAdsBought(bool value)
    {
        ...
        // Update SaveManager.Data if available
        if (SaveManager.Instance != null && SaveManager.Instance.Data != null)
        {
            SaveManager.Instance.Data.noAdsBought = value;
            Debug.Log($"No Ads bought state set to: {value} in both local cache and SaveManager.Data");
            
            // Save the game state to persist the change
            SaveManager.Instance.SaveGame();
        }
        else
        {
            Debug.LogWarning($"No Ads bought state set to: {value} in local cache only. SaveManager not available.");
        }
    }
