...
public class WordGameManager : MonoBehaviour
{
    ...
    private void ShowAd()
    {
        ...
        if (AdManager.Instance != null)
        {
            try
            {
                ...
                // Reset the counter regardless of whether ad was shown
                wordsGuessedCount = 0;
        ...
        else
        {
            ...
            // Reset counter even if AdManager is not available
            wordsGuessedCount = 0;
        }
    }
    ...
    {
        Debug.Log($"NextWord called, isAnimationPlaying: {isAnimationPlaying}");
        if (isAnimationPlaying)
        {
            Debug.Log("Animation is playing, ignoring NextWord call");
            return;
        }
        ...
        if (currentWordIndex < currentEraWords.Count - 1)
        {
            ...
            UpdateProgressBar();
            UpdateSentenceDisplay();
            Debug.Log($"Moved to next word, index: {currentWordIndex}");
        }
        else
        {
            Debug.Log($"Already at last word, index: {currentWordIndex}");
        }
    ...
    {
        Debug.Log($"PreviousWord called, isAnimationPlaying: {isAnimationPlaying}");
        if (isAnimationPlaying)
        {
            Debug.Log("Animation is playing, ignoring PreviousWord call");
            return;
        }
        ...
        if (currentWordIndex > 0)
        {
            ...
            UpdateProgressBar();
            UpdateSentenceDisplay();
            Debug.Log($"Moved to previous word, index: {currentWordIndex}");
        }
        else
        {
            Debug.Log($"Already at first word, index: {currentWordIndex}");
        }
    }
