...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    [SerializeField] private float pointAnimationDuration = 1f;
    [SerializeField] private TextMeshProUGUI pointsText; // Reference to the points text
    ...
    private IEnumerator AnimatePointsIncrease(int pointsToAdd)
    {
        ...
        while (elapsedTime < pointAnimationDuration)
        {
            ...
            float progress = elapsedTime / pointAnimationDuration;

            // Use smooth step for better animation feel
            progress = Mathf.SmoothStep(0, 1, progress);

            // Calculate current points
            int currentPoints = (int)Mathf.Lerp(startPoints, targetPoints, progress);
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin)
    {
        ...
        float smoothTime = coinSmoothTime + Random.Range(-0.2f, 0.2f);
