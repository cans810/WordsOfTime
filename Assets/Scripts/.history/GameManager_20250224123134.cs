...
public class GameManager : MonoBehaviour
{
    ...
    {
        // Find the word in the current era's word list
        if (wordSentencesPerLanguage.ContainsKey(currentLanguage) && 
            wordSentencesPerLanguage[currentLanguage].ContainsKey(CurrentEra))
        {
            var eraWords = wordSentencesPerLanguage[currentLanguage][CurrentEra];
            
            // Find the word entry in the era's word list
            if (eraWords.ContainsKey(word))
            {
                // Get the corresponding WordEntry from the WordSet
                var wordSet = GetWordSetForEra(CurrentEra);
                if (wordSet != null)
                {
                    var wordEntry = wordSet.words.FirstOrDefault(w => w.word == word);
                    if (wordEntry != null)
                    {
                        return new WordData
                        {
                            word = wordEntry.word,
                            sentence = wordEntry.sentences[0], // or choose a random sentence
        ...
        // Return default data if word not found
        return new WordData
        {
            word = word,
            sentence = "_____",
    ...
    private WordSet GetWordSetForEra(string era)
    {
        // Find the WordSet for the given era
        if (wordSentencesPerLanguage.ContainsKey(currentLanguage))
        {
            var wordSets = wordSentencesPerLanguage[currentLanguage];
            if (wordSets.ContainsKey(era))
            {
                // Assuming the WordSet is stored in the dictionary
                // You may need to adjust this based on your actual data structure
                return wordSets[era] as WordSet;
            }
        }
        return null;
    }
