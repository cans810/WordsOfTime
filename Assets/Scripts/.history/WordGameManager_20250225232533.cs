...
public class WordGameManager : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            Debug.Log($"[Android Debug] GameScene loaded, initializing for era: {GameManager.Instance.CurrentEra}");
            
            // Initialize UI elements
            InitializeUI();
            
            // Get solved words for the current era
            solvedWordsInCurrentEra = GameManager.Instance.GetSolvedWordsForEra(GameManager.Instance.CurrentEra);
            Debug.Log($"[Android Debug] Loaded {solvedWordsInCurrentEra.Count} solved words for era: {GameManager.Instance.CurrentEra}");
            
            // Create progress bar for the current era
            CreateProgressBar();
            
            if (!gameInitialized)
            {
                Debug.Log("[Android Debug] First time initialization, starting new game in era");
                StartNewGameInEra();
                gameInitialized = true;
            }
            else
            {
                Debug.Log("[Android Debug] Game already initialized, loading current word");
                // Clear the grid before loading a new word
                ClearGrid();
                
                // Make sure currentWordIndex is valid for the current era
                if (currentWordIndex >= currentEraWords?.Count || currentWordIndex < 0)
                {
                    currentWordIndex = 0;
                    Debug.Log("[Android Debug] Reset currentWordIndex to 0 as it was out of bounds");
                }
                
                // Load the current word and update UI
            ...
            Debug.Log($"[Android Debug] Scene loading complete, current word: {targetWord}, index: {currentWordIndex}");
        }
    ...
    public void StartNewGameInEra()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            // Get the words for the current era from GameManager's shuffled list
            ...
            // Load first unsolved word or first word if all are solved
            currentWordIndex = 0;
            bool foundUnsolved = false;
            
            for (int i = 0; i < currentEraWords.Count; i++)
            {
                string baseWord = GameManager.Instance.GetBaseWord(currentEraWords[i]);
                if (!GameManager.Instance.IsWordSolved(baseWord))
                {
                    currentWordIndex = i;
                    foundUnsolved = true;
                    Debug.Log($"Found unsolved word at index {i}: {currentEraWords[i]}");
                    break;
                }
            }
            ...
            if (!foundUnsolved)
            {
                Debug.Log("All words in this era are solved, starting from the first word");
            }
            ...
            // Make sure to update the display of solved words
    ...
    public void LoadWord(int index)
    {
        ...
        if (isSolved)
        {
            Debug.Log($"Word {currentBaseWord} is already solved, highlighting {targetWord} in grid");
    ...
    private void HighlightWordInGrid(string word)
    {
        ...
        {
            Debug.LogError("Cannot highlight word in grid: GridManager or grid is null");
            return;
        }
        
        // Use the stored currentBaseWord instead of calling GetBaseWord again
        // Get the translated word for the current language
        string translatedWord = GameManager.Instance.GetTranslation(currentBaseWord, GameManager.Instance.CurrentLanguage);
        
        Debug.Log($"Highlighting word in grid: Original={word}, Base={currentBaseWord}, Translated={translatedWord}, Era={GameManager.Instance.CurrentEra}");
        ...
        // Then search for the translated word
        for (int row = 0; row < gridSize; row++)
        {
            for (int col = 0; col < gridSize; col++)
            {
                if (DFS(row, col, translatedWord, 0, visited))
                {
                    Debug.Log($"Found word {translatedWord} starting at ({row},{col})");
                    wordFound = true;
                    break;
                }
            }
        ...
        if (!wordFound)
        {
            Debug.LogError($"Failed to find word: {translatedWord} in grid for era {GameManager.Instance.CurrentEra}!");
            
            // Try with the original word as a fallback
            Debug.Log($"Trying to find original word {word} as fallback");
            ...
            for (int row = 0; row < gridSize; row++)
            {
                for (int col = 0; col < gridSize; col++)
                {
                    ...
                    {
                        Debug.Log($"Found original word {word} starting at ({row},{col})");
                        wordFound = true;
                        break;
                    }
                }
            ...
            if (!wordFound)
            {
                Debug.LogError($"Failed to find both translated word {translatedWord} and original word {word} in grid!");
            }
        }
    }
    ...
    private bool DFS(int row, int col, string word, int index, bool[,] visited)
    {
        ...
        char gridLetter = char.ToUpperInvariant(GridManager.Instance.grid[row, col].GetLetter());
        char wordLetter = char.ToUpperInvariant(word[index]);
        
        if (gridLetter != wordLetter)
        {
            // Debug.Log($"Mismatch at ({row},{col}): Grid has '{gridLetter}', looking for '{wordLetter}'");
            return false;
        }
        ...
        for (int dir = 0; dir < 8; dir++)
        {
            ...
            {
                // Mark this tile as part of the solution
                ...
                // Debug.Log($"Found letter {wordLetter} at ({row},{col}) for word {word}");
                return true;
            }
        }
    ...
    {
        Debug.Log($"Era changed to {GameManager.Instance.CurrentEra}, updating game state");
        
        // Clear the grid
        ClearGrid();
        
        // Get the words for the new era
        currentEraWords = new List<string>(GameManager.Instance.eraWordsPerLanguage[GameManager.Instance.CurrentLanguage][GameManager.Instance.CurrentEra]);
        Debug.Log($"Words for {GameManager.Instance.CurrentEra}: {string.Join(", ", currentEraWords)}");
        
        // Create progress indicators for the new era
        CreateProgressBar();
        
        // Find the first unsolved word or stay at the current index if within bounds
        currentWordIndex = 0;
        ...
        Debug.Log($"Era change complete, current word index: {currentWordIndex}, word: {targetWord}");
    }
}