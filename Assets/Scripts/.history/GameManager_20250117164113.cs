...
public class GameManager : MonoBehaviour
{
    ...
    private void LoadWordSets()
    {
        ...
        {
            Debug.LogError($"Failed to load words.json from path: {filePath}");
            return;
        }
        ...
        try
        {
            ...
            WordSetList wordSetList = JsonUtility.FromJson<WordSetList>(jsonText);
            ...
            foreach (var wordSet in wordSetList.sets)
            {
                if (!string.IsNullOrEmpty(wordSet.era))
                {
                    ...
                    Debug.Log($"Loaded {wordDict.Count} words for era: {wordSet.era}");
                }
            }
        }
