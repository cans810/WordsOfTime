...
public class WordGameManager : MonoBehaviour
{
    ...
    public void StartNewGameInEra()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            // Get the current era and ensure it's uppercase
            string currentEra = GameManager.Instance.CurrentEra.ToUpper();
            string currentLanguage = GameManager.Instance.CurrentLanguage;
            
            // Get words for the current era
            ...
            {
                currentEraWords[i] = currentEraWords[i].ToUpper();
            }
            
            Debug.Log($"Words order for {currentEra}: {string.Join(", ", currentEraWords)}");
            ...
            if (solvedWordsInCurrentEra.Count == 0)
            {
                solvedWordsInCurrentEra = GameManager.Instance.GetSolvedWordsForEra(currentEra);
            }
    ...
    public void UpdateCurrentWord(string word)
    {
        ...
        // Check if second hint is active by checking PlayerPrefs
        string hintKey = $"hint2_{targetWord}";
        bool isSecondHintActive = PlayerPrefs.GetInt(hintKey, 0) > 0;
    ...
    public void LoadWord(string word)
    {
        ...
        // Set the target word - CONVERT TO UPPER instead of lower
    ...
    private void PreloadFactForWord(string word, string era, string language)
    {
        ...
        // Ensure word and era are in uppercase
        word = word.ToUpper();
        era = era.ToUpper();
        ...
        if (string.IsNullOrEmpty(fact) || fact == "LOADING")
        {
            ...
            {
                string baseWord = GameManager.Instance.GetBaseWord(word);
                baseWord = baseWord.ToUpper(); // Ensure base word is also uppercase
    ...
    public void ShowDidYouKnowInstantly()
    {
        ...
        try
        {
            ...
            // Get the current language and era - ensure era is uppercase
            string language = GameManager.Instance.CurrentLanguage;
            string era = GameManager.Instance.CurrentEra.ToUpper();
            ...
            else if (string.IsNullOrEmpty(fact))
            {
                ...
                baseWord = baseWord.ToUpper(); // Ensure base word is uppercase
    ...
    public void NextWord()
    {
        ...
        // Find the index of the current word - targetWord should already be uppercase from LoadWord
        int currentIndex = wordsForEra.IndexOf(targetWord);
    ...
    private void UpdateHintButton()
    {
        ...
        {
            // Get the hint cost (default to 10 if we can't get it from GameManager)
            int hintCost = 10;
            try
            {
            ...
            // Check if the player has already used this hint
            string hintKey = $"hint1_{targetWord}";
            bool hintUsed = PlayerPrefs.GetInt(hintKey, 0) > 0;
            
            // If the word is not solved, enable the hint button if the player has enough points and hasn't used this hint
            hintButton.interactable = !hintUsed && GameManager.Instance.CurrentPoints >= hintCost;
    ...
    public bool CheckWord(string word)
    {
        ...
        // Convert the input word to uppercase to match the target word
        word = word.ToUpper();
        ...
        if (isCorrect)
        {
            ...
            if (GameManager.Instance != null)
            {
                ...
                {
                    // Try to invoke a method using reflection
                    var method = GameManager.Instance.GetType().GetMethod("MarkWordAsSolved", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
                    if (method != null)
                    {
                        method.Invoke(GameManager.Instance, new object[] { baseWord });
                        Debug.Log($"[WordGameManager] Successfully marked word '{baseWord}' as solved via reflection");
                    }
                    else
                    {
                        // Try another method name
                        method = GameManager.Instance.GetType().GetMethod("AddSolvedWord", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
                        if (method != null)
                        {
                            method.Invoke(GameManager.Instance, new object[] { baseWord });
                            Debug.Log($"[WordGameManager] Successfully added word '{baseWord}' to solved words via reflection");
                        }
                        else
                        {
                            Debug.LogWarning($"[WordGameManager] Could not find method to mark word '{baseWord}' as solved");
                            
                            // As a last resort, try to access the solved words collection directly
                            var solvedWordsField = GameManager.Instance.GetType().GetField("solvedWords", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                            if (solvedWordsField != null)
                            {
                                var solvedWordsCollection = solvedWordsField.GetValue(GameManager.Instance) as HashSet<string>;
                                if (solvedWordsCollection != null)
                                {
                                    solvedWordsCollection.Add(baseWord);
                                    Debug.Log($"[WordGameManager] Added word '{baseWord}' to solved words collection directly");
                                }
                            }
                        }
                    }
                ...
                {
                    Debug.LogError($"[WordGameManager] Error marking word as solved: {e.Message}");
                }
    ...
    // Overload for CheckWord that takes a List<LetterTile>
    public bool CheckWord(string word, List<LetterTile> selectedTiles)
    {
        Debug.Log($"[WordGameManager] CheckWord with selectedTiles called for word: {word}");
        
        // Store the selected tiles for reference
    ...
    public void GiveHint()
    {
        ...
        {
            // Deduct points
            GameManager.Instance.AddPoints(-hintCost);
            
            // Mark that we've used a hint for this word
            // We need to store this information somewhere
            string hintKey = $"hint1_{targetWord}";
            PlayerPrefs.SetInt(hintKey, 1);
            PlayerPrefs.Save();
            
            Debug.Log($"[WordGameManager] Used hint 1 for word '{targetWord}', deducted {hintCost} points");
