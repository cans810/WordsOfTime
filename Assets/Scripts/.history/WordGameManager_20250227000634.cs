...
public class WordGameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Initialize didYouKnowPanel properly
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        try
        {
            ...
            if (didYouKnowPanel == null || didYouKnowText == null)
            {
                Debug.LogError("[DidYouKnow Debug] Panel or text component is null in ShowDidYouKnowCoroutine");
                yield break;
            }
            
            // Get the language-specific title
            string titleText = GameManager.Instance.CurrentLanguage == "tr" ? "BİLİYOR MUYDUNUZ?" : "DID YOU KNOW?";
            ...
            {
                // For PC, ensure the panel is centered
            ...
            // Make sure the panel is in front of other UI elements
            Canvas panelCanvas = didYouKnowPanel.GetComponentInParent<Canvas>();
            if (panelCanvas != null)
            {
            ...
            // Apply ScreenFitter to ensure proper sizing
            ScreenFitter fitter = didYouKnowPanel.GetComponent<ScreenFitter>();
            if (fitter != null)
            {
                fitter.FitToScreen();
            }
        ...
        {
            Debug.LogError($"[DidYouKnow Debug] Error showing fact panel: {e.Message}\n{e.StackTrace}");
        }
    }
    ...
    private void PositionPanelInSafeArea()
    {
        ...
        try
        {
            ...
            // Set proper anchors for centering
            ...
            // Ensure the panel is visible by setting proper Canvas sorting order
    ...
    public void ShowDidYouKnowInstantly()
    {
        ...
        // Check for panel references and initialize if needed
        if (didYouKnowPanel == null || didYouKnowText == null)
        {
            InitializeDidYouKnowPanel();
            
            // If still null after initialization, log error and return
            if (didYouKnowPanel == null)
            {
                Debug.LogError("[DidYouKnow Debug] CRITICAL: DidYouKnowPanel not found in scene! Check scene hierarchy.");
                return;
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(fact) && fact != "LOADING")
            {
                ...
                // Position the panel correctly based on platform
                if (Application.platform == RuntimePlatform.Android)
                {
                    PositionPanelInSafeArea();
                }
    ...
    // New method to properly initialize the didYouKnowPanel
    private void InitializeDidYouKnowPanel()
    {
        // First, find the panel if it's not assigned
        if (didYouKnowPanel == null)
        {
            Debug.Log("[DidYouKnow Debug] Looking for didYouKnowPanel in scene");
            didYouKnowPanel = GameObject.Find("DidYouKnowPanel");
        ...
        if (didYouKnowText == null && didYouKnowPanel != null)
        {
            didYouKnowText = didYouKnowPanel.GetComponentInChildren<TextMeshProUGUI>();
            if (didYouKnowText == null)
            {
        ...
        // Make sure the panel has a RectTransform
        RectTransform panelRect = didYouKnowPanel.GetComponent<RectTransform>();
        if (panelRect == null)
        {
            Debug.LogError("[DidYouKnow Debug] DidYouKnowPanel doesn't have a RectTransform component!");
            return;
        }
        
        // Make sure the panel is in the correct Canvas
        Canvas parentCanvas = didYouKnowPanel.GetComponentInParent<Canvas>();
        if (parentCanvas == null)
        {
            Debug.LogError("[DidYouKnow Debug] DidYouKnowPanel is not in a Canvas hierarchy!");
            return;
        }
        
        // Ensure the panel has a ScreenFitter component
        ScreenFitter fitter = didYouKnowPanel.GetComponent<ScreenFitter>();
        if (fitter == null)
        {
            Debug.Log("[DidYouKnow Debug] Adding ScreenFitter to didYouKnowPanel");
            fitter = didYouKnowPanel.AddComponent<ScreenFitter>();
        }
        
        // Make sure the panel has a CanvasGroup for proper visibility control
        CanvasGroup canvasGroup = didYouKnowPanel.GetComponent<CanvasGroup>();
        if (canvasGroup == null)
        {
            Debug.Log("[DidYouKnow Debug] Adding CanvasGroup to didYouKnowPanel");
            canvasGroup = didYouKnowPanel.AddComponent<CanvasGroup>();
        }
        
        // Ensure the panel is initially hidden
        didYouKnowPanel.SetActive(false);
        
        Debug.Log("[DidYouKnow Debug] DidYouKnowPanel initialized successfully");
    }
}