...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    [SerializeField] private float coinSpeedMin = 0.3f; // Time in seconds to reach the target (faster)
    [SerializeField] private float coinSpeedMax = 0.8f; // Time in seconds to reach the target (faster)
    ...
    private IEnumerator SpawnCoins(int coinCount, System.Action onComplete)
    {
        ...
        for (int i = 0; i < coinCount; i++)
        {
            ...
            coin.tag = "Coin"; // Tag the coin for tracking
            coins.Add(coin);
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin)
    {
        ...
        Vector2 startPosition = coin.transform.position;
        Vector2 targetPosition = pointPanel.transform.position;

        // Randomize the duration of the movement
        float duration = UnityEngine.Random.Range(coinSpeedMin, coinSpeedMax);
        float elapsedTime = 0f;

        // Move the coin to the target position using lerp
        while (elapsedTime < duration)
        {
            // Calculate the interpolation factor
            float t = elapsedTime / duration;

            // Move the coin towards the target position using lerp
            coin.transform.position = Vector2.Lerp(startPosition, targetPosition, t);
