...
public class WordGameManager : MonoBehaviour
{
    ...
    public void LoadWord(int index)
    {
        ...
        if (difficultyText != null)
        {
            ...
            // Option 2: Color-coded stars (requires TMP)
            string colorHex = difficulty switch
            {
                "easy" => "#4CAF50",    // Green
                "normal" => "#FFC107",   // Yellow/Gold
                "hard" => "#F44336",     // Red
                _ => "#FFFFFF"          // White
            };
            difficultyText.text = $"<color={colorHex}>{difficultyText.text}</color>";

            // Option 3: Difficulty Icons (using Unicode)
            difficultyText.text = difficulty switch
            {
                "easy" => "⚪",      // Circle
                "normal" => "⚡",    // Lightning
