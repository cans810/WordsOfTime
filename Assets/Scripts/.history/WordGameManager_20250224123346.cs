...
public class WordGameManager : MonoBehaviour
{
    ...
    {
        // Assuming currentEraWords is a list of words for the current era
        if (currentWordIndex < currentEraWords.Count)
        {
            string currentWord = currentEraWords[currentWordIndex];
            
            // Fetch the WordSet for the current era directly from GameManager
            GameManager gameManager = GameManager.Instance;

            // Find the WordSet that corresponds to the current era
            WordSet currentWordSet = gameManager.wordSentencesPerLanguage[gameManager.CurrentLanguage]
                .FirstOrDefault(set => set.Key == gameManager.CurrentEra).Value;

            // Find the WordEntry for the current word
            WordEntry wordEntry = currentWordSet.words.FirstOrDefault(w => w.word.Equals(currentWord, StringComparison.OrdinalIgnoreCase));
            
            if (wordEntry != null)
            {
                // Update the difficultyText field
                difficultyText.text = wordEntry.difficulty;
            }
            else
            {
                Debug.LogWarning($"WordEntry for '{currentWord}' not found.");
            }
        }
    }
}