...
public class WordGameManager : MonoBehaviour
{
    ...
    private void SynchronizeSolvedWords()
    {
        if (GameManager.Instance == null)
        {
            Debug.LogError("[Android Debug] GameManager.Instance is null, cannot synchronize solved words");
            return;
        }
        
        // Get all guessed words from GameManager
        List<string> guessedWords = GameManager.Instance.GetGuessedWords();
        
        // Add any guessed words that aren't in solvedWords
        foreach (string word in guessedWords)
        {
            if (!solvedWords.Contains(word))
            {
                solvedWords.Add(word);
                Debug.Log($"[Android Debug] Added missing word to solvedWords: {word}");
            }
        }
        ...
        foreach (string word in solvedWords)
        {
            ...
            {
                GameManager.Instance.AddGuessedWord(word);
                Debug.Log($"[Android Debug] Added missing word to GameManager's guessed words: {word}");
            }
        }
        
        Debug.Log($"[Android Debug] Synchronized solvedWords ({solvedWords.Count}) with GameManager's guessed words ({guessedWords.Count})");
