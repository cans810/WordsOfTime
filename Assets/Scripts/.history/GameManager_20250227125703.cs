...
public class GameManager : MonoBehaviour
{
    ...
    public void SetLanguage(string language)
    {
        if (language != CurrentLanguage)
        {
            ...
            UpdateSolvedWordsForNewLanguage();
    ...
    private void UpdateSolvedWordsForNewLanguage()
    {
        Debug.Log($"[GameManager] Updating solved words for new language: {CurrentLanguage}");
        
        // Make sure solvedWordsPerLanguage has an entry for the current language
        if (!solvedWordsPerLanguage.ContainsKey(CurrentLanguage))
        {
            solvedWordsPerLanguage[CurrentLanguage] = new HashSet<string>();
        }
        
        // For each solved base word, add its translation in the current language
        foreach (var era in solvedBaseWordsPerEra.Keys)
        {
            foreach (var baseWord in solvedBaseWordsPerEra[era])
            {
                string translatedWord = GetTranslation(baseWord, CurrentLanguage);
                Debug.Log($"[GameManager] Adding translated word '{translatedWord}' for base word '{baseWord}' to solved words");
                solvedWordsPerLanguage[CurrentLanguage].Add(translatedWord);
            }
        }
        
        // Log the updated solved words for the current language
        Debug.Log($"[GameManager] Updated solved words for {CurrentLanguage}: {string.Join(", ", solvedWordsPerLanguage[CurrentLanguage])}");
    }
