...
private void ClearSolvedWordsDisplay()
{
    ...
    {
        Debug.Log($"Storing solved word positions for: {word}, positions count: {positions.Count}");
    ...
    {
        if (string.IsNullOrEmpty(newEra) || !eraList.Contains(newEra)) 
        {
            Debug.LogError($"Invalid era: {newEra}");
            return;
        }
        
        // Clear previous era state
        ClearSolvedWordsDisplay();
        
        currentEra = newEra;
        OnEraChanged?.Invoke();
        
        // Update solved words for new era
    ...
    {
        if (string.IsNullOrEmpty(baseWord)) return baseWord;
        
        // Check if word exists in translations
        if (wordTranslations.ContainsKey(baseWord) && 
            wordTranslations[baseWord].ContainsKey(CurrentLanguage))
        {
            return wordTranslations[baseWord][CurrentLanguage];
        }
        
        // Fallback to base word if translation not found
        return baseWord;
    }
