...
public class WordGameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        Debug.Log("WordGameManager Start");
        ...
        {
            // Set the background image based on the current era
            if (BackgroundImage != null)
            {
                BackgroundImage.sprite = GameManager.Instance.getEraImage(GameManager.Instance.CurrentEra);
            }
            
            // Load pre-generated grids from GameManager if available
            if (GameManager.Instance.GetPreGeneratedGrids().Count > 0)
            {
                preGeneratedGrids = GameManager.Instance.GetPreGeneratedGrids();
                Debug.Log($"Loaded {preGeneratedGrids.Count} pre-generated grids from GameManager");
            }
            else
            {
                Debug.Log("No pre-generated grids found in GameManager");
            }
        }
        else
        {
        ...
        wordsGuessedCount = 0;
        isAdShowing = false;
        Debug.Log($"Ad counter initialized to 0, isAdShowing set to {isAdShowing}");
    ...
    {
        Debug.Log("Initializing ads in WordGameManager");
        
        // Check if ads are already initialized
        if (!Advertisement.isInitialized && AdManager.Instance != null)
        {
            Debug.Log("Unity Ads not initialized, requesting initialization");
            if (AdManager.Instance.GetComponent<InterstitialAdExample>() != null)
            {
        ...
        {
            Debug.Log("Unity Ads already initialized or AdManager not available");
        }
        
        // Preload an ad for later use
        if (AdManager.Instance != null)
        {
            StartCoroutine(PreloadAdAfterDelay(2.0f));
        }
    }
    
    private IEnumerator PreloadAdAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        
        if (AdManager.Instance != null && Advertisement.isInitialized)
        {
            Debug.Log("Preloading interstitial ad");
            AdManager.Instance.LoadInterstitialAd();
        }
        else
        {
            Debug.LogWarning("Cannot preload ad - Unity Ads not initialized or AdManager not available");
            // Try again after a longer delay
            yield return new WaitForSeconds(3.0f);
            
            if (AdManager.Instance != null)
            {
                Debug.Log("Retrying ad preload after delay");
                AdManager.Instance.LoadInterstitialAd();
            }
        }
    }
    ...
    // Add a method to retry showing ads after initialization
    private IEnumerator RetryShowAdAfterDelay(float delay)
    {
        Debug.Log($"Will retry showing ad after {delay} seconds to allow initialization to complete");
        yield return new WaitForSeconds(delay);
        
        // Check if initialization is complete
        if (Advertisement.isInitialized)
        {
            Debug.Log("Unity Ads is now initialized, retrying ad display");
            ShowAd();
        }
        else
        {
            Debug.LogWarning("Unity Ads still not initialized after delay");
            // Try one more time with a longer delay
            if (delay < 5.0f)
            {
                StartCoroutine(RetryShowAdAfterDelay(5.0f));
            }
            else
            {
                Debug.LogError("Failed to initialize Unity Ads after multiple attempts");
            }
        }
    }
    ...
    private void ShowAd()
    {
        ...
        Debug.Log($"Words guessed count: {wordsGuessedCount}, WORDS_BEFORE_AD: {WORDS_BEFORE_AD}, isAdShowing: {isAdShowing}");
        ...
        Debug.Log($"ShowAd called with wordsGuessedCount: {wordsGuessedCount}");

        if (GameManager.Instance != null && GameManager.Instance.NoAdsBought)
        {
            Debug.Log("No Ads purchased - skipping ad");
            // Reset counter even if ads are disabled to maintain the rhythm
            wordsGuessedCount = 0;
            Debug.Log("Counter reset to 0 due to No Ads purchased");
            return;
        }

        // Ensure ads are initialized before showing
        if (!EnsureAdsInitialized())
        {
            Debug.LogWarning("Ads not initialized, initialization requested. Will try to show ad later.");
            // Start a coroutine to retry after a delay
            StartCoroutine(RetryShowAdAfterDelay(2.0f));
            return;
        }
        ...
        if (AdManager.Instance != null)
        {
            try
            {
                ...
                Debug.Log($"Set isAdShowing to {isAdShowing}");
                
                // Force load the ad before showing to ensure it's ready
                if (!Advertisement.isInitialized)
                {
                    Debug.LogWarning("Unity Ads not initialized when trying to show ad. Initializing...");
                    if (AdManager.Instance.GetComponent<InterstitialAdExample>() != null)
                    {
                        AdManager.Instance.GetComponent<InterstitialAdExample>().Initialize();
            ...
            catch (System.Exception e)
            {
                ...
                Debug.Log($"Reset isAdShowing to {isAdShowing} due to error");
                
                // Try again after a delay if there was an error
                StartCoroutine(RetryShowAdAfterDelay(3.0f));
            }
        }
        else
        {
            Debug.LogWarning("AdManager instance not found!");
            isAdShowing = false; // Reset flag if AdManager is not found
            Debug.Log($"Reset isAdShowing to {isAdShowing} due to missing AdManager");
        }
    }
    ...
    {
        Debug.Log($"OnAdCompleted called, current wordsGuessedCount: {wordsGuessedCount}");
        isAdShowing = false;
        // Reset the counter only after the ad has successfully completed
        wordsGuessedCount = 0;
        Debug.Log($"Ad completed, reset wordsGuessedCount to {wordsGuessedCount}");
        
        // Save the counter state
        PlayerPrefs.SetInt("WordsGuessedCount", wordsGuessedCount);
        PlayerPrefs.Save();
        
        // Preload the next ad
        StartCoroutine(PreloadAdAfterDelay(1.0f));
    }

    // Method to reset just the isAdShowing flag without resetting the counter
    // Used when an ad is skipped or fails to complete
    public void ResetAdShowingFlag()
    {
        Debug.Log("Resetting isAdShowing flag without resetting counter");
        isAdShowing = false;
    }
    ...
    public void CheckWord(string word, List<LetterTile> selectedTiles)
    {
        ...
        if (word == targetWord)
        {
            ...
            // Only increment counter if this is a new word (not already solved)
            if (!solvedWords.Contains(word) && !GameManager.Instance.IsWordSolved(baseWord))
            {
            ...
            {
                Debug.Log($"Word already solved, not incrementing counter. Current count: {wordsGuessedCount}/{WORDS_BEFORE_AD}");
            }
            ...
            if (wordsGuessedCount >= WORDS_BEFORE_AD && !isAdShowing)
            {
                ...
                Debug.Log($"wordsGuessedCount: {wordsGuessedCount}, WORDS_BEFORE_AD: {WORDS_BEFORE_AD}, isAdShowing: {isAdShowing}");
                
                // Use Task.Delay to ensure the ad shows after a short delay
                // This gives time for animations to complete and prevents UI conflicts
                StartCoroutine(ShowAdAfterDelay(0.5f));
            }
            else
            {
                Debug.Log($"Not showing ad yet. wordsGuessedCount: {wordsGuessedCount}, WORDS_BEFORE_AD: {WORDS_BEFORE_AD}, isAdShowing: {isAdShowing}");
            }
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        if (pauseStatus)
        {
            // Game is paused (going to background)
            Debug.Log("Game paused, saving state");
            SaveSolvedWords();
            
            // Save ad counter state
            PlayerPrefs.SetInt("WordsGuessedCount", wordsGuessedCount);
            PlayerPrefs.Save();
        }
        else
        {
            // Game is resumed (coming from background)
            Debug.Log("Game resumed, loading state");
            LoadSolvedWords();
            
            // Load ad counter state
            if (PlayerPrefs.HasKey("WordsGuessedCount"))
            {
                wordsGuessedCount = PlayerPrefs.GetInt("WordsGuessedCount");
                Debug.Log($"Loaded wordsGuessedCount: {wordsGuessedCount}");
            }
        }
    }
    ...
    {
        Debug.Log("Application quitting, saving state");
        SaveSolvedWords();
    ...
    private bool EnsureAdsInitialized()
    {
        if (!Advertisement.isInitialized)
        {
            ...
            if (AdManager.Instance != null && AdManager.Instance.GetComponent<InterstitialAdExample>() != null)
            {
                ...
                // Also try to load an ad after initialization
                StartCoroutine(LoadAdAfterInitialization(3.0f));
    ...
    private IEnumerator LoadAdAfterInitialization(float maxWaitTime)
    {
        float elapsedTime = 0f;
        
        // Wait for initialization to complete or timeout
        while (!Advertisement.isInitialized && elapsedTime < maxWaitTime)
        {
            yield return new WaitForSeconds(0.5f);
            elapsedTime += 0.5f;
        }
        ...
        if (Advertisement.isInitialized)
        {
            Debug.Log("Unity Ads initialized, loading ad");
            if (AdManager.Instance != null)
            {
                AdManager.Instance.LoadInterstitialAd();
            }
        ...
        {
            Debug.LogWarning($"Unity Ads failed to initialize after {maxWaitTime} seconds");
        }
    }

    // Method to force show an ad for testing
    public void ForceShowAd()
    {
        Debug.Log("=== FORCE SHOWING AD FOR TESTING ===");
        
        // First ensure ads are initialized
        if (!EnsureAdsInitialized())
        {
            Debug.LogError("Cannot force show ad - Unity Ads not initialized");
            return;
        }
        
        wordsGuessedCount = WORDS_BEFORE_AD; // Set the counter to trigger an ad
        Debug.Log($"Set wordsGuessedCount to {wordsGuessedCount} to force ad");
        ShowAd();
    }
