...
public class GridManager : MonoBehaviour
{
    ...
    private Dictionary<Vector2Int, LetterTile> letterTiles = new Dictionary<Vector2Int, LetterTile>();
    ...
    {
        ClearGrid(); // Clear existing grid first
        ...
        for (int i = 0; i < gridData.Count; i++)
        {
            ...
            if (tile != null)
            {
                ...
                letterTiles[position] = tile;
            }
        }
    }

    private LetterTile CreateTile(Vector2Int gridPos)
    {
    ...
    private void ClearGrid()
    {
        ...
        foreach (var tile in letterTiles.Values)
        {
            if (tile != null)
            {
    ...
    private IEnumerator HighlightTilesCoroutine()
    {
        ...
        Color highlightColor = new Color(0f, 0.5f, 1f, 0.5f); // Semi-transparent blue
        
        // Store original colors and ensure we're not affecting solved tiles
        ...
        while (elapsedTime < WordGameManager.HINT_HIGHLIGHT_DURATION)
        {
            ...
            foreach (var tile in highlightedTiles)
            {
                if (!tile.isSolved)
                {
                    // Only modify the background color, not the text
                    Color flashColor = Color.Lerp(originalColors[tile], highlightColor, alpha);
                    tile.SetHighlightColor(flashColor);
                    tile.PreserveLetterDisplay(); // Add this method to LetterTile
                }
            }
