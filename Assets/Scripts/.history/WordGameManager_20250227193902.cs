...
public class WordGameManager : MonoBehaviour
{
    ...
    {
        Debug.Log("[DidYouKnow Debug] Language changed to: " + GameManager.Instance.CurrentLanguage);
        
        // If switching to Turkish, verify facts are loaded properly
        if (GameManager.Instance.CurrentLanguage == "tr" && Application.platform == RuntimePlatform.Android)
        {
            Debug.Log("[DidYouKnow Debug] Switching to Turkish language, verifying facts");
            WordValidator.VerifyTurkishFacts();
            
            // Force refresh the current word's fact if it's already solved
            if (IsWordSolved(targetWord))
            {
                Debug.Log("[DidYouKnow Debug] Current word is solved, refreshing fact");
                ShowDidYouKnow();
            }
        }
        
        // Get the current era with language-specific key
        string eraKey = GameManager.Instance.CurrentEra;
        if (GameManager.Instance.CurrentLanguage == "tr")
        {
            eraKey += "_tr";
        ...
        // Update the solved words collection for the current era
        solvedWordsInCurrentEra = GameManager.Instance.GetSolvedWordsForEra(eraKey);
        
        // If no language-specific solved words, try with the base era key
        if (solvedWordsInCurrentEra.Count == 0)
        {
            solvedWordsInCurrentEra = GameManager.Instance.GetSolvedWordsForEra(GameManager.Instance.CurrentEra);
            Debug.Log($"[Android Debug] No language-specific solved words found, using base era: {solvedWordsInCurrentEra.Count} words");
        }
        else
        {
            Debug.Log($"[Android Debug] Found {solvedWordsInCurrentEra.Count} language-specific solved words for era {eraKey}");
        }
        
        // Reload the words for the current language
        if (GameManager.Instance.eraWordsPerLanguage.ContainsKey(GameManager.Instance.CurrentLanguage))
        {
            if (GameManager.Instance.eraWordsPerLanguage[GameManager.Instance.CurrentLanguage].ContainsKey(GameManager.Instance.CurrentEra))
            {
                currentEraWords = new List<string>(GameManager.Instance.eraWordsPerLanguage[GameManager.Instance.CurrentLanguage][GameManager.Instance.CurrentEra]);
                Debug.Log($"[Android Debug] Reloaded {currentEraWords.Count} words for era {GameManager.Instance.CurrentEra} in {GameManager.Instance.CurrentLanguage}");
            }
            else
            {
                Debug.LogWarning($"[Android Debug] Era {GameManager.Instance.CurrentEra} not found in language {GameManager.Instance.CurrentLanguage}");
            }
        }
        else
        {
            Debug.LogWarning($"[Android Debug] Language {GameManager.Instance.CurrentLanguage} not found in eraWordsPerLanguage");
        }
        
        // On Android, we need to refresh solved words in the grid
        if (Application.platform == RuntimePlatform.Android)
        {
            Debug.Log("[Android Debug] Refreshing solved words in grid after language change");
            
            // Clear the grid first
            ClearGrid();
            
            // Get all the solved base words for the current era
            HashSet<string> solvedBaseWords = GameManager.Instance.GetSolvedBaseWordsForEra(GameManager.Instance.CurrentEra);
            
            if (solvedBaseWords != null && solvedBaseWords.Count > 0)
            {
                Debug.Log($"[Android Debug] Found {solvedBaseWords.Count} solved base words for era {GameManager.Instance.CurrentEra}");
                
                // For each solved base word, mark its translation in the current language as solved
                foreach (string baseWord in solvedBaseWords)
                {
                    ...
                    Debug.Log($"[Android Debug] Base word: {baseWord}, Translation: {translatedWord}");
                    
                    // Check if we have the positions for this word
                    List<Vector2Int> positions = GameManager.Instance.GetWordPath(translatedWord);
                    if (positions != null && positions.Count > 0)
                    {
                        Debug.Log($"[Android Debug] Found positions for {translatedWord}, marking as solved");
                        
                        // Mark these positions as solved in the grid
                        foreach (Vector2Int pos in positions)
                        {
                            if (pos.x >= 0 && pos.x < GridManager.Instance.grid.GetLength(0) &&
                                pos.y >= 0 && pos.y < GridManager.Instance.grid.GetLength(1))
                            {
                                LetterTile tile = GridManager.Instance.grid[pos.x, pos.y];
                                tile.SetSolvedColor();
                                tile.isSolved = true;
                                tile.GetComponent<Image>().raycastTarget = false;
                            }
        ...
        // Reload the current word - LoadWord will handle the translation
        LoadWord(currentWordIndex);
        ...
        if (GameObject.Find("BackgroundImage") != null)
        {
            ...
            {
                backgroundImage.sprite = GameManager.Instance.getEraImage(GameManager.Instance.CurrentEra);
                Debug.Log("[DidYouKnow Debug] Updated background image for era: " + GameManager.Instance.CurrentEra);
            }
        }
    }
