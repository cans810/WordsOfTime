...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    [SerializeField] private float coinSmoothTime = 0.6f;
    [SerializeField] private float coinMaxSpeed = 1000f;
    [SerializeField] private float curveOffset = 400f;    // Horizontal offset for the curve
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin, System.Action onDestroy)
    {
        ...
        // Calculate the midpoint and offset it to the right
        Vector3 midPoint = (startPosition + targetPosition) / 2f;
        Vector3 controlPoint = midPoint + Vector3.right * curveOffset;
        
        float elapsedTime = 0f;
        float duration = coinSmoothTime + UnityEngine.Random.Range(-0.1f, 0.1f);
