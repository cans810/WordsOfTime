...
public class WordGameManager : MonoBehaviour
{
    ...
    public void LoadWord(int index)
    {
        ...
        {
            Debug.Log($"[Android Debug] Word {targetWord} (base: {currentBaseWord}) is already solved, marking it in grid");
            ...
            if (positions == null || positions.Count == 0)
            {
                ...
                // If no positions for current word, try with base word
                positions = GameManager.Instance.GetSolvedWordPositions(currentBaseWord);
                
                // If still no positions, try with translations
                if (positions == null || positions.Count == 0)
                {
                    Debug.Log($"[Android Debug] No positions found for base word either, trying with translations");
            ...
            // Log the positions found
            if (positions != null && positions.Count > 0) 
            {
                Debug.Log($"[Android Debug] Found {positions.Count} positions for solved word: {string.Join(", ", positions.Select(p => $"({p.x},{p.y})"))}");
                
                // Mark tiles as solved using these positions
            ...
            {
                Debug.LogWarning($"[Android Debug] No positions found for solved word {targetWord} or its translations");
            }
        }

        // Refresh the fact for this word
