...
public class SaveManager : MonoBehaviour
{
    ...
    private IEnumerator ApplyLoadedDataCoroutine()
    {
        ...
        {
            Debug.LogError($"Error loading solved words: {e.Message}");
        }
        
        yield return null; // Wait a frame to prevent freezing - moved outside try block
        ...
        try
        {
            ...
            if (Data.preGeneratedGrids != null && Data.preGeneratedGrids.Count > 0)
            {
                ...
                // Set the era for each grid if available
        ...
        {
            Debug.LogError($"Error loading grid data: {e.Message}");
        }
        ...
        {
            Debug.LogError($"Error loading hint usage data: {e.Message}");
        }
        ...
        try
        {
            ...
            {
                Debug.Log($"Loading shuffled words for {Data.shuffledWords.Count} languages");
                foreach (var languageEntry in Data.shuffledWords)
                {
                    string language = languageEntry.Key;
                    if (languageEntry.Value != null && languageEntry.Value.Count > 0)
                    {
                        foreach (var eraEntry in languageEntry.Value)
                        {
                            string era = eraEntry.Key;
                            foreach (var kvp in eraEntry.Value)
                            {
        ...
        {
            Debug.LogError($"Error loading shuffled words: {e.Message}");
        }
        ...
        try
        {
            ...
            {
                Debug.Log($"Loading {Data.unlockedEras.Count} unlocked eras");
        ...
        {
            Debug.LogError($"Error loading unlocked eras: {e.Message}");
        }
