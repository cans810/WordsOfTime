    ...
    private void HandleLanguageChanged()
    {
        ...
        for (int i = 0; i < gridSize; i++)
        {
            for (int j = 0; j < gridSize; j++)
            {
                ...
                GridManager.Instance.grid[i, j].GetComponent<Image>().raycastTarget = true;
            }
        }
        
        // 2. Get the current word's translation for the new language
        string baseWord = GameManager.Instance.GetBaseWord(targetWord);
        string newWord = GameManager.Instance.GetTranslation(baseWord, GameManager.Instance.CurrentLanguage);
        targetWord = newWord;
        
        // 3. Setup a new puzzle with the translated word
        if (GridManager.Instance != null)
        {
            GridManager.Instance.SetupNewPuzzle(targetWord);
        }
        
        // 4. If the word is already solved, find and highlight it in the grid
        if (GameManager.Instance.IsWordSolved(baseWord))
        {
            Debug.Log($"Word {baseWord} is solved, finding {targetWord} in grid");
            HighlightWordInGrid(targetWord);
        }
