...
public class GameManager : MonoBehaviour
{
    ...
    private Dictionary<string, int> usedHints = new Dictionary<string, int>();
    ...
    // Store hint usage for a specific word
    public void StoreHintUsage(string word)
    {
        string key = $"{CurrentEra}_{word}";
        if (!usedHints.ContainsKey(key))
        {
            usedHints[key] = 1; // First hint used
        }
        else if (usedHints[key] == 1)
        {
            usedHints[key] = 2; // Second hint used
        }
        Debug.Log($"Stored hint for {word} in {CurrentEra}. Hint level: {usedHints[key]}");
    }

    // Get current hint level for a word (0 = no hints, 1 = first hint used, 2 = both hints used)
    public int GetHintLevel(string word)
    {
        string key = $"{CurrentEra}_{word}";
        return usedHints.ContainsKey(key) ? usedHints[key] : 0;
    }

    // Check if more hints are available for a word
    public bool CanUseMoreHints(string word)
    {
        int currentLevel = GetHintLevel(word);
        return currentLevel < 2; // Can use hints if we haven't used both yet
    }

    // Check if player can afford the hint
    public bool CanAffordHint(int hintLevel)
    {
        int cost = hintLevel == 1 ? HINT_COST : SECOND_HINT_COST;
        return CurrentPoints >= cost;
    }
