...
public class WordGameManager : MonoBehaviour
{
    ...
    public void LoadWord(int index)
    {
        ...
        // Get the current word in the current language
        string currentLanguageWord = GameManager.Instance.GetCurrentLanguageWord(currentBaseWord);
        
        // Only highlight if the word is solved
        if (GameManager.Instance.IsWordSolved(currentBaseWord))
        {
            // Get positions for ALL language versions of the word
            List<Vector2Int> positions = GameManager.Instance.GetSolvedWordPositions(currentLanguageWord);
            
            if (positions != null && positions.Count > 0)
            {
                // Highlight using stored positions
                GridManager.Instance.ShowSolvedWord(currentLanguageWord, positions);
            }
            else
            {
                // If positions aren't stored, try to find the word in the grid
                HighlightWordInGrid(currentLanguageWord);
            }
        }
        ...
        // Get the word's difficulty from GameManager
    ...
    public void HighlightWordInGrid(string word)
    {
        ...
        List<Vector2Int> positions = gridManager.FindWordInGrid(word, gridManager.grid);
        ...
        if (positions != null && positions.Count > 0)
        {
            ...
            // Highlight the word
            GridManager.Instance.ShowSolvedWord(word, positions);
        }
    }
    ...
    private void HandleLanguageChanged()
    {
        ...
        string currentLanguageWord = GameManager.Instance.GetCurrentLanguageWord(currentBaseWord);
        UpdateSentenceDisplay(currentLanguageWord);
        ...
        if (GameManager.Instance.IsWordSolved(currentBaseWord))
        {
            // Find the word in the current grid dynamically
            List<Vector2Int> positions = GridManager.Instance.FindWordInGrid(currentLanguageWord, GridManager.Instance.grid);
