...
public class MarketManager : MonoBehaviour, IStoreListener
{
    ...
    private void Start()
    {
        ...
        // Always check the no ads state when the market starts
        CheckNoAdsState();
    }

    private void OnEnable()
    {
        // Ensure we check the no ads state every time the market screen is shown
        CheckNoAdsState();
    }
    ...
    private void DestroyNoAdsButton()
    {
        if (noAdsButton != null)
        {
            Debug.Log("Hiding No Ads button");
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (productId == NO_ADS_PRODUCT_ID)
        {
            ...
            GameManager.Instance.EnableNoAds();
            DestroyNoAdsButton();
            // Hide any active ads immediately
    ...
    public bool IsNoAdsPurchased()
    {
        ...
        // Check GameManager state directly for the most up-to-date state
        ...
        if (storeController != null && storeController.products.WithID(NO_ADS_PRODUCT_ID) != null)
        {
            ...
            if (hasReceipt)
            {
                ...
                GameManager.Instance.EnableNoAds(); // This will save the state
                return true;
            }
        }
    ...
    // Method to purchase No Ads from the market
    public void BuyNoAds()
    {
        Debug.Log("Attempting to buy No Ads");
