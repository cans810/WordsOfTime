...
public class WordGameManager : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (scene.name == "GameScene")
        {
            ...
            {
                Debug.LogWarning("GridManager instance is null in OnSceneLoaded, attempting to find it");
    ...
    public void StartNewGameInEra()
    {
        ...
        // Initialize GridManager if needed
        if (GridManager.Instance == null)
        {
            Debug.LogWarning("GridManager instance is null, attempting to find it");
            GridManager.Instance = FindObjectOfType<GridManager>();
        }
    ...
    private void ClearGrid()
    {
        ...
        {
            Debug.LogWarning("Cannot clear grid: GridManager or grid is null");
            return;
        }
        ...
        for (int row = 0; row < gridSize; row++)
        {
            ...
            {
                if (GridManager.Instance.grid[row, col] != null)
                {
                    GridManager.Instance.grid[row, col].ResetTile();
                    GridManager.Instance.grid[row, col].isSolved = false;
                    GridManager.Instance.grid[row, col].GetComponent<Image>().raycastTarget = true;
