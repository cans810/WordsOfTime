...
public class WordGameManager : MonoBehaviour
{
    ...
    private void HandleLanguageChanged()
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            if (GridManager.Instance != null)
            {
                ...
                if (currentBaseWord != null)
                {
                    ...
                    if (GameManager.Instance.IsWordSolved(currentBaseWord) && 
                        GameManager.Instance.GetSolvedWordPositions(currentLangWord) != null)
                    {
                        List<Vector2Int> positions = GameManager.Instance.GetSolvedWordPositions(currentLangWord);
                        foreach (Vector2Int pos in positions)
                        {
                            if (GridManager.Instance.GetTileAtPosition(pos) != null)
                            {
                        ...
                        Debug.Log($"[Android Debug] Marked tiles for solved word: {currentLangWord}");
                    }
                }
            }
            else
            {
                Debug.LogWarning("[Android Debug] GridManager.Instance is null, can't refresh grid");
            }
        }
        
        // Reload the current word and update UI
        LoadWord(currentWordIndex);
        UpdateProgressBar();
        ...
        if (GameObject.Find("BackgroundImage") != null)
        {
            ...
            {
                backgroundImage.sprite = GameManager.Instance.getEraImage(GameManager.Instance.CurrentEra);
                Debug.Log("[Android Debug] Updated background image for era: " + GameManager.Instance.CurrentEra);
            }
        }
    }
    ...
    private void RefreshCurrentWordFact()
    {
        ...
        if (!string.IsNullOrEmpty(fact))
        {
            ...
            {
                if (didYouKnowFactText != null)
                {
                    didYouKnowFactText.text = fact;
