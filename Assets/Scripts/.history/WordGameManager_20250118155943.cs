...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator AnimatePointsChange(int startPoints, int endPoints)
    {
        ...
        while (elapsedTime < animationDuration)
        {
            ...
            {
                // Start a bump animation for this number change
                StartCoroutine(BumpScale(pointText.transform));
    ...
    private IEnumerator BumpScale(Transform target)
    {
        Vector3 originalScale = target.localScale;
        float bumpDuration = 0.05f; // Duration of each bump
        float elapsedTime = 0f;
        ...
        while (elapsedTime < bumpDuration)
        {
            ...
            float scale = 1f + 0.5f * Mathf.Sin(t * Mathf.PI); // Peak scale is 1.5x
