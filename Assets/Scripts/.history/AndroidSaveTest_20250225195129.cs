using UnityEngine;
using UnityEngine.UI;
using TMPro;

/// <summary>
/// Component to add to a UI button for testing the SimpleSaveSystem on Android
/// </summary>
public class AndroidSaveTest : MonoBehaviour
{
    public TextMeshProUGUI statusText;
    
    public void TestSimpleSaveSystem()
    {
        #if UNITY_ANDROID && !UNITY_EDITOR
        try
        {
            if (statusText != null)
            {
                statusText.text = "Testing SimpleSaveSystem...";
            }
            
            Debug.Log("Testing SimpleSaveSystem from UI button...");
            
            // Call the test method in SaveManager
            if (SaveManager.Instance != null)
            {
                SaveManager.Instance.TestSimpleSaveSystem();
                
                if (statusText != null)
                {
                    statusText.text = "Test completed. Check logs for results.";
                }
            }
            else
            {
                Debug.LogError("SaveManager.Instance is null");
                
                if (statusText != null)
                {
                    statusText.text = "Error: SaveManager.Instance is null";
                }
            }
        }
        catch (System.Exception e)
        {
            Debug.LogError($"Error testing SimpleSaveSystem: {e.Message}");
            
            if (statusText != null)
            {
                statusText.text = $"Error: {e.Message}";
            }
        }
        #else
        Debug.Log("SimpleSaveSystem test is only available on Android devices");
        
        if (statusText != null)
        {
            statusText.text = "Test only available on Android devices";
        }
        #endif
    }
    
    public void DisplaySaveInfo()
    {
        string info = "Save System Info:\n";
        
        #if UNITY_ANDROID && !UNITY_EDITOR
        info += "Platform: Android\n";
        
        // Check if SimpleSaveSystem file exists
        bool simpleSaveExists = SimpleSaveSystem.SaveFileExists();
        info += $"SimpleSaveSystem file exists: {simpleSaveExists}\n";
        
        // Check if JSON save file exists
        string jsonPath = System.IO.Path.Combine(Application.persistentDataPath, "gamesave.json");
        bool jsonSaveExists = System.IO.File.Exists(jsonPath);
        info += $"JSON save file exists: {jsonSaveExists}\n";
        
        // Add persistent data path
        info += $"PersistentDataPath: {Application.persistentDataPath}";
        #else
        info += "Platform: Not Android\n";
        info += $"PersistentDataPath: {Application.persistentDataPath}";
        #endif
        
        Debug.Log(info);
        
        if (statusText != null)
        {
            statusText.text = info;
        }
    }
} 