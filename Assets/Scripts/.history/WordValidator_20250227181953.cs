...
public static class WordValidator
{
                    ...
                    if (!string.IsNullOrEmpty(wordEntry.word))
                    {
                        ...
                        else if (language == "tr")
                        {
                            ...
                            if (wordEntry.translations != null && !string.IsNullOrEmpty(wordEntry.translations.tr))
                            {
                                ...
                                Debug.Log($"[Android Debug] Storing Turkish word '{wordKey}' with didYouKnow: '{(string.IsNullOrEmpty(wordEntry.didYouKnow) ? "EMPTY" : wordEntry.didYouKnow.Substring(0, Math.Min(30, wordEntry.didYouKnow.Length)) + "...")}'");
                            }
                            else
                            {
    ...
    public static string GetFactForWord(string word, string era, string language)
    {
        ...
        if (string.IsNullOrEmpty(fact))
        {
            ...
            // If we're in Turkish language and the fact is empty, try to get it from the English version
            if (language == "tr" && loadedWords.ContainsKey("en") && loadedWords["en"].ContainsKey(normalizedEra))
            {
