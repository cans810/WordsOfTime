...
public class InterstitialAdExample : MonoBehaviour, IUnityAdsLoadListener, IUnityAdsShowListener, IUnityAdsInitializationListener
{
    ...
    public void LoadAd()
    {
        ...
        // Don't try to load if we're already loading or have loaded an ad
        if (isAdLoading)
        {
            Debug.Log("Ad is already being loaded, ignoring duplicate request");
            return;
        }
        
        if (isAdLoaded)
        {
            Debug.Log("Ad is already loaded, no need to load again");
            return;
        }
    ...
    public void OnUnityAdsFailedToLoad(string placementId, UnityAdsLoadError error, string message)
    {
        ...
        StartCoroutine(RetryLoadAfterDelay(3.0f));
    }
    
    private IEnumerator RetryLoadAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);
        LoadAd(); // Try to load another ad after delay
    }
