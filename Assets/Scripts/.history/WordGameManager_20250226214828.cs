...
public class WordGameManager : MonoBehaviour
{
    ...
    private void UpdateProgressBar()
    {
        ...
        for (int i = 0; i < progressImages.Count; i++)
        {
            ...
            if (wordAtIndex != null)
            {
                // Use GameManager.Instance.IsWordSolved instead of checking allSolvedBaseWords directly
                isSolved = GameManager.Instance.IsWordSolved(wordAtIndex);
                Debug.Log($"Progress bar word {wordAtIndex} is solved: {isSolved}");
            }
    ...
    public void StartNewGameInEra()
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            {
                if (!GameManager.Instance.IsWordSolved(currentEraWords[i]))
                {
                    currentWordIndex = i;
                    break;
                }
    ...
    private void UpdateSentenceDisplay(string word = null)
    {
        if (sentenceText != null && !string.IsNullOrEmpty(originalSentence))
        {
            ...
            if (targetWord != null && originalSentence.Contains("_____"))
            {
                ...
                if (GameManager.Instance.IsWordSolved(targetWord))
                {
                    // Show guessed word
    ...
    public bool IsWordSolved(string word)
    {
        ...
        // Directly use GameManager.Instance.IsWordSolved
        bool isSolved = GameManager.Instance.IsWordSolved(word);
        Debug.Log($"WordGameManager.IsWordSolved: Checking if word '{word}' is solved: {isSolved}");
        return isSolved;
    }
    ...
    public void LoadWord(int index)
    {
        ...
        Debug.Log($"Word {targetWord} is solved: {isSolved}, isCurrentWordNewlySolved: {isCurrentWordNewlySolved}");
        
        if (isSolved && !isCurrentWordNewlySolved)
        {
            Debug.Log($"Word {targetWord} is already solved (not newly solved), showing Did You Know fact for {targetWord} instantly");
        ...
        else
        {
            ...
            if (isSolved && isCurrentWordNewlySolved)
            {
                Debug.Log($"Word {targetWord} was just solved in this session, not showing Did You Know fact instantly");
            }
            else
            {
                Debug.Log($"Word {targetWord} is not solved yet.");
            }
        }
    ...
    private void UpdateHintButton()
    {
        if (hintButton != null)
        {
            // Use targetWord instead of currentBaseWord
            bool isSolved = GameManager.Instance.IsWordSolved(targetWord);
            Debug.Log($"UpdateHintButton: Checking if word '{targetWord}' is solved: {isSolved}");
    ...
    public void LoadSolvedWords(List<string> words)
    {
        if (words != null && words.Count > 0)
        {
            solvedWords = new HashSet<string>(words);
            Debug.Log($"Loaded {solvedWords.Count} solved words: {string.Join(", ", solvedWords)}");
            
            // Update the progress bar to reflect the loaded solved words
            UpdateProgressBar();
        }
    }
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        // If the word is not solved, hide the panel and exit
        if (!isWordSolved)
        {
            Debug.Log($"[Android Debug] Word '{targetWord}' is not solved, hiding Did You Know panel");
            didYouKnowPanel.SetActive(false);
        ...
        {
            okayButtonTransform.gameObject.SetActive(true);
            Debug.Log("[Android Debug] OkayButton set to active in ShowDidYouKnowCoroutine for newly solved word");
        }
        else
        {
            Debug.LogError("[Android Debug] OkayButton not found in didYouKnowPanel");
