...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    [SerializeField] private float coinSmoothTime = 0.6f; // Faster movement
    [SerializeField] private float coinMaxSpeed = 1000f;  // Higher max speed
    [SerializeField] private float curveHeight = 200f;    // Height of the curve
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin, System.Action onDestroy)
    {
        ...
        // Calculate control point for the curve (above the direct path)
        Vector3 midPoint = (startPosition + targetPosition) / 2f;
        Vector3 controlPoint = midPoint + Vector3.up * curveHeight;
        
        float elapsedTime = 0f;
        float duration = coinSmoothTime + UnityEngine.Random.Range(-0.1f, 0.1f); // Slight random variation

        while (elapsedTime < duration)
        {
            float t = elapsedTime / duration;
            
            // Use quadratic bezier curve for movement
