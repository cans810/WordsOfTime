...
public static class WordValidator
{
    ...
    public static void LoadWordSets()
    {
        ...
        // Initialize the dictionary for loaded words
        loadedWords = new Dictionary<string, Dictionary<string, Dictionary<string, Word>>>();
        ...
        // If we're on Android and not in the editor, wait until GameManager is ready
            ...
            didInitiateLoading = true;  // Mark that we've started the loading process
            // Start trying to load words
    ...
    // Method to initialize word loading on Android platforms - moved outside platform-specific block
    public static void TryStartAndroidLoading()
    {
        Debug.Log("[WordValidator] TryStartAndroidLoading called");
        ...
        if (GameManager.Instance == null)
        {
            ...
            {
                UnityEngine.Object.FindObjectOfType<GameManager>().StartCoroutine(RetryLoading());
            }
            return;
        }
        ...
        #else
        // For non-Android platforms, we still need to initialize
        if (!didInitiateLoading)
        {
            Debug.Log("[WordValidator] Initializing word loading on non-Android platform");
            didInitiateLoading = true;
            LoadWordsForLanguage("en");
            LoadWordsForLanguage("tr");
        }
    ...
    private static IEnumerator RetryLoading()
    {
        ...
        Debug.Log("[WordValidator] Retrying word loading...");
        TryStartAndroidLoading();
    }
    ...
    private static void ProcessWordJson(string jsonContent, string language)
    {
        ...
        try
        {
            ...
            if (wordData == null || wordData.eras == null)
            {
            ...
            // Process each era
            foreach (Era era in wordData.eras)
            {
                string eraName = era.name.ToLower();
                ...
                foreach (Word word in era.words)
                {
                    if (!string.IsNullOrEmpty(word.word))
                    {
                        loadedWords[language][eraName][word.word.ToLower()] = word;
                    }
                }
            }
    ...
    public static string GetFactForWord(string word, string era, string language)
    {
        ...
        // Log loaded languages count
        Debug.Log($"[Android Debug] Number of loaded languages: {loadedLanguages.Count}");
        Debug.Log($"[Android Debug] Loaded words dictionary has {loadedWords.Count} languages");

        // Log available languages and eras
        Debug.Log("[Android Debug] Available languages and eras:");
        foreach (var lang in loadedWords.Keys)
        {
        ...
        if (!loadedWords.ContainsKey(language))
        {
            ...
            // Force reload if no languages are found
            if (loadedWords.Count == 0)
            {
                Debug.LogWarning("[Android Debug] No languages loaded, forcing reload");
                isLoading = true;
                TryStartAndroidLoading();
            }
