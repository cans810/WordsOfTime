...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        // If we're using auto-close and this is NOT a newly solved word (no OK button)
        if (factDisplayTime > 0 && !isCurrentWordNewlySolved)
        {
            Debug.Log($"[DidYouKnow Debug] Auto-close timer started for {factDisplayTime} seconds (for previously solved word)");
    ...
    public void ShowDidYouKnowInstantly()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(fact) && fact != "LOADING")
            {
                ...
                // Start auto-close timer for already solved words
                StartCoroutine(AutoCloseDidYouKnowPanel());
            }
            else
            {
    ...
    private IEnumerator AutoCloseDidYouKnowPanel()
    {
    ...
    {
        // Implementation of PositionPanelInSafeArea method
        // This method should position the panel in the safe area of the screen
        Debug.Log("Positioning panel in safe area");
    }
}