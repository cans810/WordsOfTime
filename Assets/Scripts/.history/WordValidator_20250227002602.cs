...
public static class WordValidator
{
    ...
    private static void TryStartAndroidLoading()
    {
        ...
        {
            Debug.LogWarning("[WordValidator] GameManager.Instance is null, will retry loading later");
            // Schedule a retry
            if (Application.isPlaying)
            {
                ...
                if (mb != null)
                {
                    mb.StartCoroutine(RetryLoading());
                }
                else
                {
                    Debug.LogError("[WordValidator] Could not find any MonoBehaviour to start coroutine");
                }
            }
            return;
        }

        Debug.Log("[WordValidator] GameManager.Instance is available, starting Android loading");
        isLoading = true;
        
        // Start loading for each language
        MonoBehaviour mb = GameManager.Instance.GetComponent<MonoBehaviour>();
        if (mb != null)
        {
            mb.StartCoroutine(LoadWordsForLanguageAndroid("en"));
            mb.StartCoroutine(LoadWordsForLanguageAndroid("tr"));
        }
        else
        {
            Debug.LogError("[WordValidator] GameManager does not have a MonoBehaviour component");
        }
    }
    
    private static IEnumerator RetryLoading()
    {
        yield return new WaitForSeconds(0.5f);
        Debug.Log("[WordValidator] Retrying Android loading");
        TryStartAndroidLoading();
    }
    ...
    private static IEnumerator LoadWordsForLanguageAndroid(string language)
    {
        ...
        yield return LoadJsonFromStreamingAssets(jsonFilePath, language);
    }
    ...
    private static IEnumerator LoadJsonFromStreamingAssets(string filePath, string language)
    {
        ...
        {
            // Parse the JSON
            WordData wordData = JsonUtility.FromJson<WordData>(jsonText);
            ...
            Debug.Log($"[WordValidator] Successfully parsed JSON for language {language}, sets count: {wordData.sets.Length}");
            
            // Process the sets
            foreach (var set in wordData.sets)
            {
                string era = set.era.ToLower();
                ...
                foreach (var wordEntry in set.words)
                {
                    ...
                    string wordText = wordEntry.word.ToLower();
                    ...
                    if (!string.IsNullOrEmpty(wordEntry.didYouKnow))
                    {
                        ...
                        string originalEra = set.era; // Use original case for era
                        ...
                        string originalWord = wordEntry.word.ToUpper(); // Use uppercase for word
                        wordSetsWithFactsByLanguage[language][originalEra][originalWord] = wordEntry.didYouKnow;
                    }
                }
            }
    ...
    private static void LogSampleData(string language)
    {
        if (loadedWords.ContainsKey(language) && loadedWords[language].Count > 0)
        {
            ...
            if (loadedWords[language][firstEra].Count > 0)
            {
                ...
                Debug.Log($"[WordValidator] Sample data for language {language}: Era: {firstEra}, Word: {firstWord.word}, DidYouKnow: {firstWord.didYouKnow}");
            }
        }
