...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator AnimatePointsIncrease(int pointsToAdd)
    {
        ...
        int initialPoints = GameManager.Instance.CurrentPoints; // Renamed to avoid conflict
        int targetPoints = initialPoints + pointsToAdd;
        ...
        int lastPoints = initialPoints; // Use renamed variable
        ...
        while (elapsedTime < animationDuration)
        {
            ...
            int currentPointsValue = Mathf.RoundToInt(Mathf.Lerp(initialPoints, targetPoints, t)); // Renamed to avoid conflict
            
            // If points value changed, create a bump effect
            if (currentPointsValue != lastPoints)
            {
                ...
                lastPoints = currentPointsValue;
            }
            
            // Update points display
            if (pointText != null)
            {
                pointText.text = currentPointsValue.ToString();
    ...
    public void HandleCorrectWord(List<LetterTile> guessedTiles)
    {
        ...
        StartCoroutine(SpawnCoins(8, GameManager.POINTS_PER_WORD, guessedTiles));
        StartCoroutine(AnimatePointsIncrease(GameManager.POINTS_PER_WORD, guessedTiles));
    ...
    public void CheckWord(string word, List<LetterTile> selectedTiles)
    {
        ...
        if (word == targetWord)
        {
            ...
            // Start both animations simultaneously
            StartCoroutine(SpawnCoins(8, GameManager.POINTS_PER_WORD, selectedTiles));
    ...
    private IEnumerator SpawnCoins(int coinCount, int pointsToAdd, List<LetterTile> guessedTiles)
    {
