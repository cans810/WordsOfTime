...
public class SaveManager : MonoBehaviour
{
    ...
    private void Start()
    {
        // Don't automatically load game here anymore, GameManager will handle it
        // LoadGame();
    }
    ...
    public void SaveGame()
    {
        ...
        try
        {
            ...
            // Save solved word positions
            Data.solvedWordPositions.Clear();
            foreach (var entry in GameManager.Instance.SolvedWordPositions)
            {
                List<Vector2IntSerializable> serializablePositions = new List<Vector2IntSerializable>();
                foreach (var pos in entry.Value)
                {
                    serializablePositions.Add(new Vector2IntSerializable(pos.x, pos.y));
                }
                Data.solvedWordPositions[entry.Key] = serializablePositions;
            }
            Debug.Log($"Saving {Data.solvedWordPositions.Count} solved word positions");
            
            // Save solved base words per era
            Data.solvedBaseWordsPerEra.Clear();
            Debug.Log($"Saving solved base words per era for {GameManager.Instance.GetAllEras().Count} eras");

            // Debug logging to check all eras
            Debug.Log($"All eras in GameManager: {string.Join(", ", GameManager.Instance.GetAllEras())}");
            Debug.Log($"Unlocked eras: {string.Join(", ", GameManager.Instance.GetUnlockedEras())}");
            Debug.Log($"Current era: {GameManager.Instance.CurrentEra}");

            foreach (var era in GameManager.Instance.GetAllEras())
            {
                HashSet<string> baseWords = GameManager.Instance.GetSolvedBaseWordsForEra(era);
                Debug.Log($"Era {era} has {(baseWords != null ? baseWords.Count : 0)} solved base words");
                if (baseWords != null && baseWords.Count > 0)
                {
                    Data.solvedBaseWordsPerEra[era] = new List<string>(baseWords);
                    Debug.Log($"Saving {baseWords.Count} solved base words for era {era}: {string.Join(", ", baseWords)}");
                }
            }
    ...
    public void LoadGame(bool updateGameManager)
    {
        ...
        try
        {
            if (File.Exists(SavePath))
            {
                ...
                if (updateGameManager && GameManager.Instance != null)
                {
                    ...
                    // Load solved word positions
                    if (Data.solvedWordPositions != null && Data.solvedWordPositions.Count > 0)
                    {
                        Debug.Log($"Loading {Data.solvedWordPositions.Count} solved word positions");
                        foreach (var entry in Data.solvedWordPositions)
                        {
                            List<Vector2Int> unityPositions = new List<Vector2Int>();
                            foreach (var pos in entry.Value)
                            {
                                unityPositions.Add(new Vector2Int(pos.x, pos.y));
                            }
                            GameManager.Instance.StoreSolvedWordPositions(entry.Key, unityPositions);
                        }
                    }
                    
                    // Load solved base words per era
                    if (Data.solvedBaseWordsPerEra != null && Data.solvedBaseWordsPerEra.Count > 0)
                    {
                        Debug.Log($"Loading solved base words for {Data.solvedBaseWordsPerEra.Count} eras: {string.Join(", ", Data.solvedBaseWordsPerEra.Keys)}");
                        foreach (var entry in Data.solvedBaseWordsPerEra)
                        {
                            ...
                            {
                                GameManager.Instance.StoreSolvedBaseWord(era, baseWord, false);
                                Debug.Log($"Loaded solved base word '{baseWord}' for era '{era}'");
                            }
                        }
                    }
                    else
