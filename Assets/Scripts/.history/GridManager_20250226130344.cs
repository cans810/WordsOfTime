    ...
    public void UpdateSolvedWordsDisplay()
    {
        if (GameManager.Instance != null)
        {
            ...
            foreach (var baseWord in GameManager.Instance.GetSolvedBaseWordsForEra(GameManager.Instance.CurrentEra))
            {
                ...
                List<Vector2Int> positions = FindWordInGrid(currentLanguageWord, grid);
                
                if (positions != null && positions.Count > 0)
                {
                    ShowSolvedWord(currentLanguageWord, positions);
                    Debug.Log($"Found word {currentLanguageWord} in grid, highlighting");
                }
                else
                {
                    Debug.LogWarning($"Could not find word {currentLanguageWord} in grid");
                }
            }
        }
    }
