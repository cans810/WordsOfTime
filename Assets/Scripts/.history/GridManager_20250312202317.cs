...
public class GridManager : MonoBehaviour
{
    ...
    public void StartWordSelection(LetterTile tile)
    {
        if (!isSelecting)
        {
            ...
            // Vibrate the device when a tile is first selected
            WordGameManager.Instance.VibrateDevice();
    ...
    public void AddToSelection(LetterTile tile)
    {
        if (isSelecting && !selectedTiles.Contains(tile))
        {
            ...
            if (IsAdjacent(lastPos, newPos))
            {
                ...
                // Vibrate the device when a tile is dragged
                WordGameManager.Instance.VibrateDevice();
    ...
    {
        Debug.Log("[Android Debug] Clearing all tiles from the grid");
        ...
        if (grid != null)
        {
            for (int x = 0; x < gridSize; x++)
            {
                for (int y = 0; y < gridSize; y++)
                {
                    if (grid[x, y] != null)
                    {
                        // Just reset the tile state instead of destroying it
                        grid[x, y].ResetTile();
    ...
    public LetterTile GetTileAtPosition(Vector2Int position)
    {
