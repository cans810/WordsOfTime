...
public class GameManager : MonoBehaviour
{
    ...
    {
        Debug.Log($"Getting positions for word: {word}, exists in dictionary: {solvedWordPositions.ContainsKey(word)}");
        ...
        // If we can't find the word directly, try to find its base word and then get the translation
        string baseWord = GetBaseWord(word);
        if (!string.IsNullOrEmpty(baseWord) && baseWord != word)
        {
            string translatedWord = GetCurrentLanguageWord(baseWord);
            Debug.Log($"Trying with base word: {baseWord}, translated to: {translatedWord}");
            
            if (solvedWordPositions.ContainsKey(translatedWord))
            {
                positions = solvedWordPositions[translatedWord];
                return true;
            }
        }
    ...
    public string GetBaseWord(string word)
    {
        ...
        {
            Debug.LogWarning("GetBaseWord called with null or empty word");
            return string.Empty;
        }
        
        Debug.Log($"Getting base word for: {word} in era: {CurrentEra}");
        
        // First check if the word is in the current era's word list
        if (eraWordsPerLanguage.ContainsKey(currentLanguage) && 
            eraWordsPerLanguage[currentLanguage].ContainsKey(CurrentEra) &&
            eraWordsPerLanguage["en"].ContainsKey(CurrentEra))
        {
            var currentLanguageWords = eraWordsPerLanguage[currentLanguage][CurrentEra];
            var englishWords = eraWordsPerLanguage["en"][CurrentEra];
            
            int index = currentLanguageWords.IndexOf(word);
            if (index >= 0 && index < englishWords.Count)
            {
                string baseWord = englishWords[index];
                Debug.Log($"Found base word in current era ({CurrentEra}): {baseWord}");
                return baseWord;
            }
        }
        
        // If not found in current era, check the WordSetList
    ...
    public HashSet<string> GetSolvedBaseWordsForEra(string era)
    {
        ...
        {
            solvedBaseWordsPerEra[era] = new HashSet<string>();
            Debug.Log($"Initialized solvedBaseWordsPerEra for era: {era}");
        }
    ...
    private void UpdateSolvedWordsDisplay()
    {
        if (GridManager.Instance != null)
        {
            ...
            // Make sure the current era is initialized in the dictionary
            if (!solvedBaseWordsPerEra.ContainsKey(CurrentEra))
            {
                Debug.LogWarning($"Era {CurrentEra} not found in solvedBaseWordsPerEra, initializing it");
                solvedBaseWordsPerEra[CurrentEra] = new HashSet<string>();
            }
            
            Debug.Log($"Updating solved words display for era: {CurrentEra}, found {solvedBaseWordsPerEra[CurrentEra].Count} solved words");
            ...
            foreach (var baseWord in solvedBaseWordsPerEra[CurrentEra])
            {
                ...
                Debug.Log($"Base word: {baseWord}, translated to: {currentLanguageWord}");
                
                if (solvedWordPositions.ContainsKey(currentLanguageWord))
                {
                    Debug.Log($"Found positions for {currentLanguageWord}, highlighting in grid");
                    GridManager.Instance.ShowSolvedWord(currentLanguageWord, solvedWordPositions[currentLanguageWord]);
                }
                else
                {
                    // Try to find the word positions using the base word
                    if (solvedWordPositions.ContainsKey(baseWord))
                    {
                        Debug.Log($"Found positions for base word: {baseWord}, highlighting in grid");
                        GridManager.Instance.ShowSolvedWord(currentLanguageWord, solvedWordPositions[baseWord]);
                    }
                    else
                    {
                        Debug.LogWarning($"No positions found for translated word: {currentLanguageWord} or base word: {baseWord}");
                    }
                }
            }
        }
    ...
    public void StoreSolvedBaseWord(string era, string baseWord)
    {
        ...
        // Store the current era temporarily to ensure translations are from the correct era
        string previousEra = CurrentEra;
        CurrentEra = era;
        ...
        foreach (string language in new[] { "en", "tr" })
        {
            ...
            Debug.Log($"Storing translation for {language}: {translatedWord} in era: {era}");
    ...
    public string GetTranslation(string baseWord, string language)
    {
        ...
        {
            Debug.LogWarning("GetTranslation called with null or empty baseWord");
            return string.Empty;
        }
        ...
        {
            string translation = wordTranslations[baseWord][language];
            Debug.Log($"Found translation in cache: {translation}");
            return translation;
        }
        
        // If we can't find a direct translation, try to find the word in the era words
        // But ONLY in the current era to avoid cross-era conflicts
        if (eraWordsPerLanguage.ContainsKey("en") && 
            eraWordsPerLanguage.ContainsKey(language) &&
            eraWordsPerLanguage["en"].ContainsKey(CurrentEra) &&
            eraWordsPerLanguage[language].ContainsKey(CurrentEra))
        {
            var englishWords = eraWordsPerLanguage["en"][CurrentEra];
            var targetLanguageWords = eraWordsPerLanguage[language][CurrentEra];
            
            int index = englishWords.IndexOf(baseWord);
            if (index >= 0 && index < targetLanguageWords.Count)
            {
                string translation = targetLanguageWords[index];
                Debug.Log($"Found translation from current era ({CurrentEra}) words: {translation}");
                ...
                wordTranslations[baseWord][language] = translation;
                
                return translation;
            }
        }
        
        // If we still haven't found a translation, use the WordSetList for a more accurate lookup
        WordSetList wordSetList = LoadWordSetList();
        if (wordSetList != null)
        {
            foreach (WordSet wordSet in wordSetList.sets)
            {
                foreach (WordEntry wordEntry in wordSet.words)
                {
                    ...
                    {
                        string translation = language == "tr" ? wordEntry.translations.tr : wordEntry.translations.en;
                        Debug.Log($"Found translation in WordSetList: {translation} (era: {wordSet.era})");
        ...
        Debug.LogWarning($"No translation found for {baseWord} in {language}, returning base word");
        return baseWord;
    }
