...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Set initial state before wheel becomes visible
        gameObject.SetActive(false);
        if (spinButton != null) spinButton.gameObject.SetActive(false);
        if (prizetext != null) prizetext.gameObject.SetActive(false);
        if (prizeWonPanel != null) prizeWonPanel.gameObject.SetActive(false);
        if (trianglePicker != null) trianglePicker.gameObject.SetActive(false);
