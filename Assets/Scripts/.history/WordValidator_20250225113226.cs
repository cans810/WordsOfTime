...
public static class WordValidator
{
    ...
    {
        Debug.Log($"[Android Debug] Getting fact for word: '{word}', era: '{era}', language: '{language}'");
        
        if (!isInitialized)
        {
            Debug.LogError("[Android Debug] WordValidator not initialized! Trying to initialize now...");
            LoadWordSets();
            // Return empty for now, facts will be available after initialization completes
            return string.Empty;
        }
        
        if (!wordSetsWithFactsByLanguage.ContainsKey(language))
        {
            Debug.LogWarning($"[Android Debug] No facts dictionary found for language: {language}");
            return string.Empty;
        }
        ...
        Debug.Log($"[Android Debug] Looking for era: '{translatedEra}' in {language}");
        Debug.Log($"[Android Debug] Available eras in {language}: {string.Join(", ", wordSetsWithFactsByLanguage[language].Keys)}");

        if (!wordSetsWithFactsByLanguage[language].ContainsKey(translatedEra))
        {
            Debug.LogWarning($"[Android Debug] No facts found for era: '{translatedEra}' in language: {language}. Available eras: {string.Join(", ", wordSetsWithFactsByLanguage[language].Keys)}");
            return string.Empty;
        }

        string wordKey = word.ToUpper();
        Debug.Log($"[Android Debug] Available words in era '{translatedEra}': {string.Join(", ", wordSetsWithFactsByLanguage[language][translatedEra].Keys)}");
        ...
        if (wordSetsWithFactsByLanguage[language][translatedEra].ContainsKey(wordKey))
        {
            ...
            Debug.Log($"[Android Debug] Found fact for word '{wordKey}': {fact}");
            return fact;
        }

        Debug.LogWarning($"[Android Debug] No fact found for word: '{wordKey}' in era: '{translatedEra}', language: {language}");
        return string.Empty;
    }
}
