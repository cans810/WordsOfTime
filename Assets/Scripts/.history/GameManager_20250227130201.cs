...
public class GameManager : MonoBehaviour
{
    ...
    public void SetLanguage(string newLanguage)
    {
        ...
        // Update the solved words for the new language
        UpdateSolvedWordsForLanguage(newLanguage);
    ...
    {
        // First try to find the word in the current language's word list
        if (eraWordsPerLanguage.ContainsKey(CurrentLanguage) && 
            eraWordsPerLanguage[CurrentLanguage].ContainsKey(CurrentEra))
        {
            var wordList = eraWordsPerLanguage[CurrentLanguage][CurrentEra];
            
            // If the word is in the current language's list, find its base form
            if (wordList.Contains(word.ToUpper()))
            {
                // Use WordValidator to get the base word (English version)
                string baseWord = WordValidator.GetBaseWordForTranslation(word, CurrentLanguage, CurrentEra);
                Debug.Log($"[GameManager] Found base word '{baseWord}' for '{word}' in {CurrentLanguage}");
                return baseWord;
            }
        }
        
        // If we couldn't find it in the current language, try using the word as is
        // This handles cases where the word might already be in its base form
        Debug.Log($"[GameManager] Using word '{word}' as its own base word");
        return word.ToUpper();
    }
    ...
    {
        Debug.Log($"[GameManager] Storing solved base word: {baseWord} for era: {era}");
        
        // Ensure the word is in uppercase for consistency
        baseWord = baseWord.ToUpper();
        ...
        // Add the base word to the solved set for this era
        solvedBaseWordsPerEra[era].Add(baseWord);
        ...
        foreach (string language in new[] { "en", "tr" })
        {
            ...
            Debug.Log($"[GameManager] Storing translation for {language}: {translatedWord}");
            
            if (!solvedWordsPerLanguage.ContainsKey(language))
            {
                solvedWordsPerLanguage[language] = new HashSet<string>();
            }
            solvedWordsPerLanguage[language].Add(translatedWord.ToUpper());
        }
    ...
    private string GetBaseWordKey(string word, string language, string era)
    {
        ...
        // Use WordValidator to get the consistent word key (English version)
        string baseWordKey = WordValidator.GetWordKeyAcrossLanguages(word, language, era);
        Debug.Log($"[GameManager] Base word key for '{word}' in {language}/{era}: '{baseWordKey}'");
        return baseWordKey;
    }
    ...
    public bool IsWordSolved(string word)
    {
        ...
        // Get the base word key that's consistent across languages
        string baseWordKey = GetBaseWordKey(word, CurrentLanguage, CurrentEra);
        
        // Check if this base word has been solved in the current era
        bool isSolved = solvedBaseWordsPerEra.ContainsKey(CurrentEra) && 
                       solvedBaseWordsPerEra[CurrentEra].Contains(baseWordKey);
        
        Debug.Log($"[GameManager] Checking if word '{word}' (base key: '{baseWordKey}') is solved: {isSolved}");
        return isSolved;
    }
    ...
    public void MarkWordAsSolved(string word)
    {
        ...
        // Add the base word key to the solved set
        solvedBaseWordsPerEra[CurrentEra].Add(baseWordKey);
        
        // Also track the actual word in the current language
        if (!solvedWordsPerLanguage.ContainsKey(CurrentLanguage))
        {
            solvedWordsPerLanguage[CurrentLanguage] = new HashSet<string>();
        }
        solvedWordsPerLanguage[CurrentLanguage].Add(word.ToUpper());
        
        Debug.Log($"[GameManager] Marked word '{word}' (base key: '{baseWordKey}') as solved in {CurrentEra}");
        
        // Save the game state
        SaveGameState();
    }

    // Add this method to update solved words when changing language
    public void UpdateSolvedWordsForLanguage(string language)
    {
        Debug.Log($"[GameManager] Updating solved words for language: {language}");
        
        // Make sure we have the language dictionary initialized
        if (!solvedWordsPerLanguage.ContainsKey(language))
        {
            solvedWordsPerLanguage[language] = new HashSet<string>();
        }
        
        // For each era where we have solved words
        foreach (var eraPair in solvedBaseWordsPerEra)
        {
            string era = eraPair.Key;
            var solvedBaseWords = eraPair.Value;
            
            Debug.Log($"[GameManager] Processing {solvedBaseWords.Count} solved words for era: {era}");
            
            // For each solved base word in this era
            foreach (string baseWordKey in solvedBaseWords)
            {
                // Find the corresponding word in the target language
                // Since we don't have GetTranslation, we'll use the word lists we already have
                if (eraWordsPerLanguage.ContainsKey(language) && 
                    eraWordsPerLanguage[language].ContainsKey(era))
                {
                    // Look through the words in this language/era to find matching translations
                    foreach (var word in eraWordsPerLanguage[language][era])
                    {
                        // Get the base key for this word and check if it matches our target
                        string wordBaseKey = GetBaseWordKey(word, language, era);
                        if (wordBaseKey.Equals(baseWordKey, StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Log($"[GameManager] Adding translated word '{word}' for base word '{baseWordKey}' to solved words for {language}");
                            solvedWordsPerLanguage[language].Add(word.ToUpper());
                            break;
                        }
                    }
                }
        ...
        Debug.Log($"[GameManager] Updated solved words for {language}: {string.Join(", ", solvedWordsPerLanguage[language])}");
    }
}