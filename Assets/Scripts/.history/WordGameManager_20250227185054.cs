...
public class WordGameManager : MonoBehaviour
{
    ...
    private void HandleLanguageChanged()
    {
        ...
        // No need to call UpdateSentenceText() as LoadWord will handle this
        
        // Update the sentence text with the current language
        UpdateSentenceText();
    }

    // Add a method to update the sentence text based on the current language
    private void UpdateSentenceText()
    {
        if (string.IsNullOrEmpty(targetWord) || GameManager.Instance == null)
            return;
            
        string language = GameManager.Instance.CurrentLanguage;
        string era = GameManager.Instance.CurrentEra;
        
        // Get a sentence for the current word in the current language
        List<string> sentences = GameManager.Instance.GetSentencesForWord(targetWord, era, language);
