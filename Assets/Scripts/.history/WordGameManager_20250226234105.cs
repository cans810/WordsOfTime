...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        // Make sure the panel references exist
        if (didYouKnowPanel == null)
        {
            ...
            if (didYouKnowPanel == null)
            {
                Debug.LogError("[Android Debug] Could not find DidYouKnowPanel in the scene!");
                yield break;
            }
            else
            {
        ...
        if (fact == "LOADING")
        {
            Debug.Log("[Android Debug] Facts are still loading, will retry");
            
            // Wait and retry up to 15 times (increased from 10 to give more time on slower devices)
            int retryCount = 0;
            int maxRetries = 15;
            ...
            while (retryCount < maxRetries)
            {
                ...
                try
                {
                    // Explicitly check if WordValidator is available
                    if (GameManager.Instance != null)
                    {
                        fact = WordValidator.GetFactForWord(targetWord, GameManager.Instance.CurrentEra, GameManager.Instance.CurrentLanguage);
                        Debug.Log($"[Android Debug] Got fact on retry {retryCount}: {(fact == "LOADING" ? "STILL LOADING" : (string.IsNullOrEmpty(fact) ? "EMPTY" : "SUCCESS"))}");
                    ...
                    {
                        Debug.LogError("[Android Debug] GameManager.Instance is null!");
                        break;
                    }
                }
                catch (System.Exception e)
                {
                    Debug.LogError($"[Android Debug] Error getting fact: {e.Message}");
                }
        ...
        try
        {
            ...
            {
                Debug.LogError("[Android Debug] didYouKnowPanel is null!");
                isAnimationPlaying = false;
                EnableNavigationButtons();
            }
        }
        ...
        {
            Debug.LogError($"[Android Debug] Error showing fact panel: {e.Message}");
            isAnimationPlaying = false;
            EnableNavigationButtons();
        }
    }
            ...
            if (panelRect != null)
            {
                try
                {
                    ...
                    if (canvas != null)
                    {
                        ...
                        panelRect.anchoredPosition = new Vector2(safeAreaCenterX, safeAreaCenterY);
                        Debug.Log($"[Android Debug] Positioned didYouKnowPanel at {safeAreaCenterX}, {safeAreaCenterY}");
                    }
                    else
                    {
                        Debug.LogWarning("[Android Debug] Canvas not found for didYouKnowPanel");
                    }
                }
                catch (System.Exception e)
                {
