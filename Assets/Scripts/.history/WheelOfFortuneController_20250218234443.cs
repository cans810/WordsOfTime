...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    [SerializeField] private float coinSpeedMin = 0.0000000001f; // Extremely slow
    [SerializeField] private float coinSpeedMax = 0.0000000005f;   // Very slow
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin)
    {
        ...
        float speed = Random.Range(coinSpeedMin, coinSpeedMax);

        // Move the coin to the target position
        while (Vector2.Distance(coin.transform.position, targetPosition) > 0.1f) // Reduced threshold for precision
        {
            // Move the coin towards the target position at the calculated speed
