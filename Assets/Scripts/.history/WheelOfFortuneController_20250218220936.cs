...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private IEnumerator SpawnCoins(int coinCount, System.Action onComplete)
    {
        ...
        {
            // Instantiate a coin at the triangle's position
            GameObject coin = Instantiate(coinForAnimationPrefab, trianglePicker.transform.position, Quaternion.identity, canvas.transform);

            // Add a random offset to the starting position
            Vector2 randomOffset = new Vector2(UnityEngine.Random.Range(-coinSpread, coinSpread), Random.Range(-coinSpread, coinSpread));
