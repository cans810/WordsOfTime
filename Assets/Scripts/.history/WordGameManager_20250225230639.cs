...
public class WordGameManager : MonoBehaviour
{
    ...
    public void LoadWord(int index)
    {
        ...
        // Get the base word (English version)
        string baseWord = GameManager.Instance.GetBaseWord(originalWord);
        
        // Get the proper translation for the current language
        targetWord = GameManager.Instance.GetTranslation(baseWord, GameManager.Instance.CurrentLanguage);
        
        Debug.Log($"LoadWord: Original={originalWord}, Base={baseWord}, Translated={targetWord}");
        ...
        {
            Debug.Log($"Word {baseWord} is already solved, highlighting {targetWord} in grid");
    ...
    public void NextWord()
    {
        ...
        if (GameManager.Instance.IsWordSolved(baseWord))
        {
            Debug.Log($"Word {baseWord} is solved, highlighting in grid");
            HighlightWordInGrid(targetWord);
        }
        else
        {
    ...
    {
        // Make sure we're using the word in the current language
        string baseWord = GameManager.Instance.GetBaseWord(word);
        string translatedWord = GameManager.Instance.GetTranslation(baseWord, GameManager.Instance.CurrentLanguage);
        
        Debug.Log($"Highlighting word in grid: Original={word}, Base={baseWord}, Translated={translatedWord}");
