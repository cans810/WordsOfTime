...
public class WordGameManager : MonoBehaviour
{
    ...
    private void ShowAd()
    {
        ...
        if (!EnsureAdsInitialized())
        {
            ...
            // Reset the counter anyway to avoid showing multiple ads in a row
            // when initialization completes
            wordsGuessedCount = 0;
            Debug.Log("Counter reset to 0 due to ads not being initialized");
            
            // Try to initialize ads for next time
        ...
        if (AdManager.Instance != null)
        {
            try
            {
                ...
                wordsGuessedCount = 0;
                Debug.Log("Counter reset to 0 after showing ad");
            }
            catch (System.Exception e)
            {
                Debug.LogError($"Error showing ad: {e.Message}");
                
                // Reset counter even if there was an error
                wordsGuessedCount = 0;
                Debug.Log("Counter reset to 0 due to error showing ad");
            }
        }
        else
        {
