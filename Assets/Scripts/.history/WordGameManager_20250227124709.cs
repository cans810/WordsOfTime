...
public class WordGameManager : MonoBehaviour
{
    ...
    private void HandleLanguageChanged()
    {
        ...
        // Get the base words for the current era
        HashSet<string> solvedBaseWords = GameManager.Instance.GetSolvedBaseWordsForEra(GameManager.Instance.CurrentEra);
        Debug.Log($"[Language Debug] Found {solvedBaseWords.Count} solved base words for era {GameManager.Instance.CurrentEra}");
        
        // Add any solved base words to our solvedWords collection
        foreach (string baseWord in solvedBaseWords)
        {
            // Add the base word (English)
            if (!solvedWords.Contains(baseWord))
            {
                solvedWords.Add(baseWord);
                Debug.Log($"[Language Debug] Added base word {baseWord} to solvedWords");
            }
            
            // Add the current language translation
            string translatedWord = GameManager.Instance.GetTranslation(baseWord, GameManager.Instance.CurrentLanguage);
            if (!string.IsNullOrEmpty(translatedWord) && !solvedWords.Contains(translatedWord))
            {
                solvedWords.Add(translatedWord);
                Debug.Log($"[Language Debug] Added translated word {translatedWord} to solvedWords");
            }
        }
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(fact) && fact != "LOADING")
            {
                ...
                if (titleTransform != null)
                {
                    ...
                    {
                        Debug.LogError("[DidYouKnow Debug] Title component is null!");
                    }
                }
                else
                {
