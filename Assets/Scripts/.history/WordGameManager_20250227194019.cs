    ...
    private void HandleLanguageChanged()
    {
        ...
        if (Application.platform == RuntimePlatform.Android)
        {
            if (GridManager.Instance != null)
            {
                ...
                // Get the current language-specific word to check if it's solved
                if (currentBaseWord != null)
                {
                    string currentLangWord = GameManager.Instance.GetTranslation(currentBaseWord, GameManager.Instance.CurrentLanguage);
                    
                    // If the current word has been solved, mark it as solved in the grid
