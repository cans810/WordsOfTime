...
public class WordGameManager : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (scene.name == "GameScene")
        {
            ...
            HashSet<string> solvedWordsSet = GameManager.Instance.GetSolvedWordsForEra(eraKey);
            solvedWordsInCurrentEra = ConvertToWordIndices(solvedWordsSet);
            ...
            {
                solvedWordsSet = GameManager.Instance.GetSolvedWordsForEra(GameManager.Instance.CurrentEra);
                solvedWordsInCurrentEra = ConvertToWordIndices(solvedWordsSet);
            }
    ...
    // Add this helper method to convert HashSet<string> to HashSet<int>
    private HashSet<int> ConvertToWordIndices(HashSet<string> solvedWords)
    {
        HashSet<int> result = new HashSet<int>();
        
        if (solvedWords == null || currentEraWords == null)
            return result;
        
        // Convert each solved word to its index in the current era words list
        for (int i = 0; i < currentEraWords.Count; i++)
        {
            if (solvedWords.Contains(currentEraWords[i].ToUpper()))
            {
                result.Add(i);
            }
        }
