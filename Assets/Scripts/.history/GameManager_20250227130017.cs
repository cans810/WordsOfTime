...
public class GameManager : MonoBehaviour
{
    ...
    public void SetLanguage(string language)
    {
        ...
        {
            Debug.Log($"[GameManager] Changing language from {CurrentLanguage} to {language}");
            CurrentLanguage = language;
            
            // When changing language, we need to ensure solved words are properly tracked
            UpdateSolvedWordsForLanguage(language);
    ...
    private void UpdateSolvedWordsForLanguage(string language)
    {
        ...
        foreach (var eraPair in solvedBaseWordsPerEra)
        {
            ...
            foreach (string baseWordKey in solvedBaseWords)
            {
                ...
                string translatedWord = WordValidator.GetTranslation(baseWordKey, language, era);
                if (!string.IsNullOrEmpty(translatedWord))
                {
                    Debug.Log($"[GameManager] Adding translated word '{translatedWord}' for base word '{baseWordKey}' to solved words for {language}");
    ...
    // Modify your language change method to update solved words
