...
public class WordGameManager : MonoBehaviour
{
    ...
    {
        Debug.Log($"Starting points animation from {startPoints} to {endPoints}");

        // Ensure points text is initialized
        if (pointsText == null)
        {
        ...
        // Set initial value
        pointsText.text = startPoints.ToString();
        Debug.Log($"Initial points text set to: {startPoints}");
        ...
        while (elapsed < duration)
        {
            ...
            // Calculate interpolated points value
            int displayPoints = (int)Mathf.Lerp(startPoints, endPoints, t);
            
            // Update the points display
            pointsText.text = displayPoints.ToString();
            Debug.Log($"Updating points text to: {displayPoints}");
        ...
        Debug.Log($"Final points text set to: {endPoints}");
        
        // Clear the coroutine reference
        pointAnimationCoroutine = null;
        Debug.Log("Points animation completed");
    }
    ...
    public void HandleCorrectWord(List<LetterTile> guessedTiles)
    {
        ...
        StartCoroutine(SpawnCoins(8, guessedTiles));
    ...
    public void GiveHint()
    {
        ...
        if (GameManager.Instance.CanUseHint(hintToUse))
        {
            ...
            pointAnimationCoroutine = StartCoroutine(AnimatePointsIncrease(
                GameManager.Instance.CurrentPoints - GameManager.POINTS_PER_WORD, 
    ...
    public void CheckWord(string word, List<LetterTile> selectedTiles)
    {
        if (word == targetWord)
        {
            ...
            // Start animation with the new points value
            StartCoroutine(AnimatePoints(currentPoints, GameManager.Instance.CurrentPoints, true));
        }
        else
        {
    ...
    {
        int currentPoints = GameManager.Instance.CurrentPoints;
        GameManager.Instance.AddPoints(-amount); // Deduct points
        StartCoroutine(AnimatePoints(currentPoints, GameManager.Instance.CurrentPoints, false));
    }

    private IEnumerator AnimatePoints(int from, int to, bool isIncrease)
    {
        float duration = f; // Animation duration
        float elapsed = 0f;
        ...
        // Initial scale for bump effect
        Vector3 originalScale = pointsText.transform.localScale;
        
        // Ensure proper direction for decrease
        if (!isIncrease && from < to)
        {
            // Swap values if decrease animation is going up
            int temp = from;
            from = to;
            to = temp;
        }
        ...
        while (elapsed < duration)
        {
            ...
            int displayPoints = (int)Mathf.Lerp(from, to, t);
            ...
            pointsText.transform.localScale = originalScale * scale;
            
            yield return null;
        }
        
        // Ensure final value is set
        pointsText.text = to.ToString();
        pointsText.transform.localScale = originalScale;
        
        // Fade back to original color
        float fadeDuration = 0.2f;
        float fadeElapsed = 0f;
        Color originalColor = pointsText.color; // Store the current color
        
        while (fadeElapsed < fadeDuration)
        {
            fadeElapsed += Time.deltaTime;
            float t = Mathf.Clamp01(fadeElapsed / fadeDuration);
            pointsText.color = Color.Lerp(targetColor, Color.white, t);
            yield return null;
        }
