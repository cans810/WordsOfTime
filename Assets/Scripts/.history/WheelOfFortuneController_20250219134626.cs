...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private void UpdateSpinAvailability()
    {
        if (SaveManager.Instance != null)
        {
            ...
            Debug.Log($"Last spin time: {lastSpinTime}, Next available: {nextAvailableTime}, Can spin: {canSpin}");
        }
    ...
    private IEnumerator SpinWheel()
    {
        ...
        if (actualPrize != null)
        {
            ...
            {
                wheelAnimator.SetBool("Hide", true);
                // Wait for the animation to complete (assuming it's 1 second long)
                yield return new WaitForSeconds(1f);
            }
            else
            {
                Debug.LogWarning("No Animator component found on the wheel");
            }
