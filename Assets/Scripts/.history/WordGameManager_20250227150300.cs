...
public class WordGameManager : MonoBehaviour
{
    ...
    private void PreloadFactForWord(string word)
    {
        ...
        // First try to get the fact directly
        ...
        {
            Debug.Log($"[WordGameManager] Fact not available for '{word}', checking JSON files");
            WordValidator.CheckWordInJsonFiles(word, era);
        ...
        if (!string.IsNullOrEmpty(fact) && fact != "LOADING")
        {
            Debug.Log($"[WordGameManager] Successfully preloaded fact for '{word}'");
        }
        else
        {
            Debug.LogWarning($"[WordGameManager] Failed to preload fact for '{word}'");
        }
    }
    ...
    public void NextWord()
    {
        ...
        {
            Debug.LogWarning("[WordGameManager] No words available for the current era");
            return;
        }
        
        // Calculate the next index, wrapping around if necessary
        int nextIndex = (currentWordIndex + 1) % currentEraWords.Count;
        
        // Get the next word
        string nextWord = currentEraWords[nextIndex];
        
        // Check if the next word is already solved
        bool isNextWordSolved = GameManager.Instance.IsWordSolved(GameManager.Instance.GetBaseWord(nextWord));
        
        // If the next word is already solved, preload its fact
        if (isNextWordSolved)
        {
            Debug.Log($"[WordGameManager] Next word '{nextWord}' is already solved, preloading fact");
            PreloadFactForWord(nextWord);
        }
        
        // Load the next word
        LoadWord(nextIndex);
        
        // Reset the current word forming state
        ClearCurrentWord();
        
        // Reset the hint level for the new word
        hintLevel = 0;
        
        // Reset the newly solved flag
        isCurrentWordNewlySolved = false;
    }
    ...
    public void PreviousWord()
    {
        ...
        int prevIndex = (currentWordIndex - 1 + currentEraWords.Count) % currentEraWords.Count;
        
        // Get the previous word
        string prevWord = currentEraWords[prevIndex];
        
        // Check if the previous word is already solved
        bool isPrevWordSolved = GameManager.Instance.IsWordSolved(GameManager.Instance.GetBaseWord(prevWord));
        
        // If the previous word is already solved, preload its fact
        if (isPrevWordSolved)
        {
            Debug.Log($"[WordGameManager] Previous word '{prevWord}' is already solved, preloading fact");
            PreloadFactForWord(prevWord);
        }
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        try
        {
            ...
            Debug.Log("[DidYouKnow Debug] Set didYouKnowText content: " + (fact.Length > 30 ? fact.Substring(0, 30) + "..." : fact));
            
            
            // Check panel hierarchy and components
            Debug.Log("[DidYouKnow Debug] Panel hierarchy check:");
            for (int i = 0; i < didYouKnowPanel.transform.childCount; i++)
            {
                Transform child = didYouKnowPanel.transform.GetChild(i);
                Debug.Log($"[DidYouKnow Debug] Child {i}: {child.name}, Active: {child.gameObject.activeSelf}");
            }
            
            // Check if panel has a Canvas component
            Canvas panelCanvas = didYouKnowPanel.GetComponent<Canvas>();
            if (panelCanvas != null)
            {
                Debug.Log($"[DidYouKnow Debug] Panel has Canvas component. Enabled: {panelCanvas.enabled}, Sort Order: {panelCanvas.sortingOrder}");
            }
            
            // Check if panel has a CanvasGroup component
            ...
            if (canvasGroup != null)
            {
                ...
                // Ensure CanvasGroup settings don't hide the panel
