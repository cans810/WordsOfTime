...
public class EraSelectionManager : MonoBehaviour
{
    ...
    public void SelectEra(string era)
    {
        ...
        if (GameManager.Instance != null)
        {
            ...
            if (isUnlocked)
            {
                ...
                // If we're in the GameScene, immediately refresh the word game with the new era
                if (SceneManager.GetActiveScene().name == "GameScene")
                {
                    WordGameManager wordGameManager = FindObjectOfType<WordGameManager>();
                    if (wordGameManager != null)
                    {
                        // Trigger a refresh of the game content with the new era
                        wordGameManager.StartNewGameInEra();
                    }
                }
                
                // Immediately close the era selection screen by starting the animation
                // The animation will trigger OnAnimationComplete when finished, which will fully deactivate the screen
    ...
    public void OnReturnLanguageButtonClickedSetDeactive()
    {
        ...
        if (SceneManager.GetActiveScene().name == "GameScene")
        {
            ...
            if (wordGameManager != null)
            {
                // This ensures the game is using the newly selected era
                wordGameManager.StartNewGameInEra();
            }
        }
    }
