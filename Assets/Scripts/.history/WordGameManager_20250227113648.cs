...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        Debug.Log("[DidYouKnow Debug] Waiting for animations to complete...");
        yield return new WaitForSeconds(0.5f);
        Debug.Log("[DidYouKnow Debug] Animation wait completed");
        ...
        try
        {
            Debug.Log("[DidYouKnow Debug] Panel state before setting text: " + (didYouKnowPanel != null ? didYouKnowPanel.activeSelf.ToString() : "panel is null"));
            didYouKnowText.text = fact;
            Debug.Log("[DidYouKnow Debug] Set didYouKnowText content");
            
            // Position the panel in the safe area
            Debug.Log("[DidYouKnow Debug] Positioning panel in safe area");
            PositionPanelInSafeArea();
            
            // Activate the panel
            Debug.Log("[DidYouKnow Debug] Panel state before activation: " + (didYouKnowPanel != null ? didYouKnowPanel.activeSelf.ToString() : "panel is null"));
            didYouKnowPanel.SetActive(true);
            Debug.Log("[DidYouKnow Debug] Panel activated, active state: " + didYouKnowPanel.activeSelf);
            
            // After activating the panel
            didYouKnowPanel.SetActive(true);
            Debug.Log("[DidYouKnow Debug] Panel activated, active state: " + didYouKnowPanel.activeSelf);

            // Force the panel to be visible by bringing it to the front
            Canvas panelCanvas = didYouKnowPanel.GetComponent<Canvas>();
            if (panelCanvas != null)
            {
                panelCanvas.sortingOrder = 1000; // Very high sorting order to ensure visibility
                Debug.Log($"[DidYouKnow Debug] Forced panel canvas sorting order to 1000");
            }
            else
            {
                // If the panel doesn't have its own canvas, try to add one
                Debug.Log("[DidYouKnow Debug] Panel doesn't have its own Canvas, adding one");
                panelCanvas = didYouKnowPanel.AddComponent<Canvas>();
                panelCanvas.overrideSorting = true;
                panelCanvas.sortingOrder = 1000;
                
                // Add a GraphicRaycaster to handle input
                if (didYouKnowPanel.GetComponent<GraphicRaycaster>() == null)
                {
                    didYouKnowPanel.AddComponent<GraphicRaycaster>();
                }
            }

            // Check if the panel is visible in the hierarchy
            Debug.Log("[DidYouKnow Debug] Panel hierarchy path: " + GetGameObjectPath(didYouKnowPanel));
            Debug.Log("[DidYouKnow Debug] Panel parent active: " + (didYouKnowPanel.transform.parent != null ? didYouKnowPanel.transform.parent.gameObject.activeSelf.ToString() : "no parent"));
        ...
        // If we're using auto-close, set up a timer
        if (factDisplayTime > 0)
        {
            Debug.Log($"[DidYouKnow Debug] Auto-close timer started for {factDisplayTime} seconds");
            yield return new WaitForSeconds(factDisplayTime);
            
            try
            {
                // Check if the panel is still active (user might have closed it manually)
                if (didYouKnowPanel != null && didYouKnowPanel.activeSelf)
                {
                    Debug.Log("[DidYouKnow Debug] Auto-closing panel after timer");
                    didYouKnowPanel.SetActive(false);
                    EnableNavigationButtons();
                }
            }
            ...
            {
                Debug.LogError($"[DidYouKnow Debug] Error closing didYouKnow panel: {e.Message}");
            }
        }
    }
    ...
    private void PositionPanelInSafeArea()
    {
        ...
        try
        {
            // Check panel visibility before positioning
            Debug.Log($"[DidYouKnow Debug] Panel visibility before positioning - Active: {didYouKnowPanel.activeSelf}, Scale: {didYouKnowPanel.transform.localScale}");
            ...
            Debug.Log($"[DidYouKnow Debug] Canvas details - RenderMode: {canvas.renderMode}, ScaleFactor: {canvas.scaleFactor}, Enabled: {canvas.enabled}");
            ...
            Debug.Log($"[DidYouKnow Debug] Panel RectTransform before positioning - AnchoredPosition: {panelRectTransform.anchoredPosition}, Size: {panelRectTransform.sizeDelta}, Pivot: {panelRectTransform.pivot}");
            ...
            // Check if the panel is properly anchored
            Debug.Log($"[DidYouKnow Debug] Panel anchors - Min: {panelRectTransform.anchorMin}, Max: {panelRectTransform.anchorMax}");
            
            // Check if the panel has any layout groups that might affect its position
            LayoutGroup[] layoutGroups = didYouKnowPanel.GetComponents<LayoutGroup>();
            if (layoutGroups.Length > 0)
            {
                Debug.Log($"[DidYouKnow Debug] Panel has {layoutGroups.Length} layout groups:");
                foreach (var layoutGroup in layoutGroups)
                {
                    Debug.Log($"[DidYouKnow Debug]   - {layoutGroup.GetType().Name}, Enabled: {layoutGroup.enabled}");
                }
            }
            
            // Check if the panel has a Canvas Group that might affect its visibility
            CanvasGroup canvasGroup = didYouKnowPanel.GetComponent<CanvasGroup>();
            if (canvasGroup != null)
            {
                Debug.Log($"[DidYouKnow Debug] Panel has CanvasGroup - Alpha: {canvasGroup.alpha}, Interactable: {canvasGroup.interactable}, BlocksRaycasts: {canvasGroup.blocksRaycasts}");
                
                // Ensure the panel is visible
                if (canvasGroup.alpha < 1.0f)
                {
                    Debug.LogWarning($"[DidYouKnow Debug] Panel CanvasGroup alpha is low ({canvasGroup.alpha}), setting to 1.0");
                    canvasGroup.alpha = 1.0f;
                }
            }
            
            Debug.Log($"[DidYouKnow Debug] Panel RectTransform after positioning - AnchoredPosition: {panelRectTransform.anchoredPosition}, Size: {panelRectTransform.sizeDelta}");
        ...
        // After checking the CanvasGroup
        // Check if the panel is in the correct layer and sorting order
        Canvas panelCanvas = didYouKnowPanel.GetComponent<Canvas>();
        if (panelCanvas != null)
        {
            Debug.Log($"[DidYouKnow Debug] Panel has its own Canvas - SortingOrder: {panelCanvas.sortingOrder}, SortingLayerID: {panelCanvas.sortingLayerID}");
            
            // Ensure the panel's canvas has a high sorting order to appear on top
            if (panelCanvas.sortingOrder < 100)
            {
                Debug.LogWarning($"[DidYouKnow Debug] Panel Canvas sorting order is low ({panelCanvas.sortingOrder}), setting to 100");
                panelCanvas.sortingOrder = 100;
            }
        }
        else
        {
            // Check the parent canvas sorting order
            if (canvas != null)
            {
                Debug.Log($"[DidYouKnow Debug] Parent Canvas - SortingOrder: {canvas.sortingOrder}, SortingLayerID: {canvas.sortingLayerID}");
            }
        }

        // Check if there are any other active panels that might be overlapping
        Canvas[] allCanvases = FindObjectsOfType<Canvas>();
        if (allCanvases.Length > 1)
        {
            Debug.Log($"[DidYouKnow Debug] Found {allCanvases.Length} active canvases in the scene:");
            foreach (var c in allCanvases)
            {
                if (c != canvas && c != panelCanvas)
                {
                    Debug.Log($"[DidYouKnow Debug]   - Canvas: {c.name}, SortingOrder: {c.sortingOrder}, Active: {c.gameObject.activeSelf}");
                }
            }
        }
    }
    ...
    public void ShowDidYouKnowInstantly()
    {
        ...
        try
        {
            ...
            if (fact == "LOADING")
                ...
                Debug.LogWarning("[DidYouKnow Debug] Fact is empty or null");
            else
                Debug.Log($"[DidYouKnow Debug] Successfully got fact: {(fact.Length > 30 ? fact.Substring(0, 30) + "..." : fact)}");
            ...
            if (!string.IsNullOrEmpty(fact) && fact != "LOADING")
            {
                ...
                // Position the panel correctly for Android
