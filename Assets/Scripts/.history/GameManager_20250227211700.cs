...
public class GameManager : MonoBehaviour
{
    ...
    public void LoadGame()
    {
        ...
        if (SaveManager.Instance != null)
        {
            // First ensure SaveData is loaded from file
            if (File.Exists(SaveManager.Instance.SavePath))
            {
                try
                {
                    using (FileStream stream = new FileStream(SaveManager.Instance.SavePath, FileMode.Open))
                    {
                        BinaryFormatter formatter = new BinaryFormatter();
                        SaveManager.Instance.Data = (SaveData)formatter.Deserialize(stream);
                    }
                    Debug.Log("Successfully loaded SaveData from file in GameManager.LoadGame()");
                }
                catch (Exception e)
                {
                    Debug.LogError($"Error loading SaveData in GameManager.LoadGame(): {e.Message}");
            ...
            {
                Debug.Log("No save file found, initializing new SaveData");
                SaveManager.Instance.Data = new SaveData();
                return;
            }
