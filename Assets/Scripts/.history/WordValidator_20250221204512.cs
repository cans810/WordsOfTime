...
public static class WordValidator
{
    ...
    public static void LoadWordSets()
    {
        ...
        Debug.Log("Word sets loaded successfully");
    }
    ...
    public static string GetFactForWord(string word, string era, string language = "en")
    {
        ...
        {
            Debug.LogWarning($"No facts found for era: '{era}' in language: {language}. Available eras: {string.Join(", ", wordSetsWithFactsByLanguage[language].Keys)}");
            return string.Empty;
        }

        string wordKey = word.ToUpper();
        Debug.Log($"Available words in era '{era}': {string.Join(", ", wordSetsWithFactsByLanguage[language][era].Keys)}");
