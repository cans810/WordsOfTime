...
public class WordGameManager : MonoBehaviour
{
    ...
    private void UpdateHintButton()
    {
        if (hintButton != null)
        {
            ...
            // Update button state
            bool canUseHint = nextHintLevel > 0 && GameManager.Instance.CanUseHint(nextHintLevel);
            ...
            if (hintButtonText != null)
            {
                if (nextHintLevel == 0)
                {
                    hintButtonText.text = "No Hints Left";
                }
                else
                {
                    hintButtonText.text = $"Hint {nextHintLevel} ({hintCost})";
                }
            }
        }
    }
