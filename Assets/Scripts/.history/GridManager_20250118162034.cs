...
public class GridManager : MonoBehaviour
{
    ...
    {
        Debug.Log($"Trying to highlight letter: {letter}");
        ...
        foreach (Button button in buttons)
        {
            ...
            if (buttonText == null || buttonImage == null) continue;

            // Reset color first
            buttonImage.color = Color.white;

            // Check if this button has the letter we're looking for
            if (buttonText.text.Length > 0 && char.ToUpper(buttonText.text[0]) == char.ToUpper(letter))
            {
                Debug.Log($"Found matching letter on button: {buttonText.text}");
        ...
        if (!foundAny)
        {
            Debug.LogWarning($"No buttons found with letter: {letter}");
        }
    }
