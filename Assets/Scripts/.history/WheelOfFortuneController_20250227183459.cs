...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (canvas == null)
        {
            Debug.LogError("Canvas reference is missing in WheelOfFortuneController!");F    
            return;
        }
    ...
    private void GivePrizeToPlayer(Prize prize)
    {
        ...
        {
            case "1000 Points":
                // Add points immediately to ensure they're added even if animation fails
                Debug.Log("Added 1000 points directly to player total");
                StartCoroutine(SpawnCoins(15, 1000));
                break;
            case "500 Points":
                // Add points immediately to ensure they're added even if animation fails
                Debug.Log("Added 500 points directly to player total");
                StartCoroutine(SpawnCoins(10, 500));
                break;
            case "250 Points":
                // Add points immediately to ensure they're added even if animation fails
                Debug.Log("Added 250 points directly to player total");
    ...
    private IEnumerator AnimatePointsIncrease(int pointsToAdd)
    {
        ...
        Debug.Log($"Starting points animation: Current={startPoints}, Adding={pointsToAdd}, Target={targetPoints}");
        ...
        // Points are now added directly in GivePrizeToPlayer, so we don't need to add them again here
        // Just play the sound effect
        Debug.Log($"Points animation complete. Current total: {GameManager.Instance.CurrentPoints}");
        SoundManager.Instance.PlaySound("PointGain");
    }
    ...
    private IEnumerator SpawnCoins(int coinCount, int pointsToAdd)
    {
        ...
        Debug.Log($"SpawnCoins started: Count={coinCount}, PointsToAdd={pointsToAdd}");
        ...
        for (int i = 0; i < coinCount; i++)
        {
            ...
            StartCoroutine(MoveCoinToPanel(coin, () => {
                if (!pointsAnimationStarted)
                {
                    ...
                    Debug.Log("Starting points animation from coin callback");
                    // Points are already added in GivePrizeToPlayer, this just animates the UI
        ...
        // Ensure points animation starts even if coin animations fail
        yield return new WaitForSeconds(1.0f);
        if (!pointsAnimationStarted)
        {
            Debug.Log("Coin animation didn't trigger points animation. Starting UI animation directly.");
            pointsAnimationStarted = true;
            // Points are already added in GivePrizeToPlayer, this just animates the UI
            StartCoroutine(AnimatePointsIncrease(pointsToAdd));
        }
    ...
    private IEnumerator MoveCoinToPanel(GameObject coin, System.Action onDestroy)
    {
        ...
        while (elapsedTime < randomDuration)
        {
            ...
            yield return null; // Removed the additional delay
        }
    ...
    private IEnumerator SpinWheel()
    {
        ...
        if (actualPrize != null)
        {
            ...
            // For point rewards, wait an additional 1.5 seconds after coins are gone
            if (actualPrize.name == "1000 Points" || actualPrize.name == "500 Points" || actualPrize.name == "250 Points")
            {
                Debug.Log("Waiting additional 1.5 seconds after point animation");
