    ...
    private void HandleLanguageChanged()
    {
        ...
        for (int i = 0; i < gridSize; i++)
        {
            for (int j = 0; j < gridSize; j++)
            {
                ...
                GridManager.Instance.grid[i, j].GetComponent<Image>().raycastTarget = true; // Make sure tiles are clickable
            }
        }
        ...
        Debug.Log($"Language changed: Base word: {baseWord}, New translated word: {newWord}");
        
        // Update the target word to the translated version
        targetWord = newWord;
        
        // 3. Update the grid with the new word
        if (GridManager.Instance != null)
        {
            // Setup a new puzzle with the translated word
            GridManager.Instance.SetupNewPuzzle(targetWord);
            
            // If the word is solved, highlight it in the grid
            if (GameManager.Instance.IsWordSolved(baseWord))
            {
                Debug.Log($"Word {baseWord} is solved, finding {targetWord} in grid to highlight");
                HighlightWordInGrid(targetWord);
        ...
        // 4. Update sentence with the new language
        string sentence = WordValidator.GetSentenceForWord(targetWord, GameManager.Instance.CurrentEra);
        ...
        // 6. Update hint button to show solved status in the current language
        UpdateHintButton();
    }
