            ...
            if (Data.shuffledWords != null && Data.shuffledWords.Count > 0)
            {
                ...
                foreach (var kvp in Data.shuffledWords)
                {
                    ...
                    if (parts.Length == 2)
                    {
                        ...
                        {
                            GameManager.Instance.eraWordsPerLanguage[language] = new Dictionary<string, List<string>>();
                        }
                        
                        if (!GameManager.Instance.eraWordsPerLanguage[language].ContainsKey(era))
                        {
                            GameManager.Instance.eraWordsPerLanguage[language][era] = new List<string>();
                        }
