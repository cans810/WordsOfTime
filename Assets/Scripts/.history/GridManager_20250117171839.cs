...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        if (allGrids.ContainsKey(word))
        {
            ...
            ConvertDictionaryToGrid(allGrids[word]);
            currentWord = word;
            return;
        }
        ...
        Dictionary<Vector2Int, LetterTile> newTiles = new Dictionary<Vector2Int, LetterTile>();
    ...
    private void ConvertDictionaryToGrid(Dictionary<Vector2Int, LetterTile> tiles)
    {
        grid = new LetterTile[gridSize, gridSize];
        foreach (var kvp in tiles)
        {
            grid[kvp.Key.x, kvp.Key.y] = kvp.Value;
        }
    }
