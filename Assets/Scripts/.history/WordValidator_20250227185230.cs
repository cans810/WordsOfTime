...
public static class WordValidator
{
    ...
    private static IEnumerator LoadWordsForLanguageAndroid(string language)
    {
        ...
        // First attempt: Try using UnityWebRequest
            ...
            if (webRequest.result == UnityWebRequest.Result.Success)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        // If we successfully loaded this language, update the loading state
                        if (language == "tr") // Assume tr is loaded last
                        {
                            isLoading = false;
                            Debug.Log("[Android Debug] All word loading completed successfully");
                        }
        ...
        // Second attempt: Try using a different URL format for Android
        string androidPath = filePath;
        if (!androidPath.StartsWith("file://"))
        {
            androidPath = "file://" + androidPath;
        }
        
        Debug.Log($"[Android Debug] Trying alternative URL format: {androidPath}");
        ...
        using (UnityWebRequest webRequest = UnityWebRequest.Get(androidPath))
        {
            ...
            {
                string jsonContent = webRequest.downloadHandler.text;
                Debug.Log($"[Android Debug] Successfully downloaded from alternative URL. Content length: {jsonContent.Length}");
                ...
                if (!string.IsNullOrEmpty(jsonContent))
                {
                    try
                    {
                        ...
                        {
                            isLoading = false;
                            Debug.Log("[Android Debug] All word loading completed successfully (alternative URL)");
                        }
                    ...
                    {
                        Debug.LogError($"[Android Debug] Error processing JSON from alternative URL: {e.Message}\n{e.StackTrace}");
                    }
                }
            }
            else
            {
                Debug.LogError($"[Android Debug] Failed to download from alternative URL: {webRequest.error}");
            }
        }
        
        // Third attempt: Try using the Application.persistentDataPath as a fallback
        string fallbackPath = System.IO.Path.Combine(Application.persistentDataPath, fileName);
        
        Debug.Log($"[Android Debug] Trying fallback path: {fallbackPath}");
        ...
        if (System.IO.File.Exists(fallbackPath))
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(jsonContent))
                {
                    Debug.Log($"[Android Debug] Successfully read fallback file. Content length: {jsonContent.Length}");
                    ProcessWordJson(jsonContent, language);
                    LogSampleData(language);
                }
                else
                {
                    Debug.LogError($"[Android Debug] Fallback file content is empty for {language}");
                }
            }
            catch (System.Exception e)
            {
                Debug.LogError($"[Android Debug] Error reading fallback file: {e.Message}\n{e.StackTrace}");
            }
        }
        else
        {
            ...
            // Final attempt: Try to copy the file from the APK to persistent data path
            Debug.Log($"[Android Debug] Attempting to extract {fileName} from APK...");
            
            // This is a last resort - we'll try to use the Android Java classes to access the file
            try
            {
                    ...
                    using (AndroidJavaObject assetManager = context.Call<AndroidJavaObject>("getAssets"))
                    {
                        try
                        {
                            using (AndroidJavaObject inputStream = assetManager.Call<AndroidJavaObject>("open", fileName))
                            {
                                ...
                                // Convert to string
                                string jsonContent = System.Text.Encoding.UTF8.GetString(buffer);
                                
                                if (!string.IsNullOrEmpty(jsonContent))
                                {
                                    Debug.Log($"[Android Debug] Successfully extracted file from APK. Content length: {jsonContent.Length}");
                                    ProcessWordJson(jsonContent, language);
                                    LogSampleData(language);
                                    
                                    // Save to persistent data path for future use
                                    System.IO.File.WriteAllText(fallbackPath, jsonContent);
                                    Debug.Log($"[Android Debug] Saved extracted file to: {fallbackPath}");
                                }
                            }
                        }
                        catch (System.Exception e)
                        {
                            Debug.LogError($"[Android Debug] Error extracting file from APK: {e.Message}\n{e.StackTrace}");
                        }
                    }
                }
            }
            ...
            {
                Debug.LogError($"[Android Debug] Error accessing Android Java classes: {e.Message}\n{e.StackTrace}");
            }
        }
        
        // Update loading state regardless of success
        if (language == "tr")
        {
            isLoading = false;
            Debug.Log("[Android Debug] All word loading attempts completed");
        }
    }
