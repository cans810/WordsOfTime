...
public class WordGameManager : MonoBehaviour
{
    ...
    public void GiveHint()
    {
        ...
        if (!GameManager.Instance.HasUsedHint(targetWord, hintToUse) && 
            GameManager.Instance.CanUseHint(hintToUse))
        {
            // Apply the hint
            if (hintToUse == 1)
            {
                // First hint: Highlight first letter
                GridManager.Instance.HighlightFirstLetter(targetWord[0]);
            }
            else
            {
                // Second hint: Show word length
                ShowWordLength();
            }
    ...
    private void UpdateHintButton()
    {
        if (hintButton != null)
        {
            ...
            int nextHintLevel = hasUsedHint1 ? (hasUsedHint2 ? 0 : 2) : 1;
            
            bool canUseNextHint = nextHintLevel > 0 && GameManager.Instance.CanUseHint(nextHintLevel);
            
            // Update hint button interactability directly
            hintButton.interactable = canUseNextHint;
            
            // Update hint button text if available
            if (hintButtonText != null)
            {
                int cost = nextHintLevel == 1 ? GameManager.HINT_COST : GameManager.SECOND_HINT_COST;
                hintButtonText.text = $"Hint ({cost})";
            }
        }
        else
        {
            Debug.LogWarning("Hint button is null!");
        }
    }
