...
public class GridManager : MonoBehaviour
{
    ...
    private void Start()
    {
        if (buttons.Count == 0)
        {
            ...
            Debug.Log($"Found {buttons.Count} buttons in grid");
        }
    }
    ...
    {
        Debug.Log($"HighlightFirstLetter called for letter: {letter}");
        ...
        // Find and highlight all buttons with the matching letter
        foreach (Button button in buttons)
        {
            if (button != null)
            {
                ...
                if (buttonText != null && buttonText.text.Length > 0 && char.ToUpper(buttonText.text[0]) == char.ToUpper(letter))
                {
                    Debug.Log($"Found matching letter in button: {buttonText.text}");
                    
                    Image buttonImage = button.GetComponent<Image>();
                    if (buttonImage != null)
                    {
                        buttonImage.color = new Color(1f, 1f, 0f, 1f); // Bright yellow
                        StartCoroutine(PulseHighlight(buttonImage));
                    }
                }
            }
        }
    ...
    private IEnumerator PulseHighlight(Image buttonImage)
    {
        float duration = 2f;
        float elapsed = 0f;
        Color highlightColor = new Color(1f, 1f, 0f, 1f); // Bright yellow
