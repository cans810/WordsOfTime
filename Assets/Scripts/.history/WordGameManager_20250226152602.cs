...
public class WordGameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Make sure didYouKnowPanel is assigned
        if (didYouKnowPanel == null)
        {
            Debug.LogWarning("[Android Debug] didYouKnowPanel not assigned in inspector, trying to find it in scene");
            didYouKnowPanel = GameObject.Find("DidYouKnowPanel");
            
            if (didYouKnowPanel == null)
            {
                Debug.LogError("[Android Debug] Could not find DidYouKnowPanel in scene! Did You Know functionality will not work.");
            }
            else
            {
                Debug.Log("[Android Debug] Found DidYouKnowPanel in scene");
                
                // Also find the text component if needed
    ...
    public void LoadWord(int index)
    {
        ...
        if (isSolved)
        {
            Debug.Log($"Word {currentBaseWord} is already solved, showing Did You Know fact for {targetWord} instantly");
    ...
    {
        Debug.Log($"[Android Debug] ShowDidYouKnow called for word: {targetWord} in language: {GameManager.Instance.CurrentLanguage}");
        ...
        string fact = WordValidator.GetFactForWord(targetWord, GameManager.Instance.CurrentEra);
    ...
    public void ShowDidYouKnowInstantly()
    {
        ...
        // Add more detailed logging about the panel
        Debug.Log($"[Android Debug] didYouKnowPanel reference check: {(didYouKnowPanel != null ? "exists" : "null")}");
        if (didYouKnowPanel != null)
        {
            Debug.Log($"[Android Debug] didYouKnowPanel active state before: {didYouKnowPanel.activeSelf}");
        }
        ...
        if (didYouKnowPanel == null)
        {
            ...
            // Try to find the panel in the scene if it's not assigned
            didYouKnowPanel = GameObject.Find("DidYouKnowPanel");
            Debug.Log($"[Android Debug] Attempted to find DidYouKnowPanel: {(didYouKnowPanel != null ? "found" : "not found")}");
        ...
        string fact = WordValidator.GetFactForWord(targetWord, GameManager.Instance.CurrentEra);
        Debug.Log($"[Android Debug] Fact retrieved: '{fact}'");
        
        if (string.IsNullOrEmpty(fact) || fact == "LOADING")
        {
            Debug.Log("[Android Debug] Facts are still loading, will retry in 1 second");
            Invoke(nameof(ShowDidYouKnowInstantly), 0f);
            return;
        }
        ...
        // Check if didYouKnowText is assigned
        if (didYouKnowText == null)
        {
            Debug.LogError("[Android Debug] didYouKnowText is not assigned!");
            didYouKnowText = didYouKnowPanel.GetComponentInChildren<TextMeshProUGUI>();
            Debug.Log($"[Android Debug] Attempted to find TextMeshProUGUI in panel: {(didYouKnowText != null ? "found" : "not found")}");
        }
        ...
        {
            Debug.LogError("[Android Debug] Cannot set text - didYouKnowText is null!");
        }
        
        // Log the panel's state after attempting to show it
        Debug.Log($"[Android Debug] didYouKnowPanel active state after: {didYouKnowPanel.activeSelf}");
        
        // Check if there's a close button and ensure it's set up correctly
        Button closeButton = didYouKnowPanel.GetComponentInChildren<Button>();
        if (closeButton != null)
        {
            Debug.Log("[Android Debug] Close button found on Did You Know panel");
            
            // Remove any existing listeners to avoid duplicates
            closeButton.onClick.RemoveAllListeners();
            
            // Add the close button listener
            closeButton.onClick.AddListener(OnCloseDidYouKnowPanel);
            Debug.Log("[Android Debug] Close button listener added");
        }
        else
        {
            Debug.LogError("[Android Debug] No close button found on Did You Know panel!");
        }
    ...
    // Add this method for testing the Did You Know panel
    public void TestDidYouKnowPanel()
    {
        Debug.Log("[Android Debug] Testing Did You Know panel");
        
        // Check if panel exists
        if (didYouKnowPanel == null)
        {
            Debug.LogError("[Android Debug] didYouKnowPanel is not assigned for testing!");
            didYouKnowPanel = GameObject.Find("DidYouKnowPanel");
            
            if (didYouKnowPanel == null)
            {
                Debug.LogError("[Android Debug] Could not find DidYouKnowPanel in scene!");
                return;
            }
        }
        ...
        if (didYouKnowText == null)
        {
            Debug.LogError("[Android Debug] didYouKnowText is not assigned for testing!");
            didYouKnowText = didYouKnowPanel.GetComponentInChildren<TextMeshProUGUI>();
        ...
        // Set test text
        didYouKnowText.text = "Did You Know?\n\nThis is a test message to verify that the Did You Know panel is working correctly.";
        ...
        if (closeButton != null)
        {
            closeButton.onClick.RemoveAllListeners();
            closeButton.onClick.AddListener(OnCloseDidYouKnowPanel);
            Debug.Log("[Android Debug] Close button set up for test panel");
        }
        else
        {
            Debug.LogError("[Android Debug] No close button found on test panel!");
        }
