...
public class WordGameManager : MonoBehaviour
{
    ...
    private void UpdateHintButton()
    {
        if (hintButton != null)
        {
            ...
            if (GameManager.Instance.IsWordSolved(baseWord))
            {
                ...
                if (hintText != null)
                {
                    hintText.text = GameManager.Instance.CurrentLanguage == "tr" ? "Çözüldü" : "Solved";
                    hintText.color = Color.black;
                }
                
                // Hide the hint text when the hint button already shows "Solved"
                if (hintText != null)
                {
            ...
            // Show the solved status text for words that are solved in the current era
            if (hintText != null)
            {
                bool isSolvedInCurrentEra = GameManager.Instance.GetSolvedBaseWordsForEra(GameManager.Instance.CurrentEra).Contains(baseWord);
                hintText.gameObject.SetActive(isSolvedInCurrentEra);
                if (isSolvedInCurrentEra)
                {
                    hintText.text = GameManager.Instance.CurrentLanguage == "tr" ? "Çözüldü" : "Solved";
                ...
                {
                    // Hide the hint text when the word is not solved in the current era
                    hintText.gameObject.SetActive(false);
                }
            }
            ...
            // Check if player can afford the hinth
    ...
    private void HandleLanguageChanged()
    {
        ...
        // 4. Update hint button to show solved status in the current language
        UpdateHintButton();
    }
