...
public class GameManager : MonoBehaviour
{
    ...
    private void LoadLanguageWords(string filePath, string language)
    {
        try 
        {
            ...
            {
                // Map Turkish era names to English ones if needed
                string era = language == "tr" ? MapTurkishEraName(set.era) : set.era;
                
                if (!eraWordsPerLanguage[language].ContainsKey(era))
                {
                    eraWordsPerLanguage[language][era] = new List<string>();
                    wordSentencesPerLanguage[language][era] = new Dictionary<string, List<string>>();
                }

                foreach (var wordEntry in set.words)
                {
                    string word = wordEntry.word.ToUpper();
                    eraWordsPerLanguage[language][era].Add(word);
                    wordSentencesPerLanguage[language][era][word] = new List<string>(wordEntry.sentences);
                }
            }
            Debug.Log($"Loaded {language} words for {eraWordsPerLanguage[language].Count} eras");
        }
        catch (System.Exception e)
        {
    ...
    private string MapTurkishEraName(string turkishEra)
    {
        Dictionary<string, string> eraMapping = new Dictionary<string, string>
        {
            ...
            // Add other era mappings as needed
        };

        return eraMapping.ContainsKey(turkishEra) ? eraMapping[turkishEra] : turkishEra;
    }
    ...
    public List<string> GetSentencesForWord(string word)
    {
        if (wordSentencesPerLanguage.ContainsKey(currentLanguage) && 
            ...
            wordSentencesPerLanguage[currentLanguage][currentEra].ContainsKey(word))
        {
            return wordSentencesPerLanguage[currentLanguage][currentEra][word];
        }
        return new List<string>();
    }
