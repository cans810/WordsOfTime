...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (pointsText == null)
        {
            Debug.LogError("pointsText reference is missing in WheelOfFortuneController! Points animation will not work.");
            // Try to find it in the scene
            pointsText = pointPanel.GetComponentInChildren<TextMeshProUGUI>();
            if (pointsText != null)
            {
                Debug.Log("Found pointsText reference automatically.");
            }
        }
    ...
    {
        // Show prize panel for all prize types
        prizetext.gameObject.SetActive(true);
        prizeWonPanel.gameObject.SetActive(true);
        
        // Display appropriate text based on prize type
        if (prize.name == "1000 Points" || prize.name == "500 Points" || prize.name == "250 Points")
        {
            prizetext.text = $"You won {prize.prizeName}!";
        }
        else if (prize.name == "Random Era Unlocked" || prize.name == "Try Again")
        {
