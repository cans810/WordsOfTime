...
public class GridManager : MonoBehaviour
{
    ...
    private IEnumerator GenerateGridCoroutine(List<char> letters)
    {
        ...
        const float SPAWN_DELAY = 0.05f; // Shorter delay for a smoother animation
        int index = 0;
        ...
        // Instantiate tiles from top-left to bottom-right
        for (int y = 0; y < gridSize; y++)
        {
            for (int x = 0; x < gridSize; x++)
            {
                if (index < letters.Count)
                {
                    ...
                    // Start with zero scale and animate to full size
                    ...
                    index++;
                    yield return new WaitForSeconds(SPAWN_DELAY); // Shorter delay between each tile spawn
                }
            }
        }
    }
    ...
    {
        float elapsedTime = 0;
        float duration = 0.3f; // Shorter duration for a snappier animation
