...
public class WordGameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Initialize WordValidator with this MonoBehaviour for coroutines
        WordValidator.Initialize(this);
        ...
        // Add ScreenFitter to didYouKnowPanel if it doesn't have one
        if (didYouKnowPanel != null && didYouKnowPanel.GetComponent<ScreenFitter>() == null)
        {
            ...
            ScreenFitter fitter = didYouKnowPanel.AddComponent<ScreenFitter>();
            fitter.scaleMultiplier = 1.0f; // Use a safe default value
        }
    ...
    private void ShowAd()
    {
        ...
        if (AdManager.Instance != null)
        {
            ...
            // Unload the ad after showing
            AdManager.Instance.UnloadInterstitialAd();
    ...
    private IEnumerator ShowDidYouKnowCoroutine(string fact)
    {
        ...
        // Ensure panel is within safe area on Android
        #if UNITY_ANDROID && !UNITY_EDITOR
        RectTransform panelRect = didYouKnowPanel.GetComponent<RectTransform>();
        if (panelRect != null)
        {
            Debug.Log("[Android Debug] Adjusting panel position for Android safe area");
            // Get safe area
            ...
            if (canvas != null)
            {
                ...
                // Position panel at center of safe area
                panelRect.anchoredPosition = new Vector2(
                    (safeCenter.x - 0.5f) * canvas.GetComponent<RectTransform>().sizeDelta.x,
                    (safeCenter.y - 0.5f) * canvas.GetComponent<RectTransform>().sizeDelta.y
                );
                
                Debug.Log($"[Android Debug] Panel positioned at {panelRect.anchoredPosition} within safe area");
            }
        }
        #endif
        ...
        // Force a canvas update to ensure the panel is rendered
        Canvas.ForceUpdateCanvases();
        
        // Keep the panel visible for a minimum amount of time
        float visibilityTime = 0f;
        float minVisibilityTime = 5f; // Minimum time in seconds
        
        while (visibilityTime < minVisibilityTime)
        {
            visibilityTime += Time.deltaTime;
            
            // Check every second if the panel is still active
            if (visibilityTime % 1f < Time.deltaTime)
            {
                Debug.Log($"[Android Debug] Panel visibility time: {visibilityTime}/{minVisibilityTime}");
                
                // If panel was deactivated, reactivate it
                if (!didYouKnowPanel.activeSelf)
                {
                    Debug.Log("[Android Debug] Panel was deactivated, reactivating");
                    didYouKnowPanel.SetActive(true);
                }
            }
        ...
        Debug.Log("[Android Debug] Minimum visibility time reached");
        
        // Reset animation flag
        isAnimationPlaying = false;
        
        // Note: We don't automatically hide the panel anymore
