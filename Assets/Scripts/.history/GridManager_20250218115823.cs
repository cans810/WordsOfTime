...
public class GridManager : MonoBehaviour
{
    ...
    private IEnumerator GenerateGridCoroutine(List<char> letters)
    {
        ...
        const float SPAWN_DELAY = 0.1f; // Delay between each tile spawn
        const float WAVE_DELAY = 0.5f; // Delay between each wave
        int index = 0;
        ...
        for (int sum = 0; sum < gridSize * 2 - 1; sum++)
        {
            ...
            foreach (Vector2Int pos in currentWavePositions)
            {
                if (index < letters.Count)
                {
                    ...
                    // Start the scale animation and wait for it to complete
                    letterTileObj.transform.localScale = Vector3.zero;
                    yield return AnimateScale(letterTileObj); // Wait for the animation to finish
            ...
            yield return new WaitForSeconds(WAVE_DELAY); // Delay between each wave
        }
    }
