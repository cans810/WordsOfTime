...
public class GameManager : MonoBehaviour
{
    ...
    public bool IsEraUnlocked(string eraName)
    {
        // Ancient Egypt is always unlocked
        if (eraName == "Ancient Egypt" || eraName == "Medieval Europe") return true;
        
        // Check if player has enough points to unlock this era
        return CurrentPoints >= eraPrices[eraName];
    }

    public void UnlockEra(string eraName)
    {
        // Set the points requirement to 0 for this era
        if (eraPrices.ContainsKey(eraName))
        {
            eraPrices[eraName] = 0;
            Debug.Log($"Era unlocked: {eraName}");
            
            // Save the updated era prices
            SaveEraPrices();
        }
    }

    private void SaveEraPrices()
    {
        // Convert dictionary to JSON
        string json = JsonUtility.ToJson(new SerializableDictionary<string, int>(eraPrices));
        PlayerPrefs.SetString("EraPrices", json);
        PlayerPrefs.Save();
    }
