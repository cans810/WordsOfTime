...
public static class WordValidator
{
    ...
    public static string GetBaseWordForTranslation(string word, string language, string era)
    {
        Debug.Log($"[WordValidator] Looking for base word of '{word}' in language '{language}' for era '{era}'");
        
        if (!loadedWords.ContainsKey(language))
        {
            Debug.LogWarning($"[WordValidator] Language '{language}' not found in loadedWords");
            return word;
        }
        ...
        if (!loadedWords[language].ContainsKey(normalizedEra))
        {
            ...
            {
                Debug.LogWarning($"[WordValidator] Era '{normalizedEra}' not found in language '{language}'");
                return word;
            }
        }
        
        string wordKey = word.ToLower();
        if (!loadedWords[language][normalizedEra].ContainsKey(wordKey))
        {
            // Try case-insensitive matching
            var possibleMatches = loadedWords[language][normalizedEra].Keys.Where(k => k.Equals(wordKey, StringComparison.OrdinalIgnoreCase)).ToList();
            if (possibleMatches.Any())
            {
                wordKey = possibleMatches.First();
            }
            ...
            {
                Debug.LogWarning($"[WordValidator] Word '{wordKey}' not found in era '{normalizedEra}' for language '{language}'");
                return word;
            }
        }
        
        // Get the word object
        Word wordObj = loadedWords[language][normalizedEra][wordKey];
        
        // Return the English version as the base word
        if (wordObj.translations != null && !string.IsNullOrEmpty(wordObj.translations.en))
        {
            Debug.Log($"[WordValidator] Found base word: '{wordObj.translations.en}' for '{word}'");
            return wordObj.translations.en.ToUpper();
        }
