...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator AnimatePointsIncrease(int startPoints, int endPoints)
    {
        ...
        Debug.Log($"Text component found: {pointsText.gameObject.name}");
    ...
    {
        Debug.Log($"CheckWord called with word: {word}");
        
        if (word == targetWord)
        {
            Debug.Log("Word is correct, starting point handling");
            
            // Store points before adding
            int startPoints = GameManager.Instance.CurrentPoints;
            Debug.Log($"Starting points: {startPoints}");

            // Add points through GameManager
            GameManager.Instance.AddPoints(GameManager.POINTS_PER_WORD);
            Debug.Log("Points added through GameManager");

            // Get points after adding
            int endPoints = GameManager.Instance.CurrentPoints;
            Debug.Log($"Ending points: {endPoints}");

            // Start the animation with the correct values
            if (pointAnimationCoroutine != null)
            {
                Debug.Log("Stopping existing points animation");
                StopCoroutine(pointAnimationCoroutine);
            }
            
            Debug.Log("Starting new points animation");
            if (this.isActiveAndEnabled)
            {
                pointAnimationCoroutine = StartCoroutine(AnimatePointsIncrease(startPoints, endPoints));
                Debug.Log("Coroutine started successfully");
            }
            else
            {
                Debug.LogError("Cannot start coroutine - MonoBehaviour is not active and enabled");
            }
