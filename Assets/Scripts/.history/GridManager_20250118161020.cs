...
public class GridManager : MonoBehaviour
{
    ...
    private void Start()
    {
        // Initialize buttons list if it's empty
        if (buttons.Count == 0)
        {
    ...
    {
        if (buttons == null || currentSelectedButtons == null)
        {
            Debug.LogError("Button lists not initialized!");
            return;
        }
        ...
        if (currentSelectedButtons.Count > 0)
        {
            foreach (Button button in currentSelectedButtons)
            {
                if (button != null)
                {
                    TextMeshProUGUI buttonText = button.GetComponentInChildren<TextMeshProUGUI>();
                    
                    if (buttonText != null && buttonText.text.Length > 0 && buttonText.text[0] == letter)
                    {
        ...
        {
            Debug.LogWarning("No buttons currently selected");
        }
    }
