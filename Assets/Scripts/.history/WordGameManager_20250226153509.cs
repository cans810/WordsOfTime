...
public class WordGameManager : MonoBehaviour
{
    ...
    public void ShowDidYouKnowInstantly()
    {
        ...
        // Get the appropriate era name based on the current language
        string currentEra = GameManager.Instance.CurrentEra;
        string language = GameManager.Instance.CurrentLanguage;
        
        // If language is Turkish, we need to translate the era name to Turkish
        // since GetFactForWord expects Turkish era names when language is Turkish
        if (language == "tr")
        {
            // Translate English era name to Turkish
            switch (currentEra)
            {
                case "Ancient Egypt":
                    currentEra = "Antik Mısır";
                    break;
                case "Medieval Europe":
                    currentEra = "Orta Çağ Avrupası";
                    break;
                case "Renaissance":
                    currentEra = "Rönesans";
                    break;
                case "Industrial Revolution":
                    currentEra = "Sanayi Devrimi";
                    break;
                case "Ancient Greece":
                    currentEra = "Antik Yunan";
                    break;
                case "Viking Age":
                    currentEra = "Viking Çağı";
                    break;
                case "Feudal Japan":
                    currentEra = "Feodal Japonya";
                    break;
                case "Ottoman Empire":
                    currentEra = "Osmanlı İmparatorluğu";
                    break;
            }
            Debug.Log($"[Android Debug] Translated era name from '{GameManager.Instance.CurrentEra}' to '{currentEra}' for Turkish");
        }
        
        string fact = WordValidator.GetFactForWord(targetWord, currentEra, language);
