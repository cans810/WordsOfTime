...
public class WordGameManager : MonoBehaviour
{
    ...
    private const float POINT_ANIMATION_DURATION = 2f; // Even faster overall animation
    ...
    private void CreateProgressBar()
    {
        ...
        // Calculate size based on container width and number of words
        RectTransform containerRect = progressBarContainer.GetComponent<RectTransform>();
        float containerWidth = containerRect.rect.width;
        float maxWidth = Mathf.Min(50f, containerWidth / wordCountInEra); // Max width of 50px per bar
        float spacing = 5f; // Spacing between bars

        // Calculate starting position to center the bars
        float totalWidth = (wordCountInEra * maxWidth) + ((wordCountInEra - 1) * spacing);
        float startX = -totalWidth / 2 + maxWidth / 2;
        ...
        for (int i = 0; i < wordCountInEra; i++)
        {
            ...
            RectTransform rect = progressImage.GetComponent<RectTransform>();
            if (rect != null)
            {
                // Set base size
                rect.sizeDelta = new Vector2(maxWidth, rect.sizeDelta.y); // Keep original height
                
                // Position the bar
                float xPos = startX + i * (maxWidth + spacing);
            ...
            // Set initial color
            Image image = progressImage.GetComponent<Image>();
            if (image != null)
            {
    ...
    private void UpdateProgressBar()
    {
        ...
        for (int i = 0; i < progressImages.Count; i++)
        {
            if (i < eraWords.Count)
            {
                string word = eraWords[i];
                string baseWord = GameManager.Instance.GetBaseWord(word);
                
                // Check if the word is solved in any language
                bool isSolved = GameManager.Instance.GetAllSolvedBaseWords().Contains(baseWord);
                
                // Update the progress image color
                Image progressImage = progressImages[i].GetComponent<Image>();
                if (progressImage != null)
                {
                    progressImage.color = isSolved ? Color.green : Color.white;
                }
            }
        }
    }
    ...
    private IEnumerator AnimatePointsIncrease(int pointsToAdd)
    {
        ...
        int targetPoints = startPoints + pointsToAdd; // Points are only added once here
        ...
        // Remove the redundant points addition since GameManager.AddPoints is called elsewhere
    ...
    {
        string baseWord = GameManager.Instance.GetBaseWord(word);
        return GameManager.Instance.GetAllSolvedBaseWords().Contains(baseWord);
    }
    ...
    {
        if (hintButton == null || hintButtonText == null) return;

        // Check if word is solved in any language
        bool isSolved = IsWordSolved(targetWord);
        
        hintButton.interactable = !isSolved && GameManager.Instance.CurrentPoints >= GetHintCost();
        hintButtonText.text = $"Hint ({GetHintCost()})";
    }
    ...
    {
        Debug.Log($"Showing Did You Know for word: {targetWord} in language: {GameManager.Instance.CurrentLanguage}");
        
        // Get the fact directly for the current word in the current language
        string fact = WordValidator.GetFactForWord(targetWord, GameManager.Instance.CurrentEra, GameManager.Instance.CurrentLanguage);
        Debug.Log($"Retrieved fact: {fact}");
        ...
        if (!string.IsNullOrEmpty(fact))
        {
            ...
            if (didYouKnowPanel != null)
            {
                ...
                {
                    Debug.LogError("didYouKnowText is null!");
                }
            }
            else
            {
                Debug.LogError("didYouKnowPanel is null!");
            }
        }
        else
        {
            Debug.LogWarning($"No fact found for word: {targetWord} in language: {GameManager.Instance.CurrentLanguage}");
        }
    }
    ...
    {
        // Get the current hint level for this word
        int hintLevel = GameManager.Instance.GetHintLevel(targetWord);
        
        // Return the appropriate cost based on hint level
        return hintLevel switch
        {
            0 => GameManager.HINT_COST,        // First hint
            1 => GameManager.SECOND_HINT_COST, // Second hint
            _ => GameManager.HINT_COST         // Default to first hint cost
        };
    }
}