...
public class WheelOfFortuneController : MonoBehaviour
{
    ...
    private void ShowPrizeWon(Prize prize)
    {
        if (prize == null)
        {
            Debug.LogError("ShowPrizeWon: Prize is null");
            return;
        }

        try
        {
            // Always show the prize panel and text for any prize
            prizetext.gameObject.SetActive(true);
            prizeWonPanel.gameObject.SetActive(true);
            
            // Set the prize text based on the prize name
            switch (prize.name)
            {
                case "1000 Points":
                ...
                case "Try Again":
                default:
                    // Use the prize's prizeName property for other prizes
                    prizetext.text = $"{prize.prizeName}!";
                    break;
            }
            
            Debug.Log($"Showing prize won: {prizetext.text}");
        }
        catch (System.Exception e)
        {
            Debug.LogError($"Error showing prize won: {e.Message}");
        }
    }
        ...
        try
        {
            ...
            switch (prize.name)
            {
                ...
                case "Try Again Later":
                    // No action needed, just show the text
                    break;
                default:
    ...
    private void UnlockRandomEra()
    {
        ...
        if (lockedEras.Count > 0)
        {
            ...
            // Create a temporary prize to show the unlocked era
            Prize tempPrize = new Prize();
            tempPrize.name = "Random Era Unlocked";
            tempPrize.prizeName = $"Unlocked {eraToUnlock}";
            ShowPrizeWon(tempPrize);
        }
        else
        {
            ...
            // Create a temporary prize to show the fallback message
            Prize tempPrize = new Prize();
            tempPrize.name = "All Eras Unlocked";
            tempPrize.prizeName = "All eras already unlocked. Awarded 1000 points instead";
            ShowPrizeWon(tempPrize);
            
            // Adjust font size for this specific message
