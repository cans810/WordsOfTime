...
public class WordGameManager : MonoBehaviour
{
    ...
    public void CheckWord(string word, List<LetterTile> selectedTiles)
    {
        ...
        if (word == targetWord)
        {
            ...
            int currentPoints = GameManager.Instance.CurrentPoints;
            GameManager.Instance.AddPoints(GameManager.POINTS_PER_WORD);
            pointAnimationCoroutine = StartCoroutine(AnimatePointsIncrease(GameManager.POINTS_PER_WORD, selectedTiles));
            SoundManager.Instance.PlaySound("PointGain");
    ...
    private IEnumerator AnimatePointsIncrease(int pointsToAdd, List<LetterTile> guessedTiles)
    {
        ...
        StartCoroutine(SpawnCoins(coinCount, pointsToAdd, guessedTiles));
    }
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            StartCoroutine(MoveCoinToPanel(coin, () => {
                if (!pointsAnimationStarted)
                {
                    ...
                    // StartCoroutine(AnimatePointsIncrease(pointsToAdd)); // Remove this line to prevent looping
                }
            }));
