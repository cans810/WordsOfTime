...
public class WordGameManager : MonoBehaviour
{
    ...
    public void LoadWord(int index)
    {
        ...
        if (isSolved)
        {
            Debug.Log($"Word {targetWord} is already solved, highlighting in grid");
            HighlightWordInGrid(targetWord);
            
            // Update the sentence to show the solved word
            if (sentenceText != null && !string.IsNullOrEmpty(originalSentence))
            {
    ...
    private void HighlightWordInGrid(string word)
    {
        ...
        // Check if we have stored positions for this word
        List<Vector2Int> storedPositions = null;
        if (GameManager.Instance.GetSolvedWordPositions(translatedWord, out storedPositions) && storedPositions != null)
        {
            Debug.Log($"Using stored positions for word {translatedWord}");
            foreach (Vector2Int pos in storedPositions)
            {
                if (pos.x >= 0 && pos.x < gridSize && pos.y >= 0 && pos.y < gridSize)
                {
                    var tile = GridManager.Instance.grid[pos.x, pos.y];
                    tile.SetSolvedColor();
                    tile.isSolved = true;
        ...
        // If no stored positions, search for the word in the grid dynamically
        ...
        if (!wordFound)
        {
            Debug.LogError($"Word {translatedWord} not found in grid!");
        }
    }
    ...
    private void UpdateHintButton()
    {
        if (hintButton != null)
        {
            // Check if the current word is solved in any language
            bool isSolved = GameManager.Instance.IsWordSolved(currentBaseWord);
            ...
            // If not solved, update button based on hint level
            int hintLevel = GameManager.Instance.GetHintLevel(targetWord);
            bool canUseHint = GameManager.Instance.CanUseHint(hintLevel + 1, targetWord);
            
            hintButton.interactable = canUseHint;
            
            if (hintButtonText != null)
            {
                int hintCost = (hintLevel == 0) ? GameManager.HINT_COST : GameManager.SECOND_HINT_COST;
                hintButtonText.text = $"Hint {hintLevel + 1} ({hintCost})";
                hintButtonText.color = canUseHint ? Color.white : Color.red;
            }
        }
    }
    ...
    private void HandleLanguageChanged()
    {
        ...
        if (GridManager.Instance != null)
        {
            // First clear the grid to ensure proper display
            GridManager.Instance.ClearSolvedWords();
            
            // Then update the solved words display
            GridManager.Instance.UpdateSolvedWordsDisplay();
            
            // If the current word is solved, highlight it
    ...
    private void HandleEraChanged()
    {
        ...
        // Get the solved words for the new era
        solvedWordsInCurrentEra = GameManager.Instance.GetSolvedWordsForEra(GameManager.Instance.CurrentEra);
        
        // Start a new game in the current era
        StartNewGameInEra();
        
        // Update the grid display to show solved words in the new era
        if (GridManager.Instance != null)
        {
            GridManager.Instance.UpdateSolvedWordsDisplay();
        }
