...
public class GridManager : MonoBehaviour
{
    ...
    private IEnumerator GenerateGridCoroutine(List<char> letters)
    {
        ...
        const float SPAWN_DELAY = 0.08f; // Increased delay to make the sequence more visible
        int index = 0;
        ...
        // Create all tile objects first but keep them invisible
        ...
        for (int y = 0; y < gridSize; y++)
        {
            for (int x = 0; x < gridSize; x++)
            {
                if (index < letters.Count)
                {
                    ...
                    // Start with zero scale to make it invisible
                    ...
                    // Store the tile object for animation
        ...
        // Now animate each tile in sequence
        for (int y = 0; y < gridSize; y++)
        {
            for (int x = 0; x < gridSize; x++)
            {
                ...
                {
                    // Animate the tile
                    StartCoroutine(AnimateScale(tileObjects[x, y]));
                    
                    // Play a sound effect if you have one
                    // SoundManager.Instance.PlaySound("TileAppear");
    ...
    {
        float elapsedTime = 0;
        float duration = 0.25f; // Slightly faster animation
        
        // Add a small bounce effect
        AnimationCurve curve = new AnimationCurve(
            new Keyframe(0f, 0f),
            new Keyframe(0.7f, 1.1f), // Overshoot a bit
            new Keyframe(1f, 1f)      // Settle back to normal
        );
