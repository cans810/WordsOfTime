...
public class GameManager : MonoBehaviour
{
    ...
    private WordSetList wordSetsData;

    // Events for game state changes
    public delegate void WordGuessedHandler(string word);
    public event WordGuessedHandler OnWordGuessed;
    
    public delegate void HintUsedHandler(string wordKey, int hintLevel);
    public event HintUsedHandler OnHintUsed;
    
    public delegate void EraUnlockedHandler(string era);
    public event EraUnlockedHandler OnEraUnlocked;
    ...
    private void Awake()
    {
        ...
        EnsureSaveSystemInitialized();
    }

    private void EnsureSaveSystemInitialized()
    {
        // Make sure SaveManager exists
        if (SaveManager.Instance == null)
        {
            Debug.LogWarning("SaveManager not found, creating one");
    ...
    {
        Debug.Log("Loading words on Android platform");
        
        // For English words
        string enPath = Path.Combine(Application.streamingAssetsPath, "words.json");
        using (UnityWebRequest enRequest = UnityWebRequest.Get(enPath))
        {
            yield return enRequest.SendWebRequest();
            
            if (enRequest.result == UnityWebRequest.Result.Success)
            {
                string enJsonContent = enRequest.downloadHandler.text;
                LoadLanguageWords(enJsonContent, "en", false);
                Debug.Log("Successfully loaded English words");
            }
            else
            {
                Debug.LogError($"Failed to load English words: {enRequest.error}");
            }
        }
        
        // For Turkish words
        string trPath = Path.Combine(Application.streamingAssetsPath, "words_tr.json");
        using (UnityWebRequest trRequest = UnityWebRequest.Get(trPath))
        {
            yield return trRequest.SendWebRequest();
            
            if (trRequest.result == UnityWebRequest.Result.Success)
            {
                string trJsonContent = trRequest.downloadHandler.text;
                LoadLanguageWords(trJsonContent, "tr", false);
                Debug.Log("Successfully loaded Turkish words");
            }
            else
            {
                Debug.LogError($"Failed to load Turkish words: {trRequest.error}");
            }
        }
    ...
    private void LoadLanguageWords(string jsonContent, string language, bool isFromFile)
    {
        try 
        {
            // Store the word set data when loading English words
            if (language == "en")
            {
                wordSetsData = JsonUtility.FromJson<WordSetList>(jsonContent);
            }
    ...
    public void AddPoints(int points)
    {
        ...
        // Trigger the points changed event
        TriggerPointsChanged(CurrentPoints);
        
        // Save game after points are added
        if (SaveManager.Instance != null)
        {
            Debug.Log($"Saving game after adding {points} points");
            SaveManager.Instance.SaveGame();
        }
    }
    ...
    public bool UseHint(string wordKey, int hintLevel)
    {
        int cost = hintLevel == 1 ? HINT_COST : SECOND_HINT_COST;
        if (currentPoints >= cost)
        {
            //currentPoints -= cost;
            // Trigger the hint used event
            TriggerHintUsed(wordKey, hintLevel);
            
            // Save game after hint is used
            if (SaveManager.Instance != null)
            {
                Debug.Log($"Saving game after using hint level {hintLevel} for {wordKey}");
                SaveManager.Instance.SaveGame();
            }
    ...
    public bool IsWordSolved(string word)
    {
        ...
        return GetAllSolvedBaseWords().Contains(baseWord);
    }
    ...
    public void UnlockEra(string era)
    {
        if (!unlockedEras.Contains(era))
        {
            ...
            // Trigger the era unlocked event
            TriggerEraUnlocked(era);
            
            // Save game after era is unlocked
            if (SaveManager.Instance != null)
            {
                Debug.Log($"Saving game after unlocking era: {era}");
                SaveManager.Instance.SaveGame();
            }
        }
    }
    ...
    {
        // Find the base English word for any translation
        foreach (var translation in wordTranslations)
        {
            foreach (var lang in translation.Value)
            {
                if (lang.Value.Equals(word, StringComparison.OrdinalIgnoreCase))
                {
                    return translation.Key;
                }
            }
        }
        return word; // If no translation found, assume it's already the base word
    }
    ...
    public void HandleWordGuessedForAds()
    {
        if (noAdsBought) return; // Skip if user has purchased no ads

        wordGuessCount++;
        Debug.Log($"Words guessed since last ad: {wordGuessCount}");
        
        if (wordGuessCount >= WORDS_BETWEEN_ADS)
        {
            Debug.Log("Reached 3 words, showing ad and resetting counter");
            wordGuessCount = 0;
    ...
    public void StoreSolvedBaseWord(string era, string baseWord)
    {
        ...
        // Initialize era-specific solved words if needed
        if (!solvedBaseWordsPerEra.ContainsKey(era))
        {
            solvedBaseWordsPerEra[era] = new HashSet<string>();
        }
        
        // Add the base word to the era-specific solved words
        solvedBaseWordsPerEra[era].Add(baseWord);
    ...
    {
        Debug.Log("GameManager: Application quitting, saving game state");
        SaveGameState();
    }
    ...
    {
        // Ensure ticks are set before serializing
        adState.nextAvailableTimeTicks = adState.nextAvailableTime.Ticks;
    ...
    public AdState LoadAdState(AdState? newState = null)
    {
        if (newState != null)
        {
            ...
            // Set the ticks value for JSON serialization
            adState.nextAvailableTimeTicks = adState.nextAvailableTime.Ticks;
            SaveAdState();
            return adState;
        }
        ...
        if (PlayerPrefs.HasKey(AD_STATE_KEY))
        {
            ...
            // If we have ticks value, use it to restore the DateTime
            if (adState.nextAvailableTimeTicks > 0)
            {
                adState.nextAvailableTime = new DateTime(adState.nextAvailableTimeTicks);
            }
            ...
            if (lastAdTime > 0)
            {
                ...
                if (!adState.canWatch)
                {
                    ...
                    // Update ticks for serialization
                    adState.nextAvailableTimeTicks = adState.nextAvailableTime.Ticks;
                }
            }
        }
        else
        {
            ...
            DateTime nextTime = DateTime.Now.AddSeconds(Math.Max(0, remainingTime));
            
            adState = new AdState
            {
                canWatch = remainingTime <= 0,
                nextAvailableTime = nextTime,
                nextAvailableTimeTicks = nextTime.Ticks
            };
        }
    ...
    {
        HashSet<string> allSolvedWords = new HashSet<string>();
        foreach (var era in solvedBaseWordsPerEra.Keys)
        {
            allSolvedWords.UnionWith(solvedBaseWordsPerEra[era]);
        }
        return allSolvedWords;
    }

    public void LoadSolvedBaseWords(HashSet<string> solvedWords)
    {
        foreach (var baseWord in solvedWords)
        {
            // Determine which era this word belongs to
            foreach (var era in eraWordsPerLanguage["en"].Keys)
            {
                if (eraWordsPerLanguage["en"][era].Contains(baseWord))
                {
                    if (!solvedBaseWordsPerEra.ContainsKey(era))
                    {
                        solvedBaseWordsPerEra[era] = new HashSet<string>();
    ...
    public void UpdatePointsDisplay()
    {
        ...
        {
            WordGameManager.Instance.UpdatePointsDisplay();
        }
        else
        {
            // Find the point text in the main menu
            Transform pointPanel = GameObject.Find("PointPanel")?.transform;
            if (pointPanel != null)
            {
                TextMeshProUGUI pointText = pointPanel.Find("point")?.GetComponent<TextMeshProUGUI>();
                if (pointText != null)
                {
    ...
    public string GetWordDifficulty(string word)
    {
        ...
        if (wordSetsData == null)
        {
            Debug.LogWarning("Word sets data not loaded");
            return difficulty;
        }

        // Look for the word in the current era
        if (eraWordsPerLanguage.ContainsKey(currentLanguage) && 
            eraWordsPerLanguage[currentLanguage].ContainsKey(CurrentEra))
        {
            var words = eraWordsPerLanguage[currentLanguage][CurrentEra];
            int wordIndex = words.IndexOf(word);
            
            if (wordIndex >= 0)
            {
                // Get the word entry from the JSON data
                var eraSet = wordSetsData.sets.FirstOrDefault(s => s.era == CurrentEra);
                if (eraSet != null && wordIndex < eraSet.words.Length)
                {
                    difficulty = eraSet.words[wordIndex].difficulty;
                }
            }
        }
    ...
    // Method to trigger the OnWordGuessed event
    public void TriggerWordGuessed(string word)
    {
        Debug.Log($"Word guessed: {word}");
        OnWordGuessed?.Invoke(word);
        
        // Also increment the word guess count for ad tracking
        HandleWordGuessedForAds();
        
        // Ensure we save the game state after guessing a word
        if (SaveManager.Instance != null)
        {
            SaveManager.Instance.SaveGame();
        }
    ...
    // Method to trigger the OnHintUsed event
    private void TriggerHintUsed(string wordKey, int hintLevel)
    {
        OnHintUsed?.Invoke(wordKey, hintLevel);
    }
    
    // Method to trigger the OnEraUnlocked event
    private void TriggerEraUnlocked(string era)
    {
        OnEraUnlocked?.Invoke(era);
    }

    // Method to trigger the OnPointsChanged event
    private void TriggerPointsChanged(int points)
    {
    ...
    private void OnApplicationPause(bool pauseStatus)
    {
        if (pauseStatus)
        {
            ...
            Debug.Log("Application paused, saving game state");
            SaveGameState();
        }
        else
        {
            ...
            // Make sure we're registered for events
            if (SaveManager.Instance != null)
            {
                SaveManager.Instance.RegisterGameEvents();
            }
        }
    }
}