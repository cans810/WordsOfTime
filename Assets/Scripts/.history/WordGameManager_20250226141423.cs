...
public class WordGameManager : MonoBehaviour
{
    ...
    private void UpdateHintButton()
    {
        ...
        {
            // Get the base word for consistent checking
            string baseWord = GameManager.Instance.GetBaseWord(targetWord);
            
            // Check if the word is solved using the base word
            bool isSolved = GameManager.Instance.IsWordSolved(baseWord);
            
            Debug.Log($"UpdateHintButton: Word={targetWord}, BaseWord={baseWord}, IsSolved={isSolved}, Era={GameManager.Instance.CurrentEra}");
            
            // Update button state
            hintButton.interactable = !isSolved;
    ...
    public void CheckWord(string word, List<LetterTile> selectedTiles)
    {
        ...
        if (word == targetWord)
        {
            ...
            // Store the solved word index and positions
            StoreSolvedWordIndex(word);
            
            // Trigger word solved event
            OnWordSolved?.Invoke(word);
            
            // Mark the word as guessed in GameManager
            GameManager.Instance.TriggerWordGuessed(word);
            
            // Update progress bar
            UpdateProgressBar();
            
            // Update sentence display to show the solved word
            UpdateSentenceDisplay();
            ...
            // Add points
            int pointsToAdd = GameManager.POINTS_PER_WORD;
            ...
            pointAnimationCoroutine = StartCoroutine(AnimatePointsIncrease(pointsToAdd));
            
            // Increment words guessed count for ad display
            wordsGuessedCount++;
            ...
            // Display a fact if available
            if (didYouKnowPanel != null && !string.IsNullOrEmpty(targetWord))
            {
                ShowDidYouKnowFact(targetWord);
            }
    ...
    {
        Debug.Log($"StoreSolvedWordIndex: Storing word {word} in era {GameManager.Instance.CurrentEra}");
        ...
        // Store the positions for both the current word and its translations
        ...
        foreach (string language in new[] { "en", "tr" })
        {
            ...
            {
                Debug.Log($"Storing positions for translated word: {translatedWord} in {language}");
                GameManager.Instance.StoreSolvedWordPositions(translatedWord, positions);
            }
        }
        
        // Force update the grid to show solved words
        if (GridManager.Instance != null)
        {
            GridManager.Instance.UpdateSolvedWordsDisplay();
        }
    ...
    {
        Debug.Log("Language changed, updating UI");
        
        // Update the sentence with the translated word
        if (currentBaseWord != null)
        {
        ...
        // Force update the grid to show solved words in the new language
        if (GridManager.Instance != null)
        {
    ...
    {
        Debug.Log($"Era changed to {GameManager.Instance.CurrentEra}, updating UI");
        ...
        // Get solved words for the new era
        solvedWordsInCurrentEra = GameManager.Instance.GetSolvedWordsForEra(GameManager.Instance.CurrentEra);
        
        // Start new game in the current era
        StartNewGameInEra();
