...
public class GameManager : MonoBehaviour
{
    ...
    public bool NoAdsBought
    {
        get
        {
            Debug.Log($"NoAdsBought checked: {noAdsBought}");
            return noAdsBought;
        }
    }
    ...
    private void Start()
    {
        ...
        // Migrate existing saved data
        MigrateSolvedWordsToUniqueFormat();
    }
    ...
    public bool IsWordSolved(string word)
    {
        ...
        // Create the unique identifier with era prefix
        string uniqueBaseWord = $"{CurrentEra}_{baseWord}";
        
        Debug.Log($"IsWordSolved check for word: '{word}', baseWord: '{baseWord}', uniqueBaseWord: '{uniqueBaseWord}'");
        
        // Check if the unique base word is solved in the current era
        return solvedBaseWordsPerEra.ContainsKey(CurrentEra) && 
               solvedBaseWordsPerEra[CurrentEra].Contains(uniqueBaseWord);
    }
    ...
    public void SetGuessedWords(List<string> words)
    {
        ...
        foreach (string word in words)
        {
            ...
            // Add the word to the appropriate era with era prefix
            string uniqueBaseWord = $"{era}_{word}";
            solvedBaseWordsPerEra[era].Add(uniqueBaseWord);
    ...
    {
        Debug.Log($"GetBaseWord called for word: '{word}'");
        
        WordSetList wordSetList = LoadWordSetList();
        if (wordSetList == null)
        {
            Debug.LogWarning($"WordSetList is null, returning original word: '{word}'");
            return word;
        }
        ...
        foreach (WordSet wordSet in wordSetList.sets)
        {
            foreach (WordEntry wordEntry in wordSet.words)
            {
                ...
                {
                    string baseWord = wordEntry.translations.en;
                    Debug.Log($"Found base word in {wordSet.era}: '{baseWord}' for Turkish word '{word}'");
                    
                    // Check if this base word maps to multiple Turkish words
                    foreach (WordSet checkSet in wordSetList.sets)
                    {
                        foreach (WordEntry checkEntry in checkSet.words)
                        {
                            if (string.Equals(checkEntry.translations.en, baseWord, StringComparison.OrdinalIgnoreCase) &&
                                !string.Equals(checkEntry.translations.tr, word, StringComparison.OrdinalIgnoreCase))
                            {
                                Debug.LogWarning($"POTENTIAL CONFLICT: Base word '{baseWord}' also maps to Turkish word '{checkEntry.translations.tr}'");
                            }
                        }
                    }
    ...
    public HashSet<string> GetSolvedBaseWordsForEra(string era)
    {
        if (solvedBaseWordsPerEra.ContainsKey(era))
        {
            // Create a new HashSet to store the extracted base words
            HashSet<string> baseWords = new HashSet<string>();
            
            // Extract the base word part from each unique identifier
            foreach (string uniqueBaseWord in solvedBaseWordsPerEra[era])
            {
                // Check if the word has the era prefix format
                if (uniqueBaseWord.StartsWith(era + "_"))
                {
                    // Extract the base word part (remove era prefix)
                    string baseWord = uniqueBaseWord.Substring(era.Length + 1);
                    baseWords.Add(baseWord);
                }
                else
                {
                    // For backward compatibility with old saved data
                    baseWords.Add(uniqueBaseWord);
                }
            }
    ...
    public void StoreSolvedBaseWord(string era, string baseWord)
    {
        ...
        // Store the base word with era prefix to avoid conflicts
        string uniqueBaseWord = $"{era}_{baseWord}";
        solvedBaseWordsPerEra[era].Add(uniqueBaseWord);
    ...
    private void MigrateSolvedWordsToUniqueFormat()
    {
        Debug.Log("Migrating solved words to unique format...");
        
        // Create a temporary copy of the data
        Dictionary<string, HashSet<string>> tempData = new Dictionary<string, HashSet<string>>();
        
        // Copy the existing data
        foreach (var eraPair in solvedBaseWordsPerEra)
        {
            string era = eraPair.Key;
            tempData[era] = new HashSet<string>();
            
            foreach (string baseWord in eraPair.Value)
            {
                // Check if the word already has the era prefix
                if (!baseWord.StartsWith(era + "_"))
                {
                    // Convert to the new format
                    string uniqueBaseWord = $"{era}_{baseWord}";
                    tempData[era].Add(uniqueBaseWord);
                    Debug.Log($"Migrated word '{baseWord}' to '{uniqueBaseWord}'");
                }
                else
                {
                    // Already in the new format
                    tempData[era].Add(baseWord);
                }
            }
        }
        
        // Replace the old data with the migrated data
        solvedBaseWordsPerEra = tempData;
