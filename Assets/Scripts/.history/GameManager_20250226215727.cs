...
public class GameManager : MonoBehaviour
{
    ...
    {
        // First check if the exact word is solved in the current language
        if (solvedWordsPerLanguage.ContainsKey(CurrentLanguage) && 
            solvedWordsPerLanguage[CurrentLanguage].Contains(word))
        {
            Debug.Log($"Word '{word}' is directly solved in {CurrentLanguage}");
            return true;
        }
        
        // As a fallback, check base word solution
        string baseWord = GetBaseWord(word);
        
        // Check if the base word is solved in current era
        if (solvedBaseWordsPerEra.ContainsKey(CurrentEra) && 
           solvedBaseWordsPerEra[CurrentEra].Contains(baseWord))
        {
            // Only return true for English or if this exact word was solved
            if (CurrentLanguage == "en" || 
                (solvedWordsPerLanguage.ContainsKey(CurrentLanguage) && 
                 solvedWordsPerLanguage[CurrentLanguage].Contains(word)))
            {
                Debug.Log($"Word '{word}' is solved via its base word '{baseWord}'");
                return true;
            }
        }
    ...
    public bool IsWordGuessed(string word)
    {
        ...
        // As a fallback, check if the base word is solved in any era
        string baseWord = GetBaseWord(word);
        foreach (var era in solvedBaseWordsPerEra.Keys)
        {
            if (solvedBaseWordsPerEra[era].Contains(baseWord))
            {
                // Don't automatically return true here
                // Only return true if we're checking in English or this is the exact word that was solved
                if (CurrentLanguage == "en" || 
                    (solvedWordsPerLanguage.ContainsKey(CurrentLanguage) && 
                     solvedWordsPerLanguage[CurrentLanguage].Contains(word)))
                {
    ...
    public void StoreSolvedBaseWord(string era, string baseWord)
    {
        ...
        // Instead of automatically storing all translations as solved, 
        // only store the exact word that was solved
        string currentLanguage = CurrentLanguage;
        string solvedWord = GetTranslation(baseWord, currentLanguage);
        
        if (!solvedWordsPerLanguage.ContainsKey(currentLanguage))
        {
            solvedWordsPerLanguage[currentLanguage] = new HashSet<string>();
        }
        solvedWordsPerLanguage[currentLanguage].Add(solvedWord);
        
        // Log what's happening for debugging
        Debug.Log($"Marked as solved: {solvedWord} in language {currentLanguage}");
