...
public class GameManager : MonoBehaviour
{
    ...
    {
        // Store positions for the word in ALL eras
        foreach (string era in eraList)
        {
            string key = $"{era}_{word}";
            if (!solvedWordPositions.ContainsKey(key))
            {
                solvedWordPositions[key] = new List<Vector2Int>();
            }
            solvedWordPositions[key] = positions;
        }
    ...
    {
        // Try to get positions for the current era first
        string currentEraKey = $"{CurrentEra}_{word}";
        if (solvedWordPositions.ContainsKey(currentEraKey))
        {
            return solvedWordPositions[currentEraKey];
        }
        
        // If not found, try any era
        foreach (string era in eraList)
        {
            string key = $"{era}_{word}";
            if (solvedWordPositions.ContainsKey(key))
            {
                return solvedWordPositions[key];
            }
        }
    ...
    public void SetLanguage(string newLanguage)
    {
        if (currentLanguage != newLanguage)
        {
            ...
            // Update the grid to show solved words in the new language
            if (GridManager.Instance != null)
            {
                GridManager.Instance.UpdateSolvedWordsDisplay();
            }
