...
public static class WordValidator
{
    ...
    public static string GetFactForWord(string word, string era, string language = "en")
    {
        ...
        // Check if the word exists in the facts dictionary
        if (wordSetsWithFactsByLanguage.ContainsKey(language) && wordSetsWithFactsByLanguage[language].ContainsKey(era))
        {
            // Check if the language exists for this era
            if (wordSetsWithFactsByLanguage[language][era].ContainsKey(language))
            {
                Debug.Log($"[WordValidator] Found fact for word '{word}' in era '{era}' and language '{language}'");
                return wordSetsWithFactsByLanguage[language][era][language];
            }
            // If language not found but English exists, return English as fallback
            else if (language != "en" && wordSetsWithFactsByLanguage[language].ContainsKey("en"))
            {
                Debug.Log($"[WordValidator] No fact found for language '{language}', using English fallback for word '{word}' in era '{era}'");
                return wordSetsWithFactsByLanguage[language]["en"];
            }
            else
            {
                Debug.LogWarning($"[WordValidator] No fact found for word '{word}' in era '{era}' and language '{language}' or English fallback");
                return string.Empty;
            }
        }
        else
        {
            Debug.LogWarning($"[WordValidator] Word '{word}' not found in facts dictionary");
            return string.Empty;
        }
    }
