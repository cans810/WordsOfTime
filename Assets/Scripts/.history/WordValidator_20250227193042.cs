...
public static class WordValidator
{
    ...
    public static void LoadWordSets()
    {
        ...
        // If we're on Android, use the specialized loading method
        #if UNITY_ANDROID && !UNITY_EDITOR
            Debug.Log("[WordValidator] Running on Android platform, will load words directly from StreamingAssets");
            // Try to start Android loading immediately, or it will be retried later
    ...
    // Define the method signature outside the conditional block
    private static IEnumerator LoadWordsForLanguageAndroid(string language)
    {
        ...
        if (!loadSuccess)
        {
            ...
            {
                Debug.LogError($"[Android Debug] Error using Android AssetManager: {e.Message}\n{e.StackTrace}");
            }
        }
        ...
        #else
        // For non-Android platforms, just load directly
        LoadWordsForLanguage(language);
        isLoading = false;
        yield return null;
        #endif
    }
    ...
    private static void ProcessWordJson(string jsonContent, string language)
    {
        ...
        try
        {
            ...
            {
                // Store the original era name for lookup
                string originalEraName = set.era;
                
                // Standardize the era name for storage - use English names internally
                string standardizedEraName = language == "tr" ? MapTurkishEraToEnglish(set.era) : set.era;
                string eraName = standardizedEraName.ToLower();
                
                Debug.Log($"[Android Debug] Processing era '{originalEraName}' (standardized to '{eraName}') for {language}");
                ...
                foreach (WordEntry wordEntry in set.words)
                {
                    if (!string.IsNullOrEmpty(wordEntry.word))
                    {
                        ...
                        // Store in the loadedWords dictionary with standardized era name
                        loadedWords[language][eraName][wordKey] = word;
                        
                        // Extra logging for Turkish words
                        if (language == "tr")
                        {
                            Debug.Log($"[Android Debug] Added Turkish word '{wordKey}' to era '{eraName}' (original era: '{originalEraName}')");
                        }
                    }
                }
            }
    ...
    public static string GetFactForWord(string word, string era, string language)
    {
        ...
        // Normalize the era name based on language - this is the key fix
        // Always use the standardized era name (English) for lookup to ensure consistency
        string normalizedEra = era.ToLower();
        if (language == "tr")
        {
            // First check if this might be a Turkish era name that needs to be mapped
            string mappedEra = MapTurkishEraToEnglish(era);
            if (mappedEra != era)
            {
                Debug.Log($"[Android Debug] Mapped Turkish era '{era}' to English '{mappedEra}' for fact lookup");
                normalizedEra = mappedEra.ToLower();
            }
        }
        ...
        if (!loadedWords[language].ContainsKey(normalizedEra))
        {
            ...
            {
                // Extra debug for Turkish
                if (language == "tr")
                {
                    Debug.LogError($"[Android Debug] Critical: Turkish era '{normalizedEra}' not found in loadedWords. This could be a mapping issue.");
                    Debug.Log($"[Android Debug] Available eras in Turkish: {string.Join(", ", loadedWords[language].Keys)}");
                    
                    // Try all possible mappings as a last resort
                    Dictionary<string, string> allMappings = new Dictionary<string, string>
                    {
                        {"antik mısır", "ancient egypt"},
                        {"antik yunan", "ancient greece"},
                        {"orta çağ avrupası", "medieval europe"},
                    ...
                    {
                        if (loadedWords[language].ContainsKey(mapping.Value))
                        {
                            Debug.Log($"[Android Debug] Found matching era using alternate mapping: '{mapping.Key}' -> '{mapping.Value}'");
                            normalizedEra = mapping.Value;
                            break;
                        }
                    }
                }
    ...
    public static void CheckWordInJsonFiles(string word, string era)
    {
        ...
        // Normalize and map the era name if needed
        string normalizedEra = era.ToLower();
        string mappedEra = era; // Default to original era name
        
        // Check if this could be a Turkish era name that needs mapping
        string possiblyMappedEra = MapTurkishEraToEnglish(era);
        if (possiblyMappedEra != era)
        {
            Debug.Log($"[WordValidator] Mapped potential Turkish era name '{era}' to '{possiblyMappedEra}'");
            mappedEra = possiblyMappedEra;
            normalizedEra = mappedEra.ToLower();
        }
        ...
        if (File.Exists(enFilePath))
        {
            ...
            try
            {
                ...
                if (wordSetList?.sets != null)
                {
                    foreach (var wordSet in wordSetList.sets)
                    {
                        if (wordSet.era.Equals(mappedEra, StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Log($"[WordValidator] Found era '{mappedEra}' in English JSON file with {wordSet.words.Length} words");
                            
                            foreach (var wordEntry in wordSet.words)
                            {
                                if (wordEntry.word.Equals(word, StringComparison.OrdinalIgnoreCase))
                                {
                                    Debug.Log($"[WordValidator] FOUND WORD '{word}' in era '{mappedEra}' in English JSON file!");
                                    ...
                                    // Use the standardized era name for storage
                                    ...
                                    Debug.Log($"[WordValidator] Successfully stored fact for word '{word}' in memory under normalized era '{normalizedEra}'");
                                    return;
                                }
                            }
                            
                            Debug.LogWarning($"[WordValidator] Word '{word}' NOT FOUND in era '{mappedEra}' in English JSON file");
        ...
        if (File.Exists(trFilePath))
        {
            ...
            try
            {
                ...
                if (wordSetList?.sets != null)
                {
                    foreach (var wordSet in wordSetList.sets)
                    {
                        // Map Turkish era name to English for comparison
                        string turkishMappedEra = MapTurkishEraToEnglish(wordSet.era);
                        
                        // Check if this Turkish era matches our target era (either mapped or original)
                        if (turkishMappedEra.Equals(mappedEra, StringComparison.OrdinalIgnoreCase) || 
                            turkishMappedEra.Equals(era, StringComparison.OrdinalIgnoreCase) ||
                            wordSet.era.Equals(era, StringComparison.OrdinalIgnoreCase))
                        {
                            Debug.Log($"[WordValidator] Found matching era '{era}' (as '{wordSet.era}') in Turkish JSON file with {wordSet.words.Length} words");
                            ...
                            foreach (var wordEntry in wordSet.words)
                            {
                                      ...
                                      wordEntry.translations.tr.Equals(word, StringComparison.OrdinalIgnoreCase))))
                                {
                                    ...
                                    // Always store Turkish facts with the English standardized era name
                                    ...
                                    Debug.Log($"[WordValidator] Successfully stored Turkish fact for word '{wordKey}' under normalized era '{normalizedEra}'");
                                    
                                    // If we have English translations, also store them
                                    if (wordEntry.translations != null && !string.IsNullOrEmpty(wordEntry.translations.en))
                                    {
                                        // Make sure the language, era, and word dictionaries exist for English
                                        if (!loadedWords.ContainsKey("en"))
                                        {
                                            loadedWords["en"] = new Dictionary<string, Dictionary<string, Word>>();
                                        ...
                                        Debug.Log($"[WordValidator] Also stored fact for English translation '{enWordKey}' under normalized era '{normalizedEra}'");
                                    }
    ...
    {
        Debug.Log("[WordValidator] Verifying Turkish facts...");
        
        // Check if data is still loading
        if (isLoading)
        {
            Debug.LogWarning("[WordValidator] Data is still loading, cannot verify facts yet");
            return;
        }
        
        // Check if Turkish language data is available
        if (!loadedWords.ContainsKey("tr"))
        {
            Debug.LogError("[WordValidator] Turkish language data not found in loadedWords!");
            return;
        }
        ...
        // Turkish-specific characters to check for
        char[] turkishChars = new char[] { 'ç', 'ğ', 'ı', 'ö', 'ş', 'ü', 'Ç', 'Ğ', 'İ', 'Ö', 'Ş', 'Ü' };
        
        // Check each era
        foreach (var era in loadedWords["tr"].Keys)
        {
            Debug.Log($"[WordValidator] Checking era: {era}");
            ...
            foreach (var wordEntry in eraWords)
            {
                ...
                if (!string.IsNullOrEmpty(wordData.didYouKnow))
                {
                    ...
                    // Check if the fact contains Turkish characters
                    bool hasTurkishChars = wordData.didYouKnow.IndexOfAny(turkishChars) >= 0;
                    if (hasTurkishChars)
                    {
                    ...
                    {
                        Debug.LogWarning($"[WordValidator] Turkish word '{word}' has a fact without Turkish characters: '{wordData.didYouKnow.Substring(0, Math.Min(30, wordData.didYouKnow.Length))}...'");
                    }
                }
            }
        }
        
        Debug.Log($"[WordValidator] Turkish facts verification complete:");
        Debug.Log($"[WordValidator] Total words: {totalWords}");
        Debug.Log($"[WordValidator] Words with facts: {wordsWithFacts} ({(totalWords > 0 ? (wordsWithFacts * 100f / totalWords).ToString("F1") : "0")}%)");
        Debug.Log($"[WordValidator] Words with Turkish facts: {wordsWithTurkishFacts} ({(wordsWithFacts > 0 ? (wordsWithTurkishFacts * 100f / wordsWithFacts).ToString("F1") : "0")}%)");
        
        // If fewer than 10% of words have Turkish facts, check if English facts are being used for Turkish words
        if (wordsWithTurkishFacts < totalWords * 0.1f && loadedWords.ContainsKey("en"))
        {
            Debug.LogWarning("[WordValidator] Very few Turkish facts detected. Checking if English facts are being used...");
            
            int englishFactsForTurkishWords = 0;
            
            foreach (var era in loadedWords["tr"].Keys)
            {
                if (!loadedWords["en"].ContainsKey(era)) continue;
                
                var trEraWords = loadedWords["tr"][era];
                var enEraWords = loadedWords["en"][era];
                
                foreach (var wordEntry in trEraWords)
                {
                    string trWord = wordEntry.Key;
                    Word trWordData = wordEntry.Value;
                    
                    // If this Turkish word has no fact, check if its English counterpart does
                    if (string.IsNullOrEmpty(trWordData.didYouKnow) && trWordData.translations != null)
                    {
                        string enWord = trWordData.translations.en.ToLower();
                        
                        if (enEraWords.ContainsKey(enWord) && !string.IsNullOrEmpty(enEraWords[enWord].didYouKnow))
                        {
                            englishFactsForTurkishWords++;
                            Debug.Log($"[WordValidator] Turkish word '{trWord}' has no fact, but English word '{enWord}' does: '{enEraWords[enWord].didYouKnow.Substring(0, Math.Min(30, enEraWords[enWord].didYouKnow.Length))}...'");
                        }
                    }
                }
            }
            
            Debug.Log($"[WordValidator] Found {englishFactsForTurkishWords} English facts that could be used for Turkish words");
        }
    }
    ...
    public static void VerifyFactsAfterLoading()
    {
        ...
        if (isLoading)
        {
            Debug.Log("[WordValidator] Facts still loading, will verify later");
            // Try again in 2 seconds
            MonoBehaviour mb = GameObject.FindObjectOfType<MonoBehaviour>();
            if (mb != null)
            {
                mb.Invoke("VerifyFactsAfterLoading", 2.0f);
            }
            return;
        }
        
        // Verify Turkish facts
        VerifyTurkishFacts();
    }
