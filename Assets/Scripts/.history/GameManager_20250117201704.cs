...
public class GameManager : MonoBehaviour
{
    ...
    public int CurrentPoints
    {
        ...
        { 
            currentPoints = value;
            OnPointsChanged?.Invoke();  // Invoke the event when points change
        }
    }
    ...
    // Era unlocking system
    private Dictionary<string, int> eraPrices = new Dictionary<string, int>()
    {
        { "Ancient Egypt", 0 },
        { "Medieval Europe", 0 },        // First era - free
        { "Renaissance", 500 },       // Second era - free
        { "Industrial Revolution", 1000 },      // Third era - 1000 points
        { "Ancient Greece", 2000 },  // Fourth era - 2000 points
        // Add more eras and prices as needed
    };

    private HashSet<string> unlockedEras = new HashSet<string>() 
    { 
        "Ancient Egypt",    // Start with first era unlocked
        "Medieval Europe"    // Start with second era unlocked
    };
    ...
    private void Awake()
    {
        if (Instance == null)
        {
            ...
            // Initialize era unlocks and prices
            eraPrices = new Dictionary<string, int>()
            {
                { "Ancient Egypt", 0 },        // First era - free
                { "Ancient Greece", 0 },       // Second era - free
                { "Renaissance", 1000 },      // Third era - 1000 points
                { "Medieval Europe", 2000 },   // Fourth era - 2000 points
                { "Industrial Revolution", 3000 }   // Fifth era - 3000 points
            };

            unlockedEras = new HashSet<string>() 
            { 
                "Ancient Egypt",    // Start with first era unlocked
                "Ancient Greece"    // Start with second era unlocked
            };

            // Start with a random UNLOCKED era
            string[] availableEras = unlockedEras.ToArray();
            System.Random rng = new System.Random();
            int randomIndex = rng.Next(availableEras.Length);
            CurrentEra = availableEras[randomIndex];
            Debug.Log($"Starting with random unlocked era: {CurrentEra}");
        }
        else
        {
            Destroy(gameObject);
        ...
        foreach (var era in eraWords.Keys)
        {
            foreach (var word in eraWords[era])
            {
                if (!initialGrids.ContainsKey(word))
                {
                    ...
                    {
                        int maxAttempts = 300; // Increased attempts for better randomization
                        bool placed = false;
                        ...
                        while (!placed && maxAttempts > 0)
                        {
                            try
                            {
                                ...
                                // Try each starting position until we find a valid path
    ...
    public HashSet<int> GetSolvedWordsForEra(string era)
    {
        if (solvedWordsPerEra.ContainsKey(era))
        {
            return new HashSet<int>(solvedWordsPerEra[era]); // Return a copy of the set
        }
    ...
    {
        CurrentEra = newEra;
        // Make sure we have an entry for this era
    ...
    // Optional: Method to clear progress (for testing)
    ...
    {
        if (IsEraUnlocked(era)) return false;
        return CurrentPoints >= GetEraPrice(era);
    }

    public bool TryUnlockEra(string era)
    {
        if (IsEraUnlocked(era)) return false;
        
        int price = GetEraPrice(era);
        if (CurrentPoints >= price)
        {
            CurrentPoints -= price;
            unlockedEras.Add(era);
            Debug.Log($"Unlocked era: {era}");
            return true;
        }
        return false;
    }
