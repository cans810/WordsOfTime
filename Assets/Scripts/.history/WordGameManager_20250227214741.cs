    ...
    private void ProcessSolvedWords()
    {
        try
        {
            ...
            if (currentWordIndex < currentEraWords.Count)
            {
                ...
                foreach (var eraEntry in GameManager.Instance.solvedBaseWordsPerEra)
                {
                    Debug.Log($"[Android Debug] Era '{eraEntry.Key}': {string.Join(", ", eraEntry.Value)}");
                }
