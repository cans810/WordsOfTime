...
public class WordGameManager : MonoBehaviour
{
    ...
    private IEnumerator SpawnCoins(int coinCount, List<LetterTile> guessedTiles)
    {
        ...
        for (int i = 0; i < coinCount; i++)
        {
            ...
            StartCoroutine(AnimateCoinMovement(coin, spawnPosition, () => {
    ...
    private IEnumerator AnimateCoinMovement(GameObject coin, Vector3 startPos, Action onDestroy)
    {
        const float DELAY_BETWEEN_COINS = 0.1f; // Coinler arası gecikme
        const float MOVEMENT_DURATION = 0.5f;   // Tek bir coinin hareket süresi

        // Coin sayısına göre gecikmeleri ayarla
        yield return new WaitForSeconds(DELAY_BETWEEN_COINS * coin.transform.GetSiblingIndex());

        float elapsedTime = 0f;
        Vector3 controlPoint1 = startPos + new Vector3(0, 200f, 0);
        Vector3 controlPoint2 = pointPanel.transform.position + new Vector3(0, 200f, 0);

        while (elapsedTime < MOVEMENT_DURATION)
        {
            elapsedTime += Time.deltaTime;
            float t = elapsedTime / MOVEMENT_DURATION;
            t = t * t * (3 - 2 * t); // Cubic easing

            // Bezier eğrisi üzerinde hareket
            coin.transform.position = CalculateBezierPoint(t, startPos, controlPoint1, controlPoint2, pointPanel.transform.position);
            yield return null;
        }

        // Son pozisyona tam olarak yerleş
        coin.transform.position = pointPanel.transform.position;
        onDestroy?.Invoke();
        Destroy(coin);
    }
