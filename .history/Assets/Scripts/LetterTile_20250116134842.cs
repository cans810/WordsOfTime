...
public class LetterTile : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    public void ResetTile()
    {
        ...
        if (!isSolved)
        {
            backgroundImage.color = defaultColor; // Reset color if not solved.
        }
        SolvedWord = "";
    }
    ...
    public void SetLetter(char letter, Vector2Int position, string solvedWord = "")
{
    ...
    // Do NOT set SolvedWord here. This will be done later in RestoreInitialGrid

    isSolved = false; // ALWAYS reset isSolved to false. 
