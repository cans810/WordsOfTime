...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        char[,] preGeneratedGrid = GameManager.Instance.GetPreGeneratedGrid(targetWord); // Retrieve the grid

        if (preGeneratedGrid != null)
        {
            ApplyPreGeneratedGrid(preGeneratedGrid);
        }
        else {
            Debug.LogError($"Failed to retrieve grid for {targetWord}");
        }
    }

    private void ApplyPreGeneratedGrid(char[,] gridToApply)
    {
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                grid[x, y].SetLetter(gridToApply[x, y], new Vector2Int(x, y));
                // ... (Rest of your logic to set solved state, color, etc.)
            }
        }
    }
    ...
    public void FillRemainingSpacesInGrid(char[,] gridToFill)
    {
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                if (gridToFill[x, y] == '\0')
                {
                    gridToFill[x, y] = (char)UnityEngine.Random.Range('A', 'Z' + 1);
                }
            }
        }
    }
    ...
    private void PlaceWordInGrid(string word)
{
    ...
    // Randomly decide whether the word will be placed horizontally or vertically
    bool isHorizontal = UnityEngine.Random.value > 0.5f;

    // Determine the grid boundaries to ensure the word fits
    int maxStartX = isHorizontal ? gridSize - word.Length : gridSize - 1;
    int maxStartY = isHorizontal ? gridSize - 1 : gridSize - word.Length;

    // Pick a random starting position within bounds
    int startX = UnityEngine.Random.Range(0, maxStartX + 1);
    int startY = UnityEngine.Random.Range(0, maxStartY + 1);

    // Place the word in the selected direction
    for (int i = 0; i < word.Length; i++)
    {
        int x = isHorizontal ? startX + i : startX;
        int y = isHorizontal ? startY : startY + i;

        grid[x, y].SetLetter(word[i], new Vector2Int(x, y));
        grid[x, y].isPartOfWord = true; // Mark the tile as part of the target word
    }

    Debug.Log($"Placed word '{word}' starting at ({startX}, {startY}) in " +
