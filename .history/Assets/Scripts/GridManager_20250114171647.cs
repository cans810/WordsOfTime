    ...
    public string SelectTargetWord()
{
    ...
    if (targetWord == null)
    {
        return null; // Signal end of game or era
    }

    string sentence = WordValidator.GetSentenceForWord(targetWord, GameManager.Instance.CurrentEra);

    ResetGridForNewWord();  // Reset the grid for the new word
    WordGameManager.Instance.SetupGame(targetWord, sentence); // Setup the WordGameManager with new word/sentence

    lettersToPlace = new List<char>(targetWord.ToCharArray()); // Update the letters (if needed)
    return targetWord;
}
