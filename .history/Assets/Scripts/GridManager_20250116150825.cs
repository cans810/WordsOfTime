...
public class GridManager : MonoBehaviour
{
    ...
    private Dictionary<string, char[,]> initialGrids = new Dictionary<string, char[,]>();
    ...
    public void SetupNewPuzzle(string word, bool isFirstTime = false)
    {


        if (isFirstTime || !initialGrids.ContainsKey(word))
        {

            if (WordGameManager.Instance.eraGrids.ContainsKey(GameManager.Instance.CurrentEra) &&
                WordGameManager.Instance.eraGrids[GameManager.Instance.CurrentEra].ContainsKey(word)
                )
            {
                initialGrids[word] = WordGameManager.Instance.eraGrids[GameManager.Instance.CurrentEra][word];
            }
            else
            {
                Debug.LogError($"Grid data not found for word: {word} in era: {GameManager.Instance.CurrentEra}");
        ...
        targetWord = word;
        RestoreInitialGrid(targetWord); //Restore the grid from GameManager.Instance.eraGrids
    }
    ...
    private void RestoreInitialGrid(string word)
    {
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                grid[x, y].isSolved = false; // Ensure all tiles are initially NOT solved.

                index++;

                grid[x, y].ResetTile(); // Reset the visual state of the tile.
            }
        }
    }

    public char[,] GenerateGrid(string word)
    {

        targetWord = word;
        char[,] gridData = new char[gridSize, gridSize];
