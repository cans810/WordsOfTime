    ...
    public void SetupNewPuzzle(string word)
{
    ...
    List<char> tiles = GameManager.Instance.GetTilesForWord(GameManager.Instance.CurrentEra, word);
    if (tiles == null)
    {
        Debug.LogError($"Failed to get tiles for word: {word}");
        return;
    }
    ...
    for (int x = 0; x < gridSize; x++)
    {
        ...
        {
            grid[x, y].SetLetter(tiles[index], new Vector2Int(x, y));
            index++;
        }
    }
}
