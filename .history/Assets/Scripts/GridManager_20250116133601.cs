...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        if (WordGameManager.Instance.IsWordSolved(word))
        {
            ...
            {
                // This should never happen if solved words are handled correctly, but adding as a safeguard.
        ...
        else
        {
            ...
            {
                 RestoreInitialGrid(targetWord); // Restore the grid even for unsolved word
            }
            // No clearing of grid needed here!
        }
    }
    ...
    private void RestoreInitialGrid(string word)
    {
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                // Only set solved color if the word is marked as solved in WordGameManager
                if (WordGameManager.Instance.IsWordSolved(word) && word.Contains(initialLetters[index].ToString()))  
                {
                    grid[x, y].SetSolvedColor(); // Only call this for truly solved words!
                }
