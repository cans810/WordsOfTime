...
public class WordGameManager : MonoBehaviour
{
    ...
    private void UpdateSentenceDisplay()
    {
        if (sentenceText != null && !string.IsNullOrEmpty(originalSentence))
        {
            ...
            else
            {
                ...
                else if (hintLevel >= 1) // If hint is active, show first letter
                {
                    string displayWord = targetWord[0] + new string('_', targetWord.Length - 1);
                    displaySentence = originalSentence.Replace("_____", displayWord);
                }
    ...
    public void GiveHint()
    {
        ...
        hintLevel++;
        if (hintLevel > 2) // Reset after second hint
        {
            hintLevel = 0;
        }
