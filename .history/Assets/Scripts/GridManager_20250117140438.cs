...
public class GridManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // If WordGameManager has a current word, set up the grid with it
        if (WordGameManager.Instance != null && !string.IsNullOrEmpty(WordGameManager.Instance.CurrentWord))
        {
            SetupNewPuzzle(WordGameManager.Instance.CurrentWord);
        }
    }
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            // Find and assign references that might have been lost during scene change
            WordGameManager = GameObject.Find("WordGameManager").GetComponent<WordGameManager>();
            
            // If we have a current target word, restore its grid
            if (!string.IsNullOrEmpty(targetWord))
            {
                SetupNewPuzzle(targetWord);
            }
        }
    }
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        {
            Debug.LogError("Attempted to setup puzzle with null or empty word");
            return;
        }
        ...
        // Get the pre-generated grid from GameManager
        List<char> gridData = GameManager.Instance.InitialGrids[word];
        
        // Apply the grid data to the tiles
        int index = 0;
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                {
                    Debug.LogError($"Grid tile at position ({x}, {y}) is null!");
                }
            }
        }
        
        Debug.Log($"Grid restored for word: {word}");
    }
    ...
    private void RestoreInitialGrid(string word)
    {
        ...
        if (!GameManager.Instance.InitialGrids.ContainsKey(word))
        {
            Debug.LogError($"Initial grid for '{word}' not found in GameManager.");
            return;
        }

        List<char> initialLetters = GameManager.Instance.InitialGrids[word];
        
        if (initialLetters == null || initialLetters.Count != gridSize * gridSize)
        {
            Debug.LogError($"Initial letters for '{word}' are invalid or do not match grid size.");
            return;
        }
    ...
    {
        if (GameManager.Instance.GridsGenerated.ContainsKey(targetWord)) return; // Skip if grid already generated
        ...
        if (GameManager.Instance.InitialGrids.ContainsKey(targetWord))
        {
            GameManager.Instance.InitialGrids[targetWord] = initialLetters; // Update existing entry
        }
        else
        {
            GameManager.Instance.InitialGrids.Add(targetWord, initialLetters);
        }

        GameManager.Instance.GridsGenerated[targetWord] = true; // Mark grid as generated
    }
    ...
    {
        grid = new LetterTile[gridSize, gridSize]; // Initialize the grid array
    ...
    public string SelectTargetWord()
    {
        ...
        ResetGridForNewWord();
        SetupNewPuzzle(targetWord); // Add this line to ensure grid is set up with pre-generated data
