    ...
    public void SetupNewPuzzle(string word)
{
    ...
    if (preGeneratedGrid != null)
    {
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                grid[x, y].isSolved = WordGameManager.Instance.solvedWordsInCurrentEra.Contains(
                    WordGameManager.Instance.currentEraWords.IndexOf(word));
    ...
    {
        Debug.LogError($"Failed to get pre-generated grid for word: {word}");
    }
}
