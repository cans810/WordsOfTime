...
public class GridManager : MonoBehaviour
{
    ...
    {
        // Ensure that we place all the letters of the target word first
        ...
        // Randomly shuffle the positions for placing target word letters
        System.Random random = new System.Random();
        emptyPositions = emptyPositions.OrderBy(x => random.Next()).ToList();

        // Place each letter from the target word into the grid
        for (int i = 0; i < targetWord.Length; i++)
        {
            Vector2Int pos = emptyPositions[i];
            grid[pos.x, pos.y].SetLetter(targetWord[i], pos);  // Set the letter from target word
        }

        // Fill the remaining empty positions with random letters
        for (int i = targetWord.Length; i < emptyPositions.Count; i++)
        {
            Vector2Int pos = emptyPositions[i];
            char randomLetter = (char)Random.Range('A', 'Z' + 1);
            grid[pos.x, pos.y].SetLetter(randomLetter, pos);
        }
    }
