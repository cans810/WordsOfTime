...
public class GridManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogWarning("Duplicate GridManager instance detected. Destroying the new one.");
            Destroy(gameObject);
        }
    }
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        {
            GenerateAndStoreInitialGrid(targetWord);
        }
    ...
    public void GenerateAndStoreInitialGrid(string word)
    {
        if (preGeneratedGrids.ContainsKey(word))
        {
            Debug.Log($"Grid for word '{word}' already exists.");
            return;
        }

        List<char> gridData = new List<char>(new char[36]); // 6x6 grid

        // Example: Fill the grid with the word's letters and random letters
        for (int i = 0; i < word.Length; i++)
        {
            gridData[i] = word[i];
        }

        for (int i = word.Length; i < 36; i++)
        {
            gridData[i] = (char)('A' + UnityEngine.Random.Range(0, 26)); // Random letters
        }

        preGeneratedGrids[word] = gridData;
        Debug.Log($"Grid generated and stored for word: {word}");
    }

    public List<char> GetGridForWord(string word)
    {
        if (preGeneratedGrids.TryGetValue(word, out List<char> gridData))
        {
            Debug.Log($"Grid retrieved for word: {word}");
            return gridData;
        }
        else
        {
    ...
    public void SaveGridsToFile(string filePath)
    {
        List<WordGridData> gridDataList = new List<WordGridData>();
        foreach (var kvp in preGeneratedGrids)
        {
            gridDataList.Add(new WordGridData { word = kvp.Key, grid = kvp.Value });
        }

        string json = JsonUtility.ToJson(new WordGridList { grids = gridDataList }, true);
        System.IO.File.WriteAllText(filePath, json);
        Debug.Log($"Grids saved to {filePath}");
    }

    public void LoadGridsFromFile(string filePath)
    {
        if (System.IO.File.Exists(filePath))
        {
            string json = System.IO.File.ReadAllText(filePath);
            WordGridList gridList = JsonUtility.FromJson<WordGridList>(json);

            foreach (var gridData in gridList.grids)
            {
                preGeneratedGrids[gridData.word] = gridData.grid;
            }
            Debug.Log($"Grids loaded from {filePath}");
        }
        else
        {
