...
public class GridManager : MonoBehaviour
{
  ...
  void UpdateLineRenderer(Vector3 position)
    ...
    private void SubmitWord()
    {
        ...
        {
            WordGameManager.Instance.MarkWordAsSolved(targetWord);

            foreach (var tile in selectedTiles)
            {
                tile.SetSolvedColor();
                tile.isSolved = true; // Correctly sets isSolved = true here.
            }

            WordGameManager.HandleCorrectWord(); // Handle correct word logic AFTER setting the tiles.
        }
        else
        {
