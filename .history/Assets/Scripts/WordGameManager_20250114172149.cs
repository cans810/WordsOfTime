...
public class WordGameManager : MonoBehaviour
{
    ...
    private void UpdateProgressBar()
    {
        ...
        for (int i = 0; i < progressImages.Count; i++)
        {
            ...
            if (i < solvedWordCount)
            {
                image.color = Color.green; // Solved words
            }
            else if (i == solvedWordCount) // If it is equal to solvedWordCount
            {
                image.color = Color.red;    // Current word
            }
            else
            {
                image.color = Color.white; // Unsolved words
            }
        }
    }
    ...
    public void HandleCorrectWord()
    {
        ...
        if (nextWord == null)
        {
            return; // No more words (era or game complete)
        }
    ...
    public void StartNewGameInEra() // Call this when starting or changing an era from the main menu
    {
        GridManager.Instance.SelectTargetWord();
        UpdateProgressBar();
        UpdateSentenceDisplay();
    }
