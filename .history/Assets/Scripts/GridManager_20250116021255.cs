...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        // If this word was already solved, don't reset the grid
        if (WordGameManager.Instance.IsWordSolved(word))
        {
            // Just highlight the solved word
            HighlightSolvedWord(); // Use HighlightSolvedWord()
            return; 
        }

        try
        {
            ClearLetters();          // These should now run only when the word isn't already solved
            PlaceWordAdjacent();
            FillRemainingSpaces();
        }
    ...
    private void HighlightSolvedWordTiles()
    {
        // Find all letters of the target word in the grid
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                if (targetWord.Contains(grid[x, y].Letter))
                {
