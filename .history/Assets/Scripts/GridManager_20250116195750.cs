...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        char[,] preGeneratedGrid = GameManager.Instance.GetPreGeneratedGrid(targetWord);
    ...
    public void PlaceWordInGrid(char[,] gridToFill, List<char> letters)
    {
        List<Vector2Int> potentialStartPositions = new List<Vector2Int>();
        for (int x = 0; x < gridSize; x++)
        {
        ...
        foreach (Vector2Int startPos in potentialStartPositions)
        {
            if (TryPlaceWord(gridToFill, letters, startPos)) // Pass gridToFill and letters
            {
                wordPlaced = true;
                break;
            }
    ...
    private bool TryPlaceWord(char[,] gridToFill, List<char> letters, Vector2Int startPos)
    {
        Vector2Int currentPos = startPos;
        ...
        for (int i = 0; i < letters.Count; i++)
        {
            ...
            if (currentPos.x < 0 || currentPos.x >= gridSize || currentPos.y < 0 || currentPos.y >= gridSize || gridToFill[currentPos.x, currentPos.y] != '\0')
            {
                return false; // Placement failed
            }

            gridToFill[currentPos.x, currentPos.y] = letters[i];

            if (i < letters.Count - 1)
            {
                List<Vector2Int> validPositions = GetValidAdjacentPositions(gridToFill, currentPos);
    ...
    private List<Vector2Int> GetValidAdjacentPositions(char[,] gridToCheck, Vector2Int currentPos)
    {
        List<Vector2Int> validPositions = new List<Vector2Int>();
        Vector2Int[] directions = { Vector2Int.right, Vector2Int.down, Vector2Int.left, Vector2Int.up };
        ...
        foreach (var dir in directions)
        {
            ...
            if (newPos.x >= 0 && newPos.x < gridSize &&
                newPos.y >= 0 && newPos.y < gridSize &&
                gridToCheck[newPos.x, newPos.y] == '\0') // Check against gridToCheck
            {
                validPositions.Add(newPos);
            }
        }
