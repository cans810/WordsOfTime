...
public class WordGameManager : MonoBehaviour
{
    ...
    private HashSet<int> solvedWordsInCurrentEra = new HashSet<int>();

    [Header("Tile Colors")]
    [SerializeField] private Color solvedWordColor = Color.yellow;
    ...
    public void LoadWord(int index)
    {
        if (currentEraWords == null || index < 0 || index >= currentEraWords.Count)
        {
            ...
            return; // Just return without changing era
        }
    ...
    // Modify NextWord to handle era completion check
    public void NextWord()
    {
        ...
        // Only allow moving to next word if it's within bounds
        if (currentWordIndex < currentEraWords.Count - 1)
        {
            currentWordIndex++;
            LoadWord(currentWordIndex);
    ...
    {
        if (currentWordIndex > 0) // Check bounds
        {
            currentWordIndex--;
            LoadWord(currentWordIndex);
            UpdateProgressBar(); // Call AFTER updating index and loading
            UpdateSentenceDisplay();
    ...
    {
        Debug.Log($"Setting up new puzzle with word: {word}");
        targetWord = word;
        ...
        try
        {
            ...
            // After placing the word, check if it's already solved
            if (WordGameManager.Instance.IsWordSolved(targetWord))
            {
                HighlightSolvedWord();
            }
        }
    ...
    {
        // Find all tiles that make up the target word
        List<LetterTile> wordTiles = new List<LetterTile>();
        
        // Search through the grid to find the tiles making up the target word
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                if (grid[x, y].Letter != '\0' && targetWord.Contains(grid[x, y].Letter))
                {
                    // Check if this tile is part of the word placement
                    Vector2Int pos = new Vector2Int(x, y);
                    if (IsPartOfTargetWord(pos))
                    {
        ...
        // Set the color of the found tiles
        foreach (var tile in wordTiles)
        {
            tile.SetSolvedColor(solvedWordColor);
        }
    }
