...
public class GridManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (EventSystem.current.currentSelectedGameObject != null && Input.GetKeyDown(KeyCode.Return))
        {
            EventSystem.current.SetSelectedGameObject(null); // This prevents Enter from clicking buttons
        }

        // Check for submit action (Enter key or touch end)
        if (Input.GetKeyDown(KeyCode.Return))
        {
            if (selectedTiles.Count >= 3)
            {
                Debug.Log("Submitting word..."); // Debug log
                GameManager.IValidateWord(selectedTiles);
    ...
    private void HandleTileSelected(LetterTile tile)
    {
        ...
        // Update current word display (if any tiles are selected)
