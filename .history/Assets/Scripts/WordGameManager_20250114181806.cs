    ...
    private void UpdateProgressBar()
{
    ...
    for (int i = 0; i < progressImages.Count; i++)
    {
        ...
        {
            image.color = Color.green; // Or your desired "completed" color
        }
        else if (i == solvedWordCountInCurrentEra)
        {
            image.color = Color.red; // Or your desired "current" color
        }
        else
        {
            image.color = Color.white; // Or your desired "incomplete" color
        }

        // Calculate fill amount based on solved/total words
        float fillAmount = (float)(solvedWordCountInCurrentEra) / progressImages.Count; // Corrected calculation
        image.fillAmount = fillAmount; // Set fill amount for each image
    }
}
