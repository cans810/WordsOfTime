...
public class GridManager : MonoBehaviour
{
   ...
   private void Start()
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        // Get pregenerated grid from GameManager
        List<char> preGeneratedGrid = GameManager.Instance.GetGridForWord(word);
        if (preGeneratedGrid != null && preGeneratedGrid.Count == gridSize * gridSize)
        {
            int index = 0;
            for (int x = 0; x < gridSize; x++)
            {
                for (int y = 0; y < gridSize; y++)
                {
                    ...
                    grid[x, y].SetLetter(preGeneratedGrid[index], new Vector2Int(x, y));

                    // Safely check if word exists in currentEraWords
                    int wordIndex = -1;
                    if (WordGameManager.Instance.currentEraWords != null)
                    {
                        wordIndex = WordGameManager.Instance.currentEraWords.IndexOf(word);
                    }

                    grid[x, y].isSolved = wordIndex != -1 && 
                                        WordGameManager.Instance.solvedWordsInCurrentEra.Contains(wordIndex);
        ...
        {
            Debug.LogError($"Failed to get valid pre-generated grid for word: {word}");
        }
    }

    public void InitializeGrid()
    {
        if (grid != null) return; // Don't reinitialize if already done
        ...
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                ...
                if (grid[x, y] == null)
                {
                    Debug.LogError($"Failed to create tile at ({x}, {y})");
                }
            }
        }
    }
