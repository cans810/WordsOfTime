...
public static class WordValidator
{
    ...
    public static Word GetCurrentWord(string era)
    {
        var wordSet = WordValidator.GetWordSetForEra(era);
        if (wordSet != null && wordSet.words != null)
        {
    ...
    public static List<string> GetWordsForEra(string era)
    {
        var wordSet = GetWordSetForEra(era);
        if (wordSet != null && wordSet.words != null)
        {
            return wordSet.words.Select(w => w.word).ToList();
        }
        return new List<string>();
    }

    private static WordSet GetWordSetForEra(string era)
    {
        // Find the WordSet for the specified era from your loaded data
        return WordSetList.sets?.FirstOrDefault(set => set.era == era);
    }
}