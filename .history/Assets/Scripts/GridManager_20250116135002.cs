...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        if (!initialGrids.ContainsKey(targetWord)) // Check only initialGrids
        {
    ...
    private void GenerateAndStoreInitialGrid()
    {
        ...
        for (int i = 0; i < targetWord.Length; i++) // Iterate over the target word
        {
            for (int x = 0; x < gridSize; x++)
            {
                for (int y = 0; y < gridSize; y++)
                {
                    if (grid[x, y].Letter == targetWord[i]) // Look for the character
                    {
                        solvedPositions.Add(new Vector2Int(x, y));
                        goto NextLetter;  // Found, move to next letter
                    }
                }
            }
        NextLetter:; // Label to skip to the next letter
        }
