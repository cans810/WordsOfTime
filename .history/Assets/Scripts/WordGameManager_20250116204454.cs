...
public class WordGameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            InitializeUI();
            GridManager.Instance.InitializeGrid(); // Make sure grid is initialized first
            StartNewGameInEra();
            CreateProgressBar();
        ...
        {
            // Ensure word list is populated before loading
            ...
            {
                Debug.LogError("Current era words not initialized!");
            }
        }
    }
    ...
    public void StartNewGameInEra()
    {
        ...
        Debug.Log($"Starting new game in era with {currentEraWords.Count} words");
        
        solvedWordsInCurrentEra.Clear();
        solvedWordCountInCurrentEra = 0;
        currentWordIndex = 0;
        
        // Safety check before loading word
        ...
        {
            Debug.LogError("Current word index out of range!");
        }
    }
    ...
    public void LoadWord(int index)
    {
        ...
        {
            Debug.LogError("Current era words list is null!");
            return;
        }

        if (index < 0 || index >= currentEraWords.Count)
        {
            Debug.LogError($"Invalid word index: {index}. Total words: {currentEraWords.Count}");
            return;
        }
        ...
        try
        {
            ...
            {
                Debug.LogError($"Sentence is null or empty for word: {targetWord}");
                return;
            }

            SetupGame(targetWord, sentence);

            // Only setup new puzzle if GridManager is ready
