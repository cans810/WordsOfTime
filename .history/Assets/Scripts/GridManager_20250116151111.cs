    ...
    public void SetupNewPuzzle(string word, bool isFirstTime = false)
{
    ...
    {
        // Access eraGrids from GameManager.Instance, NOT WordGameManager.Instance
        if (GameManager.Instance.eraGrids.ContainsKey(GameManager.Instance.CurrentEra) &&
            GameManager.Instance.eraGrids[GameManager.Instance.CurrentEra].ContainsKey(word))
        {
            initialGrids[word] = GameManager.Instance.eraGrids[GameManager.Instance.CurrentEra][word];
        }
        else
        {
