...
public class GridManager : MonoBehaviour
{
    ...
    public void SetupNewPuzzle(string word)
    {
        ...
        if (preGeneratedGrids.ContainsKey(targetWord))
        {
            RestorePreGeneratedGrid(targetWord);
        }
        else
        {
            Debug.LogError($"Pre-generated grid for word '{targetWord}' not found!");
        }

        WordGameManager.SetupGame(targetWord, WordValidator.GetSentenceForWord(targetWord, GameManager.Instance.CurrentEra));
    }

    public void PreGenerateGrid(string word)
    {
        if (preGeneratedGrids.ContainsKey(word))
        {
            return; // Grid already generated for this word
        }
        ...
        List<char> gridLetters = new List<char>();
        for (int x = 0; x < gridSize; x++)
        {
            for (int y = 0; y < gridSize; y++)
            {
                gridLetters.Add(grid[x, y].Letter);
            }
        }

        preGeneratedGrids[word] = gridLetters;
    }

    public void RestorePreGeneratedGrid(string word)
    {
        if (preGeneratedGrids.TryGetValue(word, out List<char> gridLetters))
        {
            int index = 0;
            for (int x = 0; x < gridSize; x++)
            {
                for (int y = 0; y < gridSize; y++)
                {
                    grid[x, y].SetLetter(gridLetters[index], new Vector2Int(x, y));
                    index++;
                }
            }
        }
        ...
        {
            Debug.LogError($"Pre-generated grid for word '{word}' not found!");
        }
    }
