...
public class WordGameManager : MonoBehaviour
{
    ...
    public void NextWord()
    {
        ...
        Debug.Log($"Current Word Index: {currentWordIndex}, Next Index: {nextIndex}, Era Words Count: {currentEraWords.Count}, Unsolved Words Count: {GameManager.Instance.unsolvedWordsInCurrentEra.Count}");
        ...
        {
            Debug.Log("No more words in this era.");
            ...
            if (GameManager.Instance.CurrentEra != null) // There is a next era
            {
                Debug.Log("Starting new game in the next era...");

                try
                {
                    StartNewGameInEra();

                    if (GridManager.Instance != null) //Check *immediately* after StartNewGameInEra()
                    {
                        GridManager.Instance.ResetGridForNewWord();
                    }
                    else
                    {
                        Debug.LogError("GridManager.Instance is null *after* starting new era!");
                    }
                }
                catch (System.Exception e)
                {
                    Debug.LogError($"Error starting new era in NextWord(): {e.Message}\n{e.StackTrace}");
                }
            }
