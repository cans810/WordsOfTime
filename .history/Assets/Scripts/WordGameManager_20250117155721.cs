...
public class WordGameManager : MonoBehaviour
{
    ...
    private const float POINT_ANIMATION_DURATION = 0.8f; // Reduced from 1.5f
    private const float BUMP_SCALE = 1.2f;
    private const float BUMP_DURATION = 0.08f; // Slightly faster bumps
    ...
    private IEnumerator AnimatePointsIncrease(int startPoints, int endPoints)
    {
        ...
        while (elapsedTime < POINT_ANIMATION_DURATION)
        {
            ...
            // Steeper exponential ease-in
            float t = elapsedTime / POINT_ANIMATION_DURATION;
            t = t == 1f ? 1f : 1f - Mathf.Pow(2, -12 * t); // Changed from -10 to -12
            ...
            if (currentPoints != previousPoints)
            {
                ...
                while (bumpTime < BUMP_DURATION)
                {
                    ...
                    float scale = Mathf.Lerp(BUMP_SCALE, 1f, bumpT);
