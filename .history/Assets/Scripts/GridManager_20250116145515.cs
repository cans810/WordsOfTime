...
public class GridManager : MonoBehaviour
{
  ...
  void UpdateLineRenderer(Vector3 position)
    ...
    public void GenerateAndStoreGrid(string word) // Modified and made public
    {
        if (initialGrids.ContainsKey(word) && gridsGenerated.ContainsKey(word)) return;

        targetWord = word; // Set targetWord directly
        ClearLetters();
        PlaceWordAdjacent();
        FillRemainingSpaces();
        ...
        if (initialGrids.ContainsKey(targetWord))
        {
            initialGrids[targetWord] = initialLetters; // Update
        }
        else
        {
            initialGrids.Add(targetWord, initialLetters);
        ...
        if (!gridsGenerated.ContainsKey(word))
        {
            gridsGenerated.Add(word, true);
        }
