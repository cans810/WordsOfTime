...
public class GridManager : MonoBehaviour
{
    ...
    private void PlaceWordAdjacent()
    {
        ...
        for (int i = 1; i < targetWord.Length; i++)
        {
            ...
            Debug.Log($"Placing letter '{targetWord[i]}' at {currentPos}");
    ...
    public void StartWordSelection(LetterTile tile)
    {
        ...
        ClearSelection();
        AddToSelection(tile);
         lineStartPoint = tile.transform.position;
          if (lineRendererInstance != null)
        {
          lineRendererInstance.gameObject.SetActive(true);
           lineRendererInstance.SetPosition(0,lineStartPoint);
            lineRendererInstance.SetPosition(1,lineStartPoint);
        }
    }
    ...
    public void AddToSelection(LetterTile tile)
{
    ...
    public void EndWordSelection()
    {
        if (IsSelecting)
        {
            ...
            {
                lineRendererInstance.gameObject.SetActive(false);
                  // Clear the line renderer positions to avoid drawing from the last point when starting again
                lineRendererInstance.SetPosition(0, Vector3.zero);
                lineRendererInstance.SetPosition(1, Vector3.zero);
            }
