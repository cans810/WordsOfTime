...
public class WordGameManager : MonoBehaviour
{
    ...
    {
        InitializeUI();
        GameManager.Instance.CreateProgressBarInScene(progressBarContainer, progressImagePrefab);
    }

    public void CreateProgressBarInScene(Transform container, GameObject prefab)
{
    ...
    // Clear existing progress images (if any) in this scene's container
    foreach (Transform child in container)
    {
        Destroy(child.gameObject);
    }

    progressImages = new List<GameObject>(); // Reinitialize the list for this scene

    for (int i = 0; i < EraList.Count; i++)
    {
        GameObject progressImage = Instantiate(prefab, container);
