...
public class WordGameManager : MonoBehaviour
{
    ...
    public void HandleCorrectWord()
    {
        ...
        currentWordIndex++;  // Increment to the next word's index
        LoadWord(currentWordIndex); // Load the next word immediately.

        UpdateProgressBar();
        UpdateSentenceDisplay();

        GridManager.Instance.ResetGridForNewWord(); // Call after LoadWord() to update with the new word
    }
    ...
    public void LoadWord(int index)
    {
        ...
        {
            // Handle invalid index (e.g., end of era)
            Debug.LogWarning("No more words in this era. Returning to menu.");
            GameManager.Instance.MoveToNextEra();  // Advance to the next era
            return;
        }
