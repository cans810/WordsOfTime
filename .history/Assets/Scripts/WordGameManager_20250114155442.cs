...
public class WordGameManager : MonoBehaviour
{
    ...
    {
        Word currentWord = WordValidator.GetCurrentWord(GameManager.Instance.CurrentEra);
        if (currentWord != null)
        {
            targetWord = currentWord.word;
            string sentence = currentWord.sentences[Random.Range(0, currentWord.sentences.Length)];
    ...
    {
        float progress = GameManager.Instance.GetEraProgress(GameManager.Instance.CurrentEra);
        string progressText = $"Word {GameManager.Instance.GetCurrentWordIndex() + 1} of {WordValidator.GetWordsForEra(GameManager.Instance.CurrentEra).Count}";
    ...
    public void HandleCorrectWord()
    {
        ...
        // Either load next word or return to menu if era is complete
        if (GameManager.Instance.GetCurrentWordIndex() < WordValidator.GetWordsForEra(GameManager.Instance.CurrentEra).Count)
        {
            Invoke(nameof(SetupCurrentWord), MESSAGE_DISPLAY_TIME);
        }
        else
        {
            Invoke(nameof(ReturnToMainMenu), MESSAGE_DISPLAY_TIME);
        }
    }
