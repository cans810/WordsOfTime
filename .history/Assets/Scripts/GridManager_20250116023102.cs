...
public class GridManager : MonoBehaviour
{
    ...
    private void RestoreSolvedWord(string word)
    {
        if (solvedWordPositions.TryGetValue(word, out List<Vector2Int> positions))
        {
            ...
            {
                grid[pos.x, pos.y].SetLetter(word[positions.IndexOf(pos)], pos, word);
    ...
    private void PlaceWordAdjacent()
    {
        ...
        foreach (Vector2Int startPos in potentialStartPositions)
        {
            if (TryPlaceWord(startPos))
            {
                ...
                // Store solved word positions ONLY if the word was successfully placed
                if (solvedWordPositions.ContainsKey(targetWord))
                {
                    solvedWordPositions[targetWord].Clear(); // Clear old positions
                }
                else
                {
                ...
                for (int x = 0; x < gridSize; x++)
                {
                    for (int y = 0; y < gridSize; y++)
                    {
                        if (grid[x, y].Letter != '\0' && targetWord.Contains(grid[x, y].Letter.ToString()))
                        {
                            solvedWordPositions[targetWord].Add(new Vector2Int(x, y));
                        }
                    }
                }
        ...
        if (!wordPlaced)
        {
            // ... handle error (no valid placement found)
        }
    }
