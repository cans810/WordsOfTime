...
public class WordGameManager : MonoBehaviour
{
    ...
    public void LoadWord(int index)
    {
        ...
        if (GridManager.Instance != null) // Check for null before accessing
        {
            GridManager.Instance.ResetGridForNewWord();
        }
        else
        {
            Debug.LogError("GridManager.Instance is null!"); // Handle the error (e.g., go back to main menu)
        }
    }
    ...
    Debug.Log($"Current Word Index: {currentWordIndex}, Next Index: {nextIndex}, Era Words Count: {currentEraWords.Count}, Unsolved Words Count: {GameManager.Instance.unsolvedWordsInCurrentEra.Count}"); //Crucial debug log

    if (nextIndex < currentEraWords.Count)
    {
        if (GameManager.Instance == null || GameManager.Instance.unsolvedWordsInCurrentEra == null)
        {
            Debug.LogError("GameManager or unsolvedWordsInCurrentEra is null!");
            return;
        }
        if (!GameManager.Instance.unsolvedWordsInCurrentEra.Contains(targetWord))
        {
            Debug.LogError($"Target word '{targetWord}' not found in unsolvedWordsInCurrentEra");
            // Handle the error (e.g., reset the era or go to the main menu)
            return;
        }

        try
        {
            GameManager.Instance.unsolvedWordsInCurrentEra.Remove(targetWord);  // Removing can cause an error if targetWord is null

            LoadWord(nextIndex);  // Add a try-catch block here too
            UpdateProgressBar();      // Add try-catch here if necessary
            UpdateSentenceDisplay();   // Add try-catch here if necessary
            solvedWordCountInCurrentEra++;
            GridManager.Instance.ResetGridForNewWord();
        }
        catch (System.Exception e)
        {
            Debug.LogError($"Error in NextWord (inner block): {e.Message}\n{e.StackTrace}");
        }
    } 
            else 
            {
                Debug.Log("Game Complete!"); // All eras finished
                // Implement your game complete logic here (e.g., load end screen)
            }
