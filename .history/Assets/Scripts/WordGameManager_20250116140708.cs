...
public class WordGameManager : MonoBehaviour
{
    ...
    public void HandleCorrectWord()
    {
        ...
        if (solvedWordsInCurrentEra.Count == currentEraWords.Count)
        {
            ...
            GameManager.Instance.MoveToNextEra(); // Or handle era completion as needed
            StartNewGameInEra();
            return; 
        }
        ...
        {
            currentWordIndex++;
            LoadWord(currentWordIndex); // Load the next word
        }
        else
        {
            Debug.Log("End of current Era!  Moving to the next era.");
            GameManager.Instance.MoveToNextEra(); // Or handle era completion differently
            StartNewGameInEra();
            return; // Return to prevent further execution in this era
        }
