    ...
    private void PlaceWordAdjacent()
{
    ...
    // Validate if the grid has space to place the word
    if (grid[currentPos.x, currentPos.y].Letter != '\0')
    {
        Debug.LogError("Middle position is already occupied. Cannot place the word.");
        return;
    }

    // Place the first letter in the middle
    ...
    for (int i = 1; i < targetWord.Length; i++)
    {
        ...
        // Randomly select a valid position to add some variety
